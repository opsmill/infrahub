...
test.describe("Object update", () => {
  ...
  test("should contain initial values and update them", async ({ page }) => {
    ...
    await test.step("update the object", async () => {
      ...
      await Promise.all([
        page.waitForResponse((response) => {
          ...
          return reqData?.operationName === "InfraDeviceUpdate" && status === 200;
        }),
    ...
    await test.step("assert the updates", async () => {
      ...
      await page.getByTestId("side-panel-container").getByText("greenred").scrollIntoViewIfNeeded(); // Scroll down to display tags
      await expect(page.getByTestId("side-panel-container").getByText("greenred")).toBeVisible();
    });
  });
});
