...
const getFormStructureForCreateEdit = (
  ...
  schema.attributes?.forEach((attribute) => {
    ...
    formFields.push({
      ...
      type: attribute.enum
        ? "select"
        : getFormInputControlTypeFromSchemaAttributeKind(
            attribute.kind as SchemaAttributeType
          ),
  ...
  schema.relationships
    ...
    .forEach((relationship) => {
      ...
      if (
        !isInherited &&
        dropdownOptions[schemaKindNameMap[relationship.peer]]
      ) {
      ...
      formFields.push({
        name:
          relationship.name +
          (relationship.cardinality === "one" ? ".id" : ".list"),
        kind: "String",
        type:
...
export const getFormStructureForMetaEdit = (
  ...
  const sourceOwnerFields =
    type === "attribute"
      ? ["owner", "source"]
      : ["_relation__owner", "_relation__source"];
  const booleanFields =
    type === "attribute"
  ...
  const sourceOwnerFormFields: DynamicFieldData[] = sourceOwnerFields.map(
    (f) => {
