...
const getMutationDetailsFromFormData = (
  ...
  schema.attributes?.forEach((attribute) => {
    ...
    if (mode === "update") {
      ...
      if (
        mode === "update" &&
        (!updatedValue || JSON.stringify(updatedValue) === existingValue)
      )
  ...
  schema.relationships
    ...
    .forEach((relationship) => {
      ...
      if (mode === "update") {
        if (isOneToOne) {
          const existingValue = JSON.stringify(
            existingObject[relationship.name]
          );
      ...
      if (
        isOneToOne &&
        updateObject[relationship.name] &&
        !updateObject[relationship.name].id
      ) {
        delete updateObject[relationship.name];
      }
      ...
      if (
        isOneToMany &&
        updateObject[relationship.name] &&
        updateObject[relationship.name].list
      ) {
        const fieldKeys = Object.keys(updateObject[relationship.name]).filter(
          (key) => key !== "list"
        );

        updateObject[relationship.name] = updateObject[
          relationship.name
        ].list.map((row: any) => {
