  ...
  const getFormFieldByInputType = (inputType: ControlType) => {
    ...
    switch (inputType) {
      ...
      case "select2step": {
        ...
        return <OpsSelect2StepRegister {...props} options={selectOptions} value={existingValue ?? { parent: value?.__typename?.replace(regex, ""), child: value?.id}} register={register} setValue={setValue} />;
      }
      case "multiselect": {
        ...
        return <OpsMultiSelectRegister {...props} options={options.values} value={existingValue ?? currentValue} register={register} setValue={setValue} />;
