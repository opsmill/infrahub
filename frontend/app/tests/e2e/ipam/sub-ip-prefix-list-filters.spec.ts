...
test.describe("/ipam - IP Prefix List Filtering", () => {
  test("should filter IP prefixes by search text and column filtering", async ({ page }) => {
    ...
    await test.step("verify initial sub prefix list", async () => {
      ...
      await expect(
        page.getByTestId("identifier-cell").getByRole("link", { name: "10.2.0.0/16" })
      ).toBeVisible();
    });
    ...
    await test.step("add filtering by search text", async () => {
      await page.getByPlaceholder("Search IP Prefix").fill("10.1");
