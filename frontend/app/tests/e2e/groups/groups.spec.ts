...
test.describe("/objects/CoreGroup - Generic Group Object.", () => {
  ...
  test("1. Create a new Standard Group", async ({ page }) => {
    await page.goto("/");
    await page.getByTestId("sidebar-menu").getByRole("link", { name: "All Groups" }).click();
  ...
  test("2. Add members to Standard Group", async ({ page }) => {
    ...
    await test.step("add members to Standard Group", async () => {
      ...
      await page.getByTestId("side-panel-container").getByTestId("select-input").click();
      await page
        .getByTestId("side-panel-container")
        .getByTestId("select-open-option-button")
        .click();
      await page.getByTestId("side-panel-container").getByText("Tag", { exact: true }).click();
