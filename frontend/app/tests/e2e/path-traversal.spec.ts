...
test.describe
	.skip("/path-traversal", () => {
		...
		test("should display mode toggle with Path and Dependencies buttons", async ({
			page,
		}) => {
			await page.goto("/path-traversal");
		...
		test("should show empty state message", async ({ page }) => {
			...
			await expect(
				page.getByText('Select two objects and click "Find Paths"'),
			).toBeVisible();
		});
		...
		test("auto-runs the query when source and destination are present in the URL", async ({
			...
			test.skip(
				...
				"Demo dataset has < 2 InfraDevice objects",
			);
		...
		test("shows validation message when submitting without a source", async ({
			page,
		}) => {
			await page.goto("/path-traversal");
		...
		test("UUID pasted into the source picker resolves to a single match", async ({
			page,
		}) => {
			// Get a known device id from the demo dataset.
			await page.goto("/objects/InfraDevice");
			const firstLink = page
				.getByRole("link", { name: /-edge|-leaf|-spine|-core/i })
				.first();
			...
			const searchInput = page.getByPlaceholder(
				/search by name|paste an object id/i,
			);
