...
test.describe("Object filters", () => {
  ...
  test.describe("header and toolbar parity", () => {
    test("should produce identical filter state from the header menu and the toolbar filter picker", async ({
      page,
    }) => {
      const roleHeader = page.getByTestId("object-items").getByRole("button", { name: "Role" });
      const roleHeaderFilterIcon = roleHeader.locator('iconify-icon[icon="mdi:filter-variant"]');
      let headerFiltersParam: string | null = null;

      await test.step("navigate and verify initial state", async () => {
        await page.goto("/objects/InfraDevice");
        await expect(page.getByRole("heading", { name: "Device" })).toBeVisible();
        await expect(page.getByRole("link", { name: "atl1-core1" })).toBeVisible();
        await expect(roleHeaderFilterIcon).toBeHidden();
      });
      ...
      await test.step("apply a filter via the header menu and capture the URL state", async () => {
        ...
        await expect(page.getByRole("row", { name: "Role contains edge" })).toBeVisible();
        await expect(roleHeaderFilterIcon).toBeVisible();
        await expect(page).toHaveURL(/filters=/);
        headerFiltersParam = new URL(page.url()).searchParams.get("filters");
        expect(headerFiltersParam).not.toBeNull();
      });

      await test.step("remove the filter via its tag and verify the header icon clears", async () => {
        await page.getByRole("button", { name: "Remove Role contains edge" }).click();
        await expect(page.getByRole("row", { name: "Role contains edge" })).not.toBeVisible();
        await expect(roleHeaderFilterIcon).toBeHidden();
        await expect(page).not.toHaveURL(/filters=/);
      });

      await test.step("apply the same filter via the toolbar filter picker and verify identical URL state", async () => {
        await page.getByRole("button", { name: "Filter" }).click();
        await page
          .getByRole("listbox", { name: "Filter fields" })
          .getByRole("option", { name: "Role" })
        ...
        await expect(roleHeaderFilterIcon).toBeVisible();
        await expect(page).toHaveURL(/filters=/);
        expect(new URL(page.url()).searchParams.get("filters")).toBe(headerFiltersParam);
      });
      ...
      await test.step("clear filters and verify initial state", async () => {
        await page.getByTestId("filter-reset-button").click();
        await expect(page.getByRole("row", { name: "Role contains edge" })).not.toBeVisible();
        await expect(roleHeaderFilterIcon).toBeHidden();
        await expect(page.getByRole("link", { name: "atl1-core1" })).toBeVisible();
