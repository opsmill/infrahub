...
test.describe("/proposed-changes diff data", () => {
  ...
  test("should verify the diff data with conflicts", async ({ page }) => {
    ...
    await test.step("check diff data", async () => {
      await expect(page.getByText("UpdatedInfraInterfaceL3Ethernet1")).toBeVisible();
      await expect(page.getByRole("link", { name: "Ethernet1" })).toBeVisible();
      await expect(page.getByText("UpdatedInfraDeviceden1-edge1")).toBeVisible();
      ...
      await page.getByText("UpdatedInfraInterfaceL3Ethernet1").click();
      await expect(
        page.getByText("UpdatedInfraInterfaceL3Ethernet1 main den1-maintenance-")
      ).toBeVisible();
  ...
  test("should comment a proposed changes", async ({ page }) => {
    ...
    await test.step("comment proposed changes", async () => {
      await page.locator("span").filter({ hasText: "UpdatedInfraDeviceden1-edge1" }).hover();
      await page
        .locator("span")
        .filter({ hasText: "UpdatedInfraDeviceden1-edge1" })
