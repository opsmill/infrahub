...
import { getProposedChangeDetailsFromApi } from "@/entities/proposed-changes/api/get-proposed-change-details-from-api";
...
describe("watched proposed-change polls emit X-Priority: high (FR-005)", () => {
  afterEach(() => vi.restoreAllMocks());

  it("PC-details poll (get-proposed-change-details.query.ts) inherits high", async () => {
    const context = await capturedContext(() =>
      getProposedChangeDetailsFromApi({ proposedChangeId: "pc-1" })
    );
    expect(await priorityHeaderForContext(context)).toBe("high");
  });

  it("PC-events poll (get-events.query.ts) inherits high", async () => {
    const context = await capturedContext(() => getEventsFromApi({}));
    expect(await priorityHeaderForContext(context)).toBe("high");
  });
});
