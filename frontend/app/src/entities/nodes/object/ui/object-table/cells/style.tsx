  ...
  return (
    <TableCell
      className={classNames(
        "sticky left-0 z-1",
        isMuted ? cellMutedStyle : "bg-white dark:bg-slate-700",
        className
      )}
      {...props}
    >
      {children}
      <div className="-right-4 pointer-events-none absolute top-0 bottom-0 w-4 bg-gradient-to-r from-gray-500/10 dark:from-gray-700/30" />
    </TableCell>
  );
}
...
export function StickyRightCell({ className, isMuted, children, ...props }: StickyCellProps) {
  return (
    <TableCell
      ...
      {...props}
    >
      <div className="-left-4 pointer-events-none absolute top-0 bottom-0 w-4 bg-gradient-to-r from-transparent to-gray-300/30 dark:to-gray-600/30" />
      {children}
    </TableCell>
  );
}
