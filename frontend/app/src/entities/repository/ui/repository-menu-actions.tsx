...
export function RepositoryActionsMenu({
	...
	const [reimportLastCommit] = useMutation(REIMPORT_LAST_COMMIT, {
			...
			if (data?.InfrahubRepositoryProcess?.ok) {
				...
				toast(
					<Alert
						...
						message="Failed to start import from remote."
					/>,
				);
			}
		},
		onError: (error) => {
			toast(
				<Alert
					type={ALERT_TYPES.ERROR}
					message={`Error importing from remote: ${error.message}`}
				/>,
			);
		},
	});

	const [importCurrentCommit] = useMutation(
		IMPORT_READONLY_REPOSITORY_LAST_COMMIT,
		{
			variables: {
				id: repositoryId,
			...
			onCompleted: async (data) => {
				if (data?.InfrahubReadOnlyRepositoryImportLastCommit?.ok) {
					const taskId =
						data.InfrahubReadOnlyRepositoryImportLastCommit.task?.id;
					const message = taskId ? (
						<>
					...
					toast(
						<Alert
							type={ALERT_TYPES.ERROR}
							message="Failed to start import of current commit."
						/>,
					);
				}
			},
	...
	return (
		<RepositoryMenuSection
			...
			onReimportCurrentCommit={
				isReadOnlyRepository ? () => importCurrentCommit() : undefined
			}
			permission={permission}
		/>
	);
...
const CheckConnectivityModal = ({
	...
	const [checkConnectivity, { loading, data, error, called, reset }] =
		useMutation(CHECK_REPOSITORY_CONNECTIVITY, {
				...
				<Dialog.Panel className="max-w-lg space-y-4 rounded-lg border border-gray-200 bg-white p-4">
					{!showResult ? (
						<>
							...
							<Dialog.Description>
								Check the connectivity to this repository to validate your
						...
						<>
							...
							<Dialog.Description>
								{data?.InfrahubRepositoryConnectivity?.message ||
									error?.message}
							</Dialog.Description>
