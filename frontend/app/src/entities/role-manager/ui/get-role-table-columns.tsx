...
export function getRoleTableColumns(schema: ModelSchema): Array<ColumnDef<NodeObject>> {
  const attributesVisible = (schema.attributes ?? []).filter(
    ({ name }) => ROLE_TABLE_ATTRIBUTES.includes(name) && name !== "name"
  );
  const relationshipsVisible = (schema.relationships ?? []).filter(({ name }) =>
    ROLE_TABLE_RELATIONSHIPS.includes(name)
  );
