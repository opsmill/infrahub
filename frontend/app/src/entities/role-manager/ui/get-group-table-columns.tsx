...
export function getGroupTableColumns(schema: ModelSchema): Array<ColumnDef<NodeObject>> {
  const attributesVisible = (schema.attributes ?? []).filter(
    ({ name }) => GROUP_TABLE_ATTRIBUTES.includes(name) && name !== "name"
  );
  const relationshipsVisible = (schema.relationships ?? []).filter(({ name }) =>
    GROUP_TABLE_RELATIONSHIPS.includes(name)
  );
