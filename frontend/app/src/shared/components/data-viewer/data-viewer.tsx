...
export function DataViewer({
	...
	const config =
		viewerType.type === "text"
			? TEXT_CONTENT_TYPE_CONFIG[viewerType.textContentType]
			: null;
	...
	return (
		<Col
			className={classNames(
				"grow rounded-lg bg-neutral-800 p-2 text-neutral-200",
				className,
			)}
		>
			<Row>
...
function DataViewerContent({
	...
	switch (viewerType.type) {
		case "text": {
			return (
				<TextContent
					textContentType={viewerType.textContentType}
					content={content}
				/>
			);
		}
...
function TextContent({
	...
	const config =
		TEXT_CONTENT_TYPE_CONFIG[textContentType] ??
