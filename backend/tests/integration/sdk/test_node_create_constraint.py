...
class TestSDKNodeCreateConstraints(TestInfrahubApp):
    ...
    async def test_step_01_add_node_success(
        self, client: InfrahubClient, initial_dataset, schema_02_uniqueness_constraint
    ):
        await client.schema.load(schemas=[schema_02_uniqueness_constraint])
        ...
        new_car = await client.create(
            kind=CAR_KIND, name="New", owner=john_person, manufacturer=honda_manufacturer, color=ACCORD_COLOR
        )
        await new_car.save()
    ...
    async def test_step_02_add_node_failure(
        ...
        new_car_2 = await client.create(
            kind=CAR_KIND, name="New2", owner=john_person, manufacturer=renault_manufacturer, color=ACCORD_COLOR
        )

        with pytest.raises(GraphQLError) as exc:
            await new_car_2.save()
