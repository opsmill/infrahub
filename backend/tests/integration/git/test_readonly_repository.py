...
class TestCreateReadOnlyRepository(TestInfrahubApp):
    ...
    async def test_create_repository(
        ...
        """Validate that we can create a read-only repository, that it gets updated with the commit id and that objects are created."""
        ...
        repository = await NodeManager.get_one_by_id_or_default_filter(
            db=db, id=client_repository.id, schema_name=InfrahubKind.READONLYREPOSITORY
        )
