"""One rebase carrying every kind of schema divergence at once, refused first and then accepted.

The rebase twin of ``test_merge_combined_schema_changes``: the same divergence between the branch and
main, but main's changes are written onto the branch. A regex main tightened, an attribute the branch
made unique, a rename on each side, a kind renamed on the branch and one removed on main, and two
properties of one element changed one per side.

Conflicts are absent by construction: a rebase refuses any conflict, resolved or not
(``core/branch/tasks.py``), which ``component/core/test_branch_rebase.py`` pins.

Phase one asserts the rebase is refused with every offender named in a single report and that the
branch did not move. Phase two repairs the offending data on the branch, rebases, and inspects the
branch's schema and data for every concern.
"""
...
class TestRebaseCombinedSchemaChanges(TestSchemaLifecycleBase):
    ...
    async def branch_2(self, db: InfrahubDatabase, initial_dataset: dict[str, str]) -> Branch:
        ...
        return await create_branch(db=db, branch_name="combined_schema_changes_rebase")
    ...
    async def test_step01_the_rebase_is_refused_with_every_offender_named(
        self,
        db: InfrahubDatabase,
        default_branch: Branch,
        client: InfrahubClient,
        ...
        # Pins where the refusal comes from: the post-rebase diff refresh mentions the regex too.
        ...
        assert "attribute.unique.update" in message

        offender_count = await NodeManager.count(
            db=db, schema=PERSON_KIND, branch=branch_2, filters={"name__value": ILLEGAL_NAME}
        )
        assert offender_count == 1, "a refused rebase leaves the offending object on the branch"
        refused_branch = await Branch.get_by_name(db=db, name=branch_2.name)
        assert refused_branch.status == BranchStatus.OPEN.value
        assert refused_branch.branched_from == branched_from_before, "a refused rebase moves no fork point"
        branch_person = registry.schema.get_node_schema(name=PERSON_KIND, branch=branch_2)
        name_parameters = branch_person.get_attribute(name="name").parameters
        assert isinstance(name_parameters, TextAttributeParameters)
        assert name_parameters.regex == PERMISSIVE_NAME_REGEX, "a refused rebase writes no schema onto the branch"

    async def test_step02_the_repaired_branch_rebases(
        self,
        db: InfrahubDatabase,
        client: InfrahubClient,
        branch_2: Branch,
        ...
        # civic here would be a conflict, and a rebase refuses every conflict.
        ...
        rebased_branch = await Branch.get_by_name(db=db, name=branch_2.name)
        assert rebased_branch.branched_from > branched_from_before
        rebased = await registry.schema.load_schema_from_db(db=db, branch=rebased_branch)
        assert rebased.get_hash() == registry.schema.get_schema_branch(name=branch_2.name).get_hash()

        # 1. main's tightened regex reached the branch
        person = rebased.get(name=PERSON_KIND)
        name_parameters = person.get_attribute(name="name").parameters
        assert isinstance(name_parameters, TextAttributeParameters)
        assert name_parameters.regex == STRICT_NAME_REGEX

        # 2. the branch's own unique flag survived; 3. its rename kept main's max_length
        car = rebased.get(name=CAR_KIND)
        ...
        # 5. the branch's kind rename, with the peer pointing at the new name and the generated kinds along
        assert car.get_relationship(name="manufacturer").peer == MAKER_KIND
        assert rebased.has(name=MAKER_KIND)
        assert rebased.has(name=f"Profile{MAKER_KIND}")
        assert rebased.has(name=f"Template{MAKER_KIND}")
        for old_name in (MANUFACTURER_KIND_01, f"Profile{MANUFACTURER_KIND_01}", f"Template{MANUFACTURER_KIND_01}"):
            assert not rebased.has(name=old_name), old_name

        # 4. two properties of one relationship, one per side; 6. main's rename of the tag attribute
        tag = rebased.get(name=TAG_KIND)
        ...
        # 7. the kind main removed is gone from the branch
        assert not rebased.has(name=INTERIOR_KIND)

    async def test_step03_the_rebased_data_matches_the_rebased_schema(
        self,
        db: InfrahubDatabase,
        branch_2: Branch,
        initial_dataset: dict[str, str],
        ...
        branch = await Branch.get_by_name(db=db, name=branch_2.name)
        persons = await NodeManager.query(db=db, schema=PERSON_KIND, branch=branch)
        assert sorted(str(person.name.value) for person in persons) == ["Bob", "Jane", "John"]

        accord = await NodeManager.get_one(db=db, id=initial_dataset["accord"], branch=branch, raise_on_error=True)
        assert accord.color.value == REPAIRED_COLOR
        civic = await NodeManager.get_one(db=db, id=initial_dataset["civic"], branch=branch, raise_on_error=True)
        assert civic.color.value == ACCORD_COLOR, "main's post-fork edit is visible from the rebased branch"
        assert civic.get_attribute("notes").value == "Honda Civic"

        makers = await NodeManager.query(db=db, schema=MAKER_KIND, branch=branch)
        assert {maker.id for maker in makers} == {initial_dataset["honda"], initial_dataset["renault"]}

        green = await NodeManager.get_one(
            db=db, id=data_changed_on_both_sides["green"], branch=branch, raise_on_error=True
        )
        assert green.get_attribute("label_text").value == "green", "main's rename reached the branch's own rows"
