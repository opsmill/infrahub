...
class TestSchemaLifecycleValidatorMain(TestSchemaLifecycleBase):
    ...
    # @pytest.fixture(scope="class")
    # async def initial_dataset(self, db: InfrahubDatabase, initialize_registry, schema_network):
    #     await load_schema(db=db, schema=schema_network)
    ...
    async def test_step_01_create_branch(
    ...
    async def test_step_02_load_schema(
        self, client: InfrahubClient, schema_network
    ):
