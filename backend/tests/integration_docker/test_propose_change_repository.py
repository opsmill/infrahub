...
class TestProposeChangeRepository(TestInfrahubDockerClient, SchemaCarPerson):
    """Proposed-change and triggered-action tests sharing the car-dealership dataset on one stack.

    The trigger test must stay last: earlier tests mutate persons and the group, and no
    trigger rule may exist yet when they do.
    """
    ...
    async def test_create_main_triggers(
        self, client: InfrahubClient, default_branch: str, prefect_client: PrefectClient
    ) -> None:
