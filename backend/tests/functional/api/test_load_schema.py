    ...
    "nodes": [
        {
            "name": "Child",
            "namespace": "Bug5b",
            "inherit_from": ["Bug5bParent"],
            "include_in_menu": False,
        }
    ],
}

_BUG5B_SWITCH_GENERIC_SCHEMA = {
    "version": "1.0",
    "generics": [
        {
            "name": "Parent",
    ...
    "generics": [
        {
            ...
            "namespace": "Bug5b",
            "attributes": [
                {"name": "old_id", "kind": "Number", "state": "absent"},
            ],
        }
    ],
}
...
class TestLoadSchemaInheritedIdentityAttributeRemoval(TestInfrahubApp):
    """Regression test for orphaned uniqueness_constraints / human_friendly_id
    on inheritors after a generic identity field is migrated away from an attribute
    and that attribute is then removed via state: absent.

    Reproduces the 3-step flow:
      1. Baseline: generic Bug5bParent with attrs old_id + new_id, hfid=[old_id__value].
         Node Bug5bChild inherits from Bug5bParent.
         Both generic and child end up with uniqueness_constraints=[[old_id__value]].
      2. Switch generic hfid to [new_id__value]. Identity fields do not cascade,
         so Bug5bChild still carries uniqueness_constraints=[[old_id__value]] — this
         is the inconsistent state that the bug is about.
      3. Remove old_id from the generic with state: absent.
         Today this raises HTTP 500 from the post-write schema.process() in
         sync_uniqueness_constraints_and_unique_attributes, and the write partially
         commits, wedging the branch.

    Expected: step 3 either succeeds (identity fields cascade) or fails cleanly
    with a 4xx error. Either way, the branch must remain loadable afterward.
    """
    ...
    async def test_bug_orphaned_uniqueness_constraints_on_attribute_removal(
        self,
        ...
        baseline = await client.schema.load(schemas=[_BUG5B_BASELINE_SCHEMA])
        assert not baseline.errors
        assert baseline.schema_updated

        parent_schema = await client.schema.get(kind="Bug5bParent", refresh=True)
        child_schema = await client.schema.get(kind="Bug5bChild", refresh=True)
        assert parent_schema.human_friendly_id == ["old_id__value"]
        assert parent_schema.uniqueness_constraints == [["old_id__value"]]
        assert child_schema.human_friendly_id == ["old_id__value"]
        assert child_schema.uniqueness_constraints == [["old_id__value"]]

        # Step 2: switch generic identity to new_id
        switch = await client.schema.load(schemas=[_BUG5B_SWITCH_GENERIC_SCHEMA])
        assert not switch.errors
        assert switch.schema_updated

        parent_schema = await client.schema.get(kind="Bug5bParent", refresh=True)
        child_schema = await client.schema.get(kind="Bug5bChild", refresh=True)
        assert parent_schema.human_friendly_id == ["new_id__value"]
        # Identity fields do not cascade today — child still references old_id.
        # This is the inconsistent state that triggers the bug in step 3.
        assert child_schema.uniqueness_constraints == [["old_id__value"]]

        # Step 3: remove old_id from the generic. This is what blows up today —
        # the post-write schema.process() reruns sync_uniqueness_constraints_and_
        # unique_attributes, which fails because the child's uniqueness_constraints
        # still reference old_id__value. The failure itself is acceptable — the
        # bug is that the failed update is not rolled back, leaving the branch
        # with old_id deleted from the generic but still referenced by the child.
        remove_failed = False
        try:
            remove = await client.schema.load(schemas=[_BUG5B_REMOVE_OLD_SCHEMA])
        except Exception:
            remove_failed = True
        else:
            # If the server returned a structured response, it must be either a
            # clean 4xx (surfaced via .errors) or a success that cascaded.
            remove_failed = bool(remove.errors)

        # Whether the load failed loudly (exception / 5xx) or cleanly (.errors),
        # if it did not succeed then the schema must look exactly like it did
        # before step 3 — old_id still present on the generic, and the child's
        # stale uniqueness_constraints untouched. This is what's broken today:
        # the attribute delete commits to the DB even though schema.process()
        # raised, leaving the branch in an unrecoverable state.
        if remove_failed:
            parent_schema = await client.schema.get(kind="Bug5bParent", refresh=True)
            child_schema = await client.schema.get(kind="Bug5bChild", refresh=True)
            parent_attrs = {a.name for a in parent_schema.attributes}
            assert "old_id" in parent_attrs, (
                f"Failed schema load was not rolled back: old_id missing from Bug5bParent. "
                f"Remaining attributes: {sorted(parent_attrs)}"
            )
            assert "new_id" in parent_attrs
            assert child_schema.uniqueness_constraints == [["old_id__value"]]
        else:
            # Cascade path: the inheritor's identity fields should have been
            # updated to no longer reference the removed attribute.
            parent_schema = await client.schema.get(kind="Bug5bParent", refresh=True)
            child_schema = await client.schema.get(kind="Bug5bChild", refresh=True)
            assert "old_id" not in {a.name for a in parent_schema.attributes}
            for constraint_paths in child_schema.uniqueness_constraints or []:
                assert "old_id__value" not in constraint_paths, (
                    f"Child still references removed attribute: {constraint_paths}"
                )

        # Regardless of which outcome we took, the branch must not be wedged —
        # a follow-up schema load on the branch must still succeed. Today this
        # fails with the same error on every load, because the orphaned child
        # constraint survived the partially-committed delete.
        followup = await client.schema.load(schemas=[_BUG5B_BASELINE_SCHEMA])
        assert not followup.errors, f"Branch is wedged — follow-up schema load failed: {followup.errors}"
