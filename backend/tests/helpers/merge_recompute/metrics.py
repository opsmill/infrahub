...
class CostCenterTiming:
    ...
    ``recompute_total_s`` is the summed duration of the recompute runs and stays ``None``
    unless that sum is actually available: it is not the same quantity as the wall-clock
    window, which overlaps concurrent runs, so reporting the window under both names
    would overstate one of them.

    ``recompute_nodes_written`` is the count of nodes whose derived value the pass
    actually changed. It is the quantity that catches a design trading fewer jobs for
    more work, which a flow-run count alone cannot see.
    """

    merge_critical_path_s: float
    recompute_window_s: float
    recompute_flow_runs: int
    recompute_total_s: float | None = None
    recompute_nodes_written: int | None = None
