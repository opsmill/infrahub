...
from infrahub.api.dev_diff import get_display_labels, get_display_labels_per_kind
...
async def test_diff_data_endpoint_branch_only_default(session, client, client_headers, car_person_data_generic_diff):
    ...
    with client:
        response = client.get(
            "/dev/diff/data?branch=branch2&branch_only=true",
    ...
    assert branch2[c1]["kind"] == "ElectricCar"
    ...
    assert branch2[r1]["kind"] == "Repository"
...
async def test_diff_data_endpoint_branch_time_from(session, client, client_headers, car_person_data_generic_diff):
    ...
    with client:
        response = client.get(
            f"/dev/diff/data?branch=branch2&branch_only=true&time_from={time20.to_iso8601_string()}",
            headers=client_headers,
        )
...
async def test_diff_data_endpoint_branch_time_from_to(session, client, client_headers, car_person_data_generic_diff):
    ...
    with client:
        response = client.get(
            f"/dev/diff/data?branch=branch2&branch_only=true&time_from={time0.to_iso8601_string()}&time_to={time20.to_iso8601_string()}",
            headers=client_headers,
        )
...
async def test_diff_data_endpoint_with_main_default(session, client, client_headers, car_person_data_generic_diff):
    ...
    with client:
        response = client.get(
            "/dev/diff/data?branch=branch2&branch_only=false",
            headers=client_headers,
        )
    ...
    assert main[c2]["kind"] == "ElectricCar"
...
async def test_diff_data_endpoint_with_main_time_from(session, client, client_headers, car_person_data_generic_diff):
    ...
    with client:
        response = client.get(
            f"/dev/diff/data?branch=branch2&branch_only=false&time_from={time20.to_iso8601_string()}",
            headers=client_headers,
        )
...
async def test_diff_data_endpoint_with_main_time_from_to(session, client, client_headers, car_person_data_generic_diff):
    ...
    with client:
        response = client.get(
            f"/dev/diff/data?branch=branch2&branch_only=false&time_from={time0.to_iso8601_string()}&time_to={time20.to_iso8601_string()}",
            headers=client_headers,
        )
