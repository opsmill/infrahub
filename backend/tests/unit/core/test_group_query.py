...
async def test_query_GroupAddAssociationQuery(
    ...
    group1 = await Node.init(session=session, schema="Group", branch=branch)
    await group1.new(session=session, name="group-of-person")
    await group1.save(session=session)
