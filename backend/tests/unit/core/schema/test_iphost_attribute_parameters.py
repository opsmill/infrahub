...
def _dns_record_definition() -> dict[str, Any]:
    return deepcopy(DNS_RECORD_DICT)
...
class TestDefaultValuePrefixPolicy:
    """A declared attribute refuses a prefixed default outright, rather than rewriting it."""
    ...
    @pytest.mark.parametrize(
        ...
        [
            Case(
                name="declared_ipv4_host_mask_is_rejected",
                attribute_name="dns_target",
                declared_default="10.0.0.1/32",
                expected_error="10.0.0.1/32 is not a valid default value for dns_target because a prefix is not permitted",
            ),
            Case(
                ...
                expected_error=(
                    "2001:db8::1/128 is not a valid default value for v6_target because a prefix is not permitted"
                ),
            ),
            Case(
                ...
                expected_error="10.0.0.1/24 is not a valid default value for dns_target because a prefix is not permitted",
            ),
            Case(
                name="declared_ipv4_bare_is_accepted",
                attribute_name="dns_target",
                declared_default="10.0.0.1",
                expected_default="10.0.0.1",
            ),
            Case(
                name="declared_ipv6_bare_is_accepted",
                attribute_name="v6_target",
                declared_default="2001:db8::1",
                expected_default="2001:db8::1",
            ),
            Case(
                name="declared_malformed_default_is_left_to_format_validation",
                attribute_name="dns_target",
                declared_default="not-an-address",
                expected_default="not-an-address",
            ),
            Case(
            ...
            Case(
                name="undeclared_subnet_prefix_is_untouched",
                attribute_name="mgmt_ip",
                declared_default="10.0.0.0/24",
                expected_default="10.0.0.0/24",
            ),
            Case(
    ...
    def test_default_value(self, case: Case) -> None:
        ...
        if case.expected_error is not None:
            with pytest.raises(ValidationError, match=re.escape(case.expected_error)):
