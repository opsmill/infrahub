...
from infrahub.core.attribute import Integer, String, IPHost, IPNetwork
...
async def test_validate_format_ipnetwork_and_iphost(session, default_branch: Branch, criticality_schema: NodeSchema):
    ...
    IPNetwork(name="test", schema=schema, branch=default_branch, at=Timestamp(), node=None, data="192.0.2.0/255.255.255.0")
    ...
    with pytest.raises(ValidationError):
        IPHost(name="test", schema=schema, branch=default_branch, at=Timestamp(), node=None, data="2001:db8::/ffff:ff00::")
    
    with pytest.raises(ValidationError):
        IPNetwork(name="test", schema=schema, branch=default_branch, at=Timestamp(), node=None, data="192.0.2.1/255.255.255.0")

    with pytest.raises(ValidationError):
        IPNetwork(name="test", schema=schema, branch=default_branch, at=Timestamp(), node=None, data="2001:db8::/ffff:ff00::")
