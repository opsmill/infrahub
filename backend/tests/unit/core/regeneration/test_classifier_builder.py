...
def test_build_folds_a_derived_peer_into_the_classifier() -> None:
    report = _FakeAnalyzedQuery(requested_read={"TestReader": ObjectAccess(attributes={"human_friendly_id"})})
...
# The reading kind reached from a query root, used when the reading kind is itself traversed.
ROOT_HOP = RelationshipHop(
    node_kind="TestRoot",
    relationship_identifier="root__reader",
    relationship_direction=RelationshipDirection.OUTBOUND,
)
READER_ROOT_PATH = ReachedPath(hops=(ROOT_HOP,))
OWNER_PEER = PeerDependency(
    kind="TestOwner", field_name="name", path=ReachedPath(hops=(OWNER_HOP,)), reading_kind="TestReader"
)
...
FOLD_CASES = [
    ...
    FoldCase(
        name="a_peer_of_a_traversed_reading_kind_extends_the_chain_to_the_root",
        readable_fields_by_kind={"TestRoot": {"name"}, "TestReader": {"human_friendly_id"}},
        traversed_kinds={"TestReader"},
        reached_paths_by_kind={"TestReader": (READER_ROOT_PATH,)},
        dependencies=DerivedFieldDependencies(peers=(OWNER_PEER,), widen=False),
        expected=_classifier(
            traversed_kinds={"TestReader", "TestOwner"},
            readable_fields_by_kind={"TestRoot": {"name"}, "TestReader": {"human_friendly_id"}, "TestOwner": {"name"}},
            reached_paths_by_kind={
                "TestReader": (READER_ROOT_PATH,),
                "TestOwner": (ReachedPath(hops=(OWNER_HOP, ROOT_HOP)),),
            },
            depends_on_everything=False,
        ),
    ),
    FoldCase(
        name="a_peer_of_a_traversed_reading_kind_without_a_chain_widens",
        readable_fields_by_kind={"TestReader": {"human_friendly_id"}},
        traversed_kinds={"TestReader"},
        reached_paths_by_kind={},
        dependencies=DerivedFieldDependencies(peers=(OWNER_PEER,), widen=False),
