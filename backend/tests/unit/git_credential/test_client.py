...
# Both command modules read stdin at import time to build their default CLI argument; pytest captures it.
with patch("sys.stdin"):
    from infrahub.git_credential import askpass, helper
...
def test_both_git_commands_share_the_client_config() -> None:
    # git spawns each command as its own process, so a command that builds its own Config silently
    # loses the configured CA: both must go through the shared builder.
    assert helper.build_client_config is build_client_config
    assert askpass.build_client_config is build_client_config
