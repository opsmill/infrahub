...
def test_askpass_username(mock_core_schema_01, mock_repositories_query):
    input_data = "Username for 'https://github.com/opsmill/infrahub-demo-edge.git'"

    result = runner.invoke(app=app, args=[input_data])
    assert not result.stderr
    assert result.stdout == "myusername\n"
    assert result.exit_code == 0


def test_askpass_password(mock_core_schema_01, mock_repositories_query):
