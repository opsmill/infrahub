...
async def test_updating_relationship_when_peer_side_is_required(
    ...
    animals = await updated_person1.animals.get(db=db)
    assert {a.peer_id for a in animals} == {dog1.id, dog2.id}
    updated_person2 = updated_nodes[person2.id]
    animals = await updated_person2.animals.get(db=db)
    assert len(animals) == 0
    updated_dog1 = updated_nodes[dog1.id]
    owner = await updated_dog1.owner.get(db=db)
    assert owner.peer_id == person1.id
    updated_dog2 = updated_nodes[dog2.id]
    owner = await updated_dog2.owner.get(db=db)
    assert owner.peer_id == person1.id
