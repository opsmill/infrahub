...
def test_render_cli_snippet_single_schema() -> None:
    items = [MarketplaceInstallItem(kind="schema", namespace="infrahub", name="vlan-translation", semver="1.0.0")]
    snippet = render_cli_snippet(items=items, branch_name="main")
    assert snippet.downloads[0].command == ("infrahubctl marketplace download infrahub/vlan-translation -v 1.0.0")
...
def test_render_cli_snippet_collection_uses_c_flag() -> None:
    items = [MarketplaceInstallItem(kind="collection", namespace="infrahub", name="base-schemas", semver=None)]
    snippet = render_cli_snippet(items=items, branch_name="main")
    assert snippet.downloads[0].command == ("infrahubctl marketplace download -c infrahub/base-schemas")
...
def test_render_cli_snippet_rejects_too_many() -> None:
    items = [MarketplaceInstallItem(kind="schema", namespace="n", name=f"s{i}", semver=None) for i in range(51)]
    with pytest.raises(ValueError, match="items must not exceed"):
