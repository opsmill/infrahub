...
def test_event_on_the_budget_survives_the_prefect_run_context_append(
    ...
    Prefect's events worker extends the related list in place, without re-running the client-side
    validation, so an event emitted on the maximum rather than under it reaches the API above the
    maximum. The cases span both sides of the reservation, so the floor is covered as well as the
    proportional part.
    """
