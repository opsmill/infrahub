...
""",
    ...
    for file_to_add in files_to_add:
        file_path = git_upstream_repo_02["path"] / file_to_add["name"]
...
@pytest.fixture
async def git_repo_jinja_w_client(git_repo_jinja: InfrahubRepository, client: InfrahubClient) -> InfrahubRepository:
...
@pytest.fixture
async def git_repo_transforms_w_client(
    git_repo_transforms: InfrahubRepository, client: InfrahubClient
) -> InfrahubRepository:
...
def git_allow_explicit_merge_commit_config():
    initial_allow_explicit_merge_commit = config.SETTINGS.git.allow_explicit_merge_commit
    config.SETTINGS.git.allow_explicit_merge_commit = True
    yield
    config.SETTINGS.git.allow_explicit_merge_commit = initial_allow_explicit_merge_commit
