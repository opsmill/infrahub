...
async def test_both_schema_routes_survive_merging_with_the_data_diff(
    ...
    # The schema edits are themselves data, so the data diff adds `unique` checks on the SchemaAttribute
    # nodes that were edited, alongside the ones for the two edited instance values.
    schema_attribute_unique = {
        _attribute_constraint(
            kind="SchemaAttribute",
            field_name=field_name,
            property_name="unique",
            constraint_name="attribute.unique.update",
        )
        for field_name in ("kind", "parameters", "regex")
    }
    data_unique = {
        _attribute_constraint(
            kind=CAR_KIND, field_name="color", property_name="unique", constraint_name="attribute.unique.update"
        ),
        _attribute_constraint(
            kind=PERSON_KIND, field_name="height", property_name="unique", constraint_name="attribute.unique.update"
        ),
    }

    assert set(merged) == {regex, kind, hierarchical} | schema_attribute_unique | data_unique
    assert {c.node_uuids for c in merged} == {None}
