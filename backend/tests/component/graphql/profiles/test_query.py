...
@pytest.fixture
def criticality_schema(default_branch: Branch, data_schema: None, node_group_schema: None) -> None:
    GENERIC_SCHEMA = {
...
async def test_create_profile_in_schema(
...
async def test_upsert_profile_in_schema(
    db: InfrahubDatabase, default_branch: Branch, criticality_schema: NodeSchema
) -> None:
    profile = registry.schema.get("ProfileTestCriticality", branch=default_branch)
...
async def test_profile_apply(db: InfrahubDatabase, default_branch: Branch, criticality_schema: NodeSchema) -> None:
...
async def test_profile_apply_generic(
    db: InfrahubDatabase, default_branch: Branch, criticality_schema: NodeSchema
) -> None:
...
async def test_is_from_profile_set_correctly(
    db: InfrahubDatabase, default_branch: Branch, criticality_schema: NodeSchema
) -> None:
    profile_schema = registry.schema.get("ProfileTestCriticality", branch=default_branch)
    prof_1 = await Node.init(db=db, schema=profile_schema)
