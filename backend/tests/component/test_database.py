...
#     await validate_database(driver=db._driver, database_name=config.SETTINGS.database.database_name, retry=2)

#     with pytest.raises(ClientError):
#         await validate_database(driver=db._driver, database_name=dbs_for_test[1])


async def test_database_transaction(empty_database: None, db: InfrahubDatabase) -> None:
