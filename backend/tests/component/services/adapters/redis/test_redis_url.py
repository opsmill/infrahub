...
async def test_cache_set_get_via_url(redis_url: dict[int, int] | None) -> None:
    if config.SETTINGS.cache.driver != config.CacheDriver.Redis or not redis_url:
        pytest.skip("Must use Redis with a connection URL to run this test")
...
async def test_lock_acquire_release_via_url(redis_url: dict[int, int] | None) -> None:
    if config.SETTINGS.cache.driver != config.CacheDriver.Redis or not redis_url:
        pytest.skip("Must use Redis with a connection URL to run this test")
