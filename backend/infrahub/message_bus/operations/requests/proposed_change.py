...
async def refresh_artifacts(message: messages.RequestProposedChangeRefreshArtifacts, service: InfrahubServices) -> None:
    ...
    modified_kinds_in_source = modified_kinds(diff_summary=diff_summary, branch=message.source_branch)
    ...
    for artifact_definition in artifact_definitions:
        ...
        for changed_model in modified_kinds_in_source:
            condition = False
...
async def _populate_subscribers(
    ...
    result = await service.client.execute_graphql(
        ...
        branch_name=branch,
        variables={"members": modified_nodes(diff_summary=diff_summary, branch=branch)},
    )
