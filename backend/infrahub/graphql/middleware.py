...
ALLOWED_MUTATIONS_ON_NEED_REBASE_BRANCH = ["BranchRebase", "BranchDelete"]


def raise_on_mutation_on_branch_needing_rebase(next, root, info, **kwargs):  # type: ignore  # noqa
    if info.context.branch.status == BranchStatus.NEED_REBASE.value and info.operation.operation.value == "mutation":
