...
def generate_graphql_mutations(
    ...
    upsert = generate_graphql_mutation_create(
        branch=branch, schema=schema, base_class=base_class, mutation_type="Upsert"
    )
...
def generate_graphql_mutation_create_input(
    schema: NodeSchema, input_type_name: str = "Create"
    ...
    return type(f"{schema.kind}{input_type_name}Input", (graphene.InputObjectType,), attrs)
...
def generate_graphql_mutation_create(
    ...
    input_type = generate_graphql_mutation_create_input(schema=schema, input_type_name=mutation_type)
