...
async def generate_mutation_mixin(db: InfrahubDatabase, branch: Union[Branch, str] = None) -> Type[object]:
    ...
    for node_schema in full_schema.values():
        ...
        create, update, upsert, delete = generate_graphql_mutations(
            branch=branch, schema=node_schema, base_class=base_class
        )

        class_attrs[f"{node_schema.kind}Create"] = create.Field()
        class_attrs[f"{node_schema.kind}Update"] = update.Field()
        class_attrs[f"{node_schema.kind}Upsert"] = upsert.Field()
...
def generate_graphql_mutations(
...
) -> Tuple[Type[InfrahubMutation], Type[InfrahubMutation], Type[InfrahubMutation], Type[InfrahubMutation]]:
    ...
    upsert = generate_graphql_mutation_upsert(branch=branch, schema=schema, base_class=base_class)
...
def generate_graphql_mutation_upsert_input(schema: NodeSchema) -> Type[graphene.InputObjectType]:
    """Generate an InputObjectType Object from a Infrahub NodeSchema
...
def generate_graphql_mutation_upsert(
    schema: NodeSchema,
    branch: Branch,
    base_class: type[InfrahubMutation] = InfrahubMutation,
) -> Type[InfrahubMutation]:
    """Generate a GraphQL Mutation to UPSERT an object based on the specified NodeSchema."""
    name = f"{schema.kind}Upsert"

    object_type = generate_graphql_object(schema=schema, branch=branch)
    input_type = generate_graphql_mutation_upsert_input(schema=schema)
