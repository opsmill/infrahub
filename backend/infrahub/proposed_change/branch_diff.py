...
def has_data_changes(diff_summary: list[NodeDiff], branch: str) -> bool:
    ...
    return bool([entry for entry in diff_summary if entry["branch"] == branch])
...
def has_node_changes(diff_summary: list[NodeDiff], branch: str) -> bool:
    ...
    return bool(
        [entry for entry in diff_summary if entry["branch"] == branch and not SCHEMA_CHANGE.match(entry["kind"])]
    )


def modified_kinds(diff_summary: list[NodeDiff], branch: str) -> list[str]:
...
def modified_nodes(diff_summary: list[NodeDiff], branch: str) -> list[str]:
