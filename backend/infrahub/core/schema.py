...
class AttributeSchema(HashableModel):
    ...
    inherited: bool = Field(
        default=False,
        json_schema_extra={"update": UpdateSupport.NOT_APPLICABLE.value},
...
ENUM_REF_MAP = {"#/$defs/BranchSupportType": BranchSupportType}


class AttributeSchemaGenerator:
    def __init__(self, model: type[BaseModel]) -> None:
        self.schema = model.model_json_schema()
        self._attributes = self._process_attributes()

    def _process_attributes(self) -> list[dict[str, Any]]:
        attributes = []
        for property, data in self.schema["properties"].items():
            ...
            attribute = {
                ...
                "optional": property not in self.schema["required"],
                "regex": data.get("pattern"),
                "max_length": data.get("maxLength"),
                "min_length": data.get("minLength"),
            }
            if property == "kind":
                attribute["enum"] = ATTRIBUTE_KIND_LABELS

            if data.get("default") is not None:
                attribute["default_value"] = data["default"]

            self._process_kind(attribute=attribute, data=data)

            attributes.append(attribute)
        return attributes

    def _process_anyof(self, attribute: dict[str, Any], entry: dict[str, Any]) -> None:
        if entry.get("type") and entry["type"] in TYPE_MAP:
            attribute["kind"] = TYPE_MAP[entry["type"]]
            if "minLength" in entry:
                attribute["min_length"] = entry["minLength"]
            if "maxLength" in entry:
                attribute["max_length"] = entry["maxLength"]
        elif entry.get("type") and entry["type"] == "array":
            if entry.get("items") == {}:
                attribute["kind"] = "List"
            elif entry.get("items") and entry["items"].get("$ref") == "#/$defs/DropdownChoice":
                attribute["kind"] = "List"
        elif entry.get("$ref"):
            if entry["$ref"] in ENUM_REF_MAP:
                attribute["kind"] = "Text"
                attribute["enum"] = ENUM_REF_MAP[entry["$ref"]].available_types()
            elif entry["$ref"] == "#/$defs/DropdownChoice":
                attribute["kind"] = "List"
        elif entry == {}:
            attribute["kind"] = "Any"

    def _process_kind(self, attribute: dict[str, Any], data: dict[str, Any]) -> None:
        if data.get("type"):
            attribute["kind"] = TYPE_MAP[data["type"]]
        if data.get("anyOf"):
            for entry in data["anyOf"]:
                self._process_anyof(attribute=attribute, entry=entry)
...
internal_schema_definition: dict[str, Any] = {
    "version": None,
    "nodes": [
        {
            "name": "Node",
...
def create_internal_schema() -> dict[str, Any]:
    generated_attribute_schema = AttributeSchemaGenerator(model=AttributeSchema)
    attribute_schema: dict[str, Any] = {
        ...
        "default_filter": None,
        "display_labels": ["name__value"],
        "attributes": generated_attribute_schema.attributes,
        "relationships": [
            {
                "name": "node",
                "peer": "SchemaNode",
    ...
    internal_schema_definition["nodes"].append(attribute_schema)
    return internal_schema_definition


internal_schema = create_internal_schema()
