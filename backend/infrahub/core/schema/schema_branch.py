        ...
        for name in self.all_names:
            ...
            for attribute in node.attributes:
                if keyword.iskeyword(attribute.name):
                    raise ValueError(f"Python keyword '{attribute.name}' cannot be used as an attribute name on '{node.kind}'")

            # Check for Python keywords in relationship names
            for relationship in node.relationships:
                if keyword.iskeyword(relationship.name):
                    raise ValueError(f"Python keyword '{relationship.name}' cannot be used as a relationship name on '{node.kind}'")
