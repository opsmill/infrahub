...
from infrahub.core.constants import NULL_VALUE, AttributeDBNodeType, RelationshipStatus
...
class AttributeCreateQuery(AttributeQuery):
    """Create the database row for a single attribute on an already-persisted node.

    Materialises the Attribute node together with its default value and protected flag, matching the
    node by uuid. This backs the create half of the attribute save path for attributes that exist in the
    schema but were never written for a node, such as an attribute inherited through schema evolution.
    """
    ...
    async def query_init(self, db: InfrahubDatabase, **kwargs: dict[str, Any]) -> None:  # noqa: ARG002
        ...
        branch_filter, branch_params = self.branch.get_query_filter_path(at=at.to_string())
        self.params.update(branch_params)

        self.params["node_uuid"] = self.attr.node.id
        self.params["attr_name"] = self.attr.name
        self.params["branch_support"] = self.attr.schema.get_branch().value
        ...
        if not is_large_attribute_type(self.attr.get_kind()):
            # should be indexed
        ...
        %(uuid_generation)s
        // Set metadata on the Attribute and Node vertices if on the default/global branch
        WITH a, n, has_attr_e
        CALL (a, n) {
            WITH a, n
            WHERE $set_metadata
