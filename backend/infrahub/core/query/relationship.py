...
class RelationshipQuery(Query):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.rel = rel
        self.rel_id = rel_id
        self.schema = schema or self.rel.schema  # type: ignore[misc]  # rel is an instance here, guarded above
...
class RelationshipUpdatePropertyQuery(RelationshipQuery):
    ...
    async def query_init(self, db: InfrahubDatabase, **kwargs: Any) -> None:  # noqa: ARG002
        self.params["rel_node_id"] = self.rel_id or (self.rel.id if self.rel else None)  # type: ignore[misc]  # rel narrowed to an instance
        self.params["branch"] = self.branch.name
        self.params["branch_level"] = self.branch.hierarchy_level
        self.params["user_id"] = self.user_id
        self.params["at"] = self.at.to_string()
...
class RelationshipDeleteQuery(RelationshipQuery):
    ...
    async def query_init(self, db: InfrahubDatabase, **kwargs: Any) -> None:  # noqa: ARG002
        ...
        self.params["rel_id"] = self.rel_id or self.rel.id  # type: ignore[misc]  # rel narrowed to an instance
        self.params["branch"] = self.branch.name
        self.params["rel_prop"] = self.get_relationship_properties_dict(
