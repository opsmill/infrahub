...
class CoreNumberPool(Node):
    ...
    async def get_next(self, db: InfrahubDatabase, branch: Branch, attribute: BaseAttribute) -> int:
        ...
        if not isinstance(parameters, NumberAttributeParameters):
            raise ValueError(
                "Attribute parameters are not a NumberAttributeParameters while a number pool is attached to this attribute"
            )
...
def find_next_free(start: int, end: int, taken: list[int | None], parameters: NumberAttributeParameters) -> int | None:
    used_numbers = [number for number in taken if number is not None]
    used_set = set(used_numbers)

    for num in range(start, end + 1):
        if num not in used_set and parameters.is_valid_value(num):
            return num
