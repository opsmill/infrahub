...
class BranchMergeOrchestrator:
    """Drive a full branch merge: lock, pre-checks, graph merge, migrations, status transitions, follow-ups."""
    ...
    async def merge(self, *, context: InfrahubContext, proposed_change_id: str | None = None) -> None:
        ...
        async with self.merge_locker.acquire_global_lock():
            refreshed = await Branch.get_by_name(db=self.db, name=self.source_branch.name)
            if refreshed.status != BranchStatus.OPEN:
                self.log.info(
                    f"Branch '{self.source_branch.name}' is not open (status={refreshed.status}), skipping merge"
                )
                return
