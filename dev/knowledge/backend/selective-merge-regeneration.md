...
The dependency closure and `fingerprint` are computed at repository import. The closure is trusted (`dependencies_complete = true`) or not; see [code-generation.md](code-generation.md) and the [proposed changes overview](../../../docs/docs/proposed-changes/overview.mdx) for how a closure is built (Python: the entry file alone, since imports are not analyzed; Jinja2: the static include/import/extends graph) and how `watch.files` restores a trusted closure. Note that a Python closure is `complete = true` in every case except an unusable `file_path`, which the aggregator isolates: naming the entry file reads nothing off disk, so there is no enumeration left to fail. The untrusted-closure fallback is therefore effectively dead for Python definitions, and the commit-id fold in the fingerprint is the only safety net a Python source has until it declares `watch`.
