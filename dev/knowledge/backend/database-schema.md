...
The corollary for subclasses: the decorator is applied to a specific method, so overriding a decorated method discards it. A subclass that overrides a transaction-owning method reapplies the decorator when it opens its own transaction, and also when it does work the base's scope does not reach - a read taken before it delegates to `super()` is outside the retry the base sets up. Reapplying it is safe either way: only the outermost scope replays, so a subclass scope wrapping a base scope stays one budget rather than multiplying attempts.
