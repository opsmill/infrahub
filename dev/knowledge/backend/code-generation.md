...
Schema definitions (Python)
  ...
  uv run invoke schema.generate-graphqlschema  ← offline, no instance needed
       ...
       │
       ▼
  cd frontend/app && pnpm run codegen     ← offline, reads local files
       │
...
| Changed GraphQL schema structure | `uv run invoke schema.generate-graphqlschema` + `pnpm run codegen` |
| New GraphQL queries/mutations in frontend | `pnpm run codegen` |
