...
One setting binds the two halves inside one process, not across a fleet. The filter is stored in the Prefect automation by whichever worker rebuilt the triggers, while the resolver reads the setting in the worker that runs the pass. So the value has to be the same on every worker before the reconcile runs. A fleet part-way through a flip can store the filter while the worker running the pass derives nothing, and the stored automations then flip on each reconcile until every worker agrees.
