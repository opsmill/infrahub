...
- **A query that is not pinned to a single object gets no read set at all.** Readers are resolved through query-group membership, which records what the last run read and never what the next one would. When any number of objects can answer the query, a node can enter or leave the result set while nothing changes on the nodes already in it, so a created node is invisible to the existing subscribers and a deleted one leaves no membership behind. Updates would be sound either way; create and delete churn is the hole, so the whole attribute widens. The artifact and generator selector guards the same case by regenerating every target. This restriction belongs to reader resolution alone: the schema-change backfill maps the same queries without it, because it asks which attributes a changed element feeds and then refreshes whole kinds.
...
The doubling is not compute only. Every per-node read of a transform query passes `update_group=True`, and the API submits one `update_graphql_query_group` flow per request, which upserts the query group and adds the subscriber. So a merge touching N nodes of a Python-attribute kind runs about 2N group upserts instead of N, and no value compare short-circuits the second one.
