...
While the switch is on, the two Python per-node automations match `live` only, so a merge, a rebase and a coalesced write start no per-node flow. That is also what removes the two echo loops: the coalesced Jinja2, display-label and HFID writes carry the `recompute` origin and no longer re-fire the Python automations, and a merge that changes no schema starts no `process_transform` at all, so none of its writes carry `live` back into the paths the merge suppressed. A schema-changing merge is different: the schema-scoped backfill submits its whole-kind refresh without the coalesced flag, so those writes are stamped `live` and do re-enter those paths.
