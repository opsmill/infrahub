...
`INFRAHUB_COALESCE_PYTHON_RECOMPUTE_AFTER_MERGE` governs the family, on by default, and it governs **both halves at once**: the real resolver against an inert one, and whether the two Python per-node automations carry the `live` origin filter. Keep the two halves on the same switch: with the filter applied and the resolver inert, nothing would recompute a replayed change.

The two halves do not take effect at the same moment, which matters when the setting is flipped. The resolver reads it per pass, so a restart is enough. The origin filter is baked into the stored Prefect automation when the trigger is built, and only `computed_attribute_setup_python` (a schema change or a branch deletion), `process_transform_lifecycle` (a transform edit) and `trigger_configure_all` rebuild those two trigger types — `setup_task_manager_identifiers` covers the display-label and HFID types only, and runs on a first-time database at that. So a flip needs a reconcile of the Python trigger types to land, which `infrahub upgrade` performs through `trigger_configure_all`. Until it runs, the automations keep the filter of the previous position: after switching off, a merge and a rebase recompute nothing until that reconcile.
...
- **A query that is not pinned to a single object resolves no readers.** It keeps its read set, so a change to a kind or a field it never reads still selects nothing, and the schema-scoped backfill scopes a schema change on exactly the same read set. What an unpinned root costs is the reader lookup: readers come from query-group membership, which records what the last run read and never what the next one would, so when any number of objects can answer the query a node enters or leaves the result set while nothing changes on the members already in it. A created node is invisible to the existing subscribers and a deleted one leaves no membership behind. So a change that does reach the attribute widens it to the whole kind instead of resolving nodes, creations included. The artifact and generator selector guards the same case by regenerating every target.
