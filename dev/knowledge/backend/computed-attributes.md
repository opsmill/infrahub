...
Besides the transform-lifecycle triggers, two families of data-path automations recompute the value when a node feeding the transform's query changes. The **owner automations** are keyed on `(branch, kind, attribute)` and match the attribute's own kind. The **query automations** are keyed on `(branch, transform, read kind)` and match the kinds the transform's query reads. What each action carries is the reason for that asymmetry: the owner automation names its attribute in the parameters it submits, so it needs one definition per attribute, while the query automation names none and the flow behind it resolves the attributes itself. Keying the second family on the attribute would build a copy per attribute that runs the same flow once per attribute for one change, each run repeating the whole resolution and fan-out. Within one branch the attributes a transform feeds share its query, so they read the same kinds and the same fields of them; a branch pinned to its own repository commit, or carrying its own schema, resolves that query itself and owns its definitions, which the default-branch ones then exclude. While the coalesced pass owns merge and rebase, both trigger types match `origin=live` only. `_reconcile_python_computed_attribute_automations` rebuilds these from the schema. One gather builds both trigger lists and they are applied under a single trigger-registry lock, so a concurrent reconcile cannot delete an automation another run just created, and a transform delete prunes its automation rather than leaving it stale.
