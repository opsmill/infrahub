...
The same shape applies to a single non-critical side effect inline in a request handler — emitting a telemetry or notification event before returning a response, for example — not only to per-item batch follow-ups. First confirm the call can actually fail in a way relevant to the caller (see [Exception Handling](../../guidelines/backend/python.md#exception-handling)); if it must be isolated, prefer dispatching it as a background task that runs after the response over an inline `try`/`except` in the handler.
