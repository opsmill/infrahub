...
Each class carries a clean message and a remediation hint. `webhook_send` classifies the failure, logs the outcome, and raises a `WebhookDeliveryError`; `webhook_process` settles a classified failure into a failed run state whose message is the classified reason. `WebhookDeliveryError` carries a `suppress_traceback` marker, and `SuppressMarkedTracebackFilter` (registered on the `prefect.flow_runs` and `prefect.task_runs` loggers in `backend/infrahub/log.py`) drops the traceback record Prefect's engine would otherwise log for the raised error, so the run logs carry only the classified reason. An `UNKNOWN` error is re-raised without the marker, so it keeps its traceback and surfaces as a genuine crash.
