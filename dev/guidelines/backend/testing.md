...
- **Never assert mere non-emptiness.** `assert result != frozenset()` or `assert len(result) > 0` passes as long as *anything* comes back. Assert the specific expected value instead.
