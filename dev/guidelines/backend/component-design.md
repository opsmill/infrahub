...
> Part of: `dev/guidelines/backend/` | Related: [Testing Standards](testing.md)
...
Stepping back from the individual rules above: constructor-injected long-lived dependencies plus method-passed transient entities is the boundary that lets components be reused across requests/operations and mocked with adapter implementations instead of `unittest.mock`. The [testing rule](testing.md) requires adapter/protocol patterns for tests — that requirement is only practical when production code follows this design.
