...
The `/bug-analyze` command has already identified the root cause. Your job is to write ONE test
...
If the test cannot be made to fail for the right reason after 3 attempts, inform the
developer explaining what was tried. Do NOT open a PR or include the
