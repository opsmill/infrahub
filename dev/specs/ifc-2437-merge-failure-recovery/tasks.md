...
- [X] T010 [US2] **[ASK-FIRST]** Add `MERGE_FAILED = "MERGE_FAILED"` to `BranchStatus` in `backend/infrahub/core/branch/enums.py` (not in `TERMINAL_BRANCH_STATUSES`). Auto-exposed to GraphQL via `Enum.from_enum` → changes `schema/schema.graphql` (regen in T046); confirm the additive enum value is acceptable.
...
- [X] T012 [US2] Add a configurable merge-failure **grace period** setting in `backend/infrahub/config.py` (pick a concrete default ~2–3 min) — added here, where detection first consumes it.
- [X] T013 [US2] Add a read-side helper in `backend/infrahub/core/merge/merge_locker.py` returning the current `all_branches` lock holder's `worker_id` (from the `timestamp::worker_id` token) or `None` if absent — without acquiring.
- [X] T014 [US2] Add the pure predicate `is_failed_merge(status, lock_token, active_worker_ids, merge_started_at, now, grace_period) -> bool` in `backend/infrahub/core/merge/failure_recovery.py` = `status==MERGING AND lock present AND token worker_id ∉ active set AND now-merge_started_at > grace_period` (research R2).
- [X] T015 [P] [US2] Unit test the predicate in `backend/tests/unit/core/merge/test_failed_merge_predicate.py` (lock present/absent, worker active/inactive, within/after grace).
...
- [X] T023 [US2] Integration-docker test (detection half) in `backend/tests/integration_docker/test_merge_kill_recovery.py`: SIGKILL mid-merge, idle, assert `MERGE_FAILED` within grace + one scan interval (SC-003, SC-004).
- [X] T024 [US2] Add a changelog fragment for **failed-merge detection & protection** under `changelog/`.
