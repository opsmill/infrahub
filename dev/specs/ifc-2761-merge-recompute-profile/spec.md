...
- The counting layer measures **data-only** merges. A schema-changing merge runs migrations through the workflow executor, which are recorded-not-run when workflows are neutralized for counting, so schema-migration cost is attributed only in the timing layer (on the full stack).
