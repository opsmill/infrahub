...
- [ ] T031 [US2] ⚠ Add the flag to `development/docker-compose.yml` **only**. The root `docker-compose.yml` does **not** get it, unlike its two experimental siblings there — decided, not an oversight. Consequence to accept: a deployment brought up from the root compose file cannot enable dark by setting the env var on the host, because there is no passthrough line for it. That is the intent while dark is alpha.
