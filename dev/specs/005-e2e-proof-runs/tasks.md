...
- [ ] T007 [US1] Write `.github/workflows/bug-agent-e2e-proof.yml` skeleton per the trigger contract: `pull_request [opened, synchronize, reopened]` on base `stable` with `paths: tests/e2e/**`; job `if: startsWith(head.ref, 'ai-bug-pipeline-')`; `concurrency: bug-e2e-proof-<pr>` + cancel-in-progress; permissions `contents: write`, `pull-requests: write`; timeout 45m; runs-on ubuntu-latest
- [ ] T008 [US1] Add phase/target detection step (adapt from the PoC workflow): phase from `AGENT_FIX_COMPLETE` in the PR body (via env, never shell-interpolated); target test from `git diff --diff-filter=AM base...HEAD -- 'tests/e2e/**/test_*.py'` excluding `tutorial/`; hard-fail with explicit message unless exactly one file
