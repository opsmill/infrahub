...
- Demotion behavior: after a demote push the workflow does not trigger (the PR's net diff no longer matches `paths: tests/e2e/**`); the pre-demotion GREEN verdict and body evidence persist. The job must never be a required status check.
