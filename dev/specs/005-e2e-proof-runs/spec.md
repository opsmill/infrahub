...
- Demotion push (the e2e reproduction is removed after a verified GREEN): the proof workflow does not run at all — the reproduction was added and removed within the PR, so the PR's net diff no longer touches the e2e suite and the trigger's path filter no longer matches. The GREEN verdict and the embedded before/after evidence from the pre-demotion run persist in the PR description, and the demoted cheaper-tier test is validated by the normal test jobs. Corollary: the proof job MUST NOT be configured as a required status check, or demotion would leave the final head without it.
