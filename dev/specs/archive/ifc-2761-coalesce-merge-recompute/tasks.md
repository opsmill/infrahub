...
- [X] T011 [P] [US2] integration_docker correctness test `backend/tests/integration_docker/test_merge_recompute_coalescing.py`: after a real merge, a destination-only reader (created on main after the fork, so the source branch never recomputed it) has its computed attribute and display label refreshed to the merged peer value, while its human-friendly id (local-only) is unchanged. This is the decisive case only the coalesced post-merge recompute can satisfy, and it exercises the full chain (suppression + submission + worker). Passing on the full stack (`local` image). Creation/transitive cases can be added later; the destination-only reader is the strongest correctness gate.
...
- [X] T013 [US1] Ran the profiling harness (`test_merge_recompute_timing.py`, enhanced to count process-flow dispatches) on the full stack at scale 100. Merge dispatched **2** recompute process flows (one computed, one display) for 100 changed nodes, versus ~202 with the per-node fan-out still firing (the pre-fix run, which approximates the develop baseline of about 2 per changed node). The per-reader writes (200) are unchanged, as expected. This proves the structural bound (SC-001): process-flow dispatch is bounded by the affected derived values, not the changed-node count. Merged data identical (T011).
...
- [X] T016 [US3] Ran the harness for rebase (a variant added to the timing test). Rebase dispatched **2** process flows for 100 changed nodes on the user branch, the same bound as merge, with correctness parity (T015). Reduction and correctness confirmed for both operations.
...
- [X] T017 [US4] No-regression for small graphs: the process-flow dispatch is bounded by the affected derived values (2 for this schema) independent of the changed-node count, so a small merge submits the same tiny coalesced set and the build is O(change signatures). Small merges are not slowed. (Confirmed structurally by the scale-100 run showing 2 process flows; a dedicated small-scale benchmark was not run separately.)
