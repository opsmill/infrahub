...
| Allowed values | `high`, `medium`, `low` (case-insensitive, surrounding whitespace trimmed) |
| Default when absent | `medium` |
| Default when empty/invalid | `medium` (FR-006) |
...
- `X-Priority: medium` → class `medium`.
- `X-Priority: low` → class `low`.
- Header absent, empty, whitespace-only, or any other value → class `medium`, and the request is counted as "no/invalid priority" for adoption tracking (FR-OBS-7).

**Excluded paths** (admission is bypassed entirely — never classified, never shed): `/health`, `/metrics`, the config-probe endpoint `/api/config`, and the static/docs set `/assets`, `/favicons`, `/docs`, `/api/schema`. Non-HTTP scopes (WebSocket, lifespan) are never affected.
...
**Shed reasons** (internal, surfaced only via metrics — not in the response body): `codel` (adaptive shed from sustained sojourn), `backstop` (per-class waiter cap exceeded), or `stress` (database-stress signal sheds a fraction of the class). See `metrics.md` (FR-OBS-2).

## Behavioural guarantees (acceptance-testable)

| ID | Guarantee |
|----|-----------|
| C-1 | A request with a valid `X-Priority` is classified into the matching class; missing/invalid → `medium`. (FR-006) |
...
| C-4 | Under sustained overload, `high` is shed last (≈0%), `low` first, `medium` second. (FR-005, SC-002) |
...
- **Backward compatible**: existing clients that never send `X-Priority` are treated as `medium` and see no change under normal load. The layer ships inert (SC-006).
