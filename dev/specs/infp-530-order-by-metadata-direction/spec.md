...
A schema designer maintains a `Notes` schema that is attached to many parent objects through a many-cardinality relationship. They want every list view of `Notes` — whether on the parent's detail page, in the API, or in any UI tab — to default to "newest first" without each query having to specify the order. Today they must specify the metadata-based ordering at query time on every read; the UI does not, so the notes appear in an unhelpful order.

**Why this priority**: This is the customer's real, blocking pain. Users cannot tell at a glance which note is most recent, which makes the feature unusable for its intended purpose. The fix lives entirely at the schema-definition level, so it's a single edit that propagates to every consumer.

**Independent Test**: Define a `Notes` schema whose `order_by` declares newest-first ordering by creation timestamp. Attach two notes to a parent object at different times. Open the parent's detail page; verify the most-recent note appears first. Verify the same ordering holds when fetching the parent's notes through the API without any per-query ordering argument.

**Acceptance Scenarios**:

1. **Given** a schema declares `order_by` with newest-first creation-time ordering, **When** the API or UI lists items of that schema (top-level or as a relationship target), **Then** the most recently created item appears first.
2. **Given** a schema declares newest-first ordering by update time, **When** an existing item is modified, **Then** that item moves to the top of subsequent listings.
...
- **FR-001**: The schema definition MUST accept entries in `order_by` that reference node-level creation and update timestamps, using a reserved syntax that distinguishes metadata fields from attribute names.
- **FR-002**: The schema definition MUST accept an optional ascending or descending direction marker on any `order_by` entry, applicable to both metadata and regular-attribute entries.
...
- **FR-010**: The set of metadata fields supported in schema `order_by` MUST be the same as the set supported by the existing query-time ordering input (creation timestamp and update timestamp).
...
- **Node metadata reference**: A reserved-namespace path that names one of the supported metadata fields (creation timestamp, update timestamp). Distinct from attribute paths and uses a fixed reserved prefix to avoid name collisions.
...
- **SC-002**: For the customer's primary use case (notes attached to parent objects via a many-cardinality relationship), the parent's notes list renders in newest-first order through every consumer (UI, API, SDK) once the schema is updated, with zero per-query ordering arguments.
...
- Only the creation timestamp and update timestamp are in scope for metadata-based ordering. Ordering by user-reference metadata (creator, last modifier) is explicitly out of scope: the data is a UUID reference whose ordering is not human-meaningful, and the customer did not request it.
