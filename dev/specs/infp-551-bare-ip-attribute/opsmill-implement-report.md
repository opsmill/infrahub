...
> **On the commit references below.** They describe the original linear history on
> `bare-ip-attribute-infp-551`, which was later restructured into a feature branch plus a stack of
> pull requests — see [pr-split-plan.md](./pr-split-plan.md). The SHAs cited here do not exist on the
> restructured branches. They are kept because the chunk-by-chunk provenance, and the evidence tied to
> each step, is the point of this report.
