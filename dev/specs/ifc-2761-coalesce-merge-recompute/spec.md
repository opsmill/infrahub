...
- **FR-013** (deferred to IFC-2852): Nodes that read a node deleted by the merge should have their derived values refreshed so they no longer reflect the deleted node. The coalesced pass derives these targets, but the end-to-end refresh is a known gap: the reverse-relationship reader query finds no readers once the deleted peer's edges close. A strict xfail guards the behavior until IFC-2852 fixes it.
