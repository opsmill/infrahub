...
- [ ] T004 [P] [US1] Create `backend/tests/integration/git/test_auth_and_access.py` covering wrong-credentials clone (typed credential error, message preserved) and no-write-access user push (typed access-denied error, remote response preserved) — Story 1 family 1, spec acceptance 1-2, FR-001
- [ ] T005 [P] [US1] Create `backend/tests/integration/git/test_push_failures.py` covering non-fast-forward rejection and protected-branch rejection — Story 1 family 2, spec acceptance 3-4
