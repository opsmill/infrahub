...
This feature adds graph migration `m078`, bumps `GRAPH_VERSION` 77 → 78 (corrected by T058, then again by T062, from the
...
- [X] R06 **Slice 5 — repair migration.** Blocked on T001. Supersedes Phase 4; C4 in
  ...
  **Done 2026-08-31.** T001 signed off 2026-08-25; shipped as the
  `m078_retire_agnostic_property_edges/` package (`migration.py`, `queries.py`) with
  `Migration078`, `minimum_version = 77` and `GRAPH_VERSION` 77 → 78 — the numbering corrected from
  ...
  `backend/tests/component/core/migrations/graph/m078_retire_agnostic_property_edges/`. Registration
...
- **Every test path named below is stale.** The suites were consolidated after the fact and the task
  ...
  `backend/tests/component/core/migrations/graph/m078_retire_agnostic_property_edges/` — `test_repair.py`
...
- [X] T044 [US2] Implement `Migration076` in `backend/infrahub/core/migrations/graph/m076_retire_agnostic_property_edges.py` as an `ArbitraryMigration` with `minimum_version: int = 75`, modelled on `m075_finish_deleting_branches.py`: read branches with `Branch.get_list(db=db)` (the registry may be unpopulated in an upgrade process), run the query's unbounded form **with the widened anchor** (T011), batch at the existing `MAX_AGNOSTIC_PEER_BATCH_SIZE` (500) cap, report **both** counts via `get_migration_console()`, and return `MigrationResult(errors=[...])` without raising (FR-016) Delivered as `Migration078` in the package `backend/infrahub/core/migrations/graph/m078_retire_agnostic_property_edges/` (`__init__.py`, `migration.py`, `queries.py`), `minimum_version = 77`, per T058 and T062. Four deviations from the text above, all deliberate:
  ...
  4. **The removed-vertex count comes from the query stats** (`nodes_deleted`, summed with a `or 0` guard because Bolt omits zero counters), since `DETACH DELETE` has nothing to return. The closed-edge count was originally taken the same way, from `properties_set`, on the belief that `CALL … IN TRANSACTIONS` must be a unit subquery; that restriction was lifted after Neo4j 4.4, so the write subquery now `RETURN`s `count(edge_to_close)` and the outer query sums it. Both are reported through `migration_input.console`, whose default is `get_migration_console()`.
...
- [X] T048 [US3] Document the deletion semantics for branch-agnostic attributes and relationships on branch-aware objects in the relevant page under `docs/docs/`: when the value is released, when release is deferred, what resolves the deferral (delete on the last branch holding the object, rebase past the deletion, merge of the branch the deletion was made on, or branch deletion), and what a branch forked before the deletion sees (FR-019, SC-009) **Done 2026-09-02.** Added "Deleting a branch-agnostic attribute or relationship" to `docs/docs/schema/branch-awareness.mdx` — the page that already defines `branch: agnostic`, so the semantics sit next to the setting that causes them. **Moved 2026-09-03** to its own page `docs/docs/branches/branch-agnostic-data.mdx` (new spoke under the Branches category): the rule is object-level and spans delete/merge/rebase/branch-deletion/schema-removal, so it did not belong on a page that otherwise only documents the schema `branch:` setting. `branch-awareness.mdx` keeps a three-sentence pointer, and the four deep links (schema/migration, branches/overview, branches/delete, resource-manager/overview) now target the new page. The opening rule turns on the **field** being unreachable from every branch — either no branch holds a live object, or no branch declares the field any more — rather than on the object alone, which is what keeps the schema-removal path consistent with it. Subsections: what releases the value, named per actor (delete on the last branch holding the object; merge **of the branch the deletion was made on**, since merging a branch that still reads the object releases nothing — it survives its own merge and goes on reading it, per `test_merging_the_deletion_releases_nothing_while_another_branch_retains_the_object`; rebase of a branch past the deletion; deletion of a branch that was still reading; schema removal once no branch declares the field), stating explicitly that none of them releases unconditionally; why deleting on one branch is not enough; which branches still count; and what counts as gone (one owner for an attribute, both peers for a relationship). A branch created after the deletion stops counting only if the deletion had already reached the branch it forked from — a fork from an unreached branch reads the object as live. A note ties resource-pool allocations to the same rule, and the Related concepts list gains a resource-manager link. FR-019/SC-009 covered. **Corrected 2026-09-02** after review: the first draft listed "merging that branch" among the deferral-resolvers with the retaining branch as its antecedent, which is false in both the code and the tests.
- [X] T049 [US3] Document that `m078` mutates existing data on upgrade — closing edges and hard-deleting vertices with no linked node — and that it is irreversible, in the upgrade documentation under `docs/docs/` **Closed 2026-08-31 as not required**, by maintainer decision: a database migration is Infrahub's responsibility and should be transparent to the operator, and the standard upgrade procedure already tells them to take a backup first. A section had been written into `docs/docs/deploy-manage/maintain-upgrade/upgrade/overview.mdx` and was reverted.
- [X] T050 [US3] Run `uv run invoke docs.lint` and fix any Markdown violations per `dev/guidelines/markdown.md` **Partial validation, 2026-09-02 — `uv run invoke docs.lint` itself never ran end to end in this environment.** *markdownlint: full scope.* Ran `markdownlint-cli2` with the repo's `.markdownlint-cli2.yaml` and the exact globs the task passes (`docs/docs/**/*.md`, `docs/docs/**/*.mdx`), all 187 pages, 0 errors; `docs.lint` aborts before reaching it unless `markdownlint-cli2` is on `PATH`, so run it from `docs/node_modules/.bin`. *vale: changed pages only, not the tree.* `vale` is not installed locally either — ran it through `jdkato/vale` in Docker over the changed pages plus the changelog fragments: clean after one fix, "tombstoned" tripped `Infrahub.spelling` in the fragment and was reworded. A full-tree vale run in that image reports 247 pre-existing errors, almost all frontmatter keys (`hide_table_of_contents`, `toc_max_heading_level`), which is an image/version artifact rather than a real regression; left uncleared and out of this feature's scope. `dev/` Markdown is outside `docs.lint`'s globs altogether and was wrapped to each file's existing width by hand.
...
- [X] T051 [P] Add a towncrier changelog fragment under `changelog/` for the user-visible behaviour: branch-agnostic values are released when no branch retains their object, and freed pool values become allocatable again **Written 2026-08-31** as `changelog/9762.fixed.md`, named for the issue it fixes; trimmed since, and this note was corrected on 2026-09-02 to match what the file actually says. The fragment covers the pre-fix symptom (the value still counted towards uniqueness constraints, and an allocated resource pool value stayed allocated to an object that no longer existed), the release rule with its per-kind arity (an attribute once no branch can read its object, a relationship once no branch can read both of its peers), that released pool values become allocatable again, that upgrading clears the backlog, and a link to the user-facing section. Deliberately *not* restated in the fragment: the deferral-resolving events and the six enforcement points — a changelog entry is the wrong altitude for either, and the linked section carries both. The rollback timestamp correction that rode along on this branch has its own fragment, `changelog/+rollback-restores-exact-timestamp.fixed.md`.
- [X] T052 [P] Update `dev/knowledge/backend/` with the retirement invariant and the six enforcement points, per the constitution's Documentation Requirements for backend architecture changes **Done 2026-09-02.** Added as a "Branch-agnostic fields on branch-aware Nodes" subsection under Node Lifecycle → Deletion in `dev/knowledge/backend/database-schema.md`, rather than as its own page: that doc already carries `branch_support`, the global branch and the delete edge mechanics, which is the context the rule only makes sense in. Three paragraphs — why a branch-level delete alone leaks the value, the rule that every path which can stop an object being readable must re-evaluate retention (with the per-branch/per-vertex conjunction and the two-peer requirement for a `Relationship`), and the pointer to `UNRETAINED_AGNOSTIC_FIELD_PREDICATE` as the one place the logic lives, listing the six call sites plus `m078` and stating that a seventh deletion path needs a seventh call site. A first draft as a standalone `branch-agnostic-retirement.md` was discarded as too verbose for what the reader needs.
...
- [X] T062 Renumber the repair migration from `m077` to `m078` and the version bump from 76→77 to 77→78. `m077_delete_orphaned_account_children.py` landed on the base branch (via #10435) and `GRAPH_VERSION` is already 77 there, so the T058-corrected numbers are taken and `discover_migrations()` would raise `ImportError: Duplicate migration number 077`. Renamed `backend/infrahub/core/migrations/graph/m077_retire_agnostic_property_edges/` → `m078_retire_agnostic_property_edges/` and its test package `backend/tests/component/core/migrations/graph/m077_retire_agnostic_property_edges/` → `m078_retire_agnostic_property_edges/`, class `Migration077` → `Migration078`, `name` `077_retire_agnostic_property_edges` → `078_retire_agnostic_property_edges`, `minimum_version` 76 → 77, and `GRAPH_VERSION` 77 → 78 in `backend/infrahub/core/graph/__init__.py`. Registration again needed no edit — discovery is by directory name. **Done 2026-09-02.** Stale numbers corrected in the Blocking-gate section, T001, T039–T044, T046, T047, T049, T052, T058, `contracts/retirement-component.md` C4, `research.md`, `opsmill-implement-report-r06.md`, `PR_DESCRIPTION.md` and `dev/knowledge/backend/database-schema.md`; `plan.md` and `IFC-2843-prd.md` still read `m076` from before T058 and are left for the plan's own revision. Re-confirm the number is still free at the moment it lands.
