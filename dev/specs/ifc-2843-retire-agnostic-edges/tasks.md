...
- [ ] T041 [P] [US2] Write component tests for the **shared-value** shape: two attributes sharing one `AttributeValue`, one orphaned → orphan detached, surviving attribute keeps its value (scenario 3); and for unrepairable state → reported, migration completes without raising (scenario 4) Delivered as two files. `.../m077_retire_agnostic_property_edges/test_shared_value.py` covers scenario 3 twice, because the orphan reaches the shared value through either repair: the close side (one owner tombstoned → the orphan's own `HAS_VALUE` edge closes, the survivor's edges are byte-identical and still read the value) and the hard-delete side (one node vertex detached → the orphan attribute vertex is removed, the survivor's attribute and the shared value vertex both survive). Sharing is pinned on `elementId(value)`, not on the value property, so two equal-but-separate vertices cannot pass for one. Mutation-checked: making the hard-delete `DETACH DELETE field, v` fails the delete-side test.
  ...
  `.../test_unrepairable.py` covers scenario 4, and **required a production change** — T044's note flagged that the skipped-candidate count was not reported at all, which scenario 4 asks for. Added `CountUnstampableAgnosticFieldsQuery` (read-only, `get_data() -> UnstampableAgnosticFieldCount`) plus a third exception-guarded pass in `execute` that logs `Skipped N branch-agnostic field(s) with no derivable retirement time`. Three points worth recording:

  1. **The count is not carried in `errors`.** Verified against `backend/infrahub/core/migrations/runner.py:53`: a non-empty `errors` makes `MigrationResult.success` false and the runner raises `MigrationFailureError`, which would abort the upgrade. C4's "returns `errors=[...]` on unrepairable state; never fails the upgrade" is therefore self-contradictory as written. `errors` stays reserved for caught exceptions; unrepairable **data** is reported to the console only, which is what FR-016 and SC-003 actually require. C4 should be amended.
  2. **The count pass runs last, after both repairs**, so what it counts is what could not be repaired: a field whose edges the close pass just shut no longer offers an open global edge and is no longer a candidate.
  3. **The anchor and stamp derivation are now one shared Cypher constant** (`_UNRETAINED_AGNOSTIC_FIELDS_WITH_STAMP`), consumed by the close query with `WHERE retired_at IS NOT NULL` and by the count query with `WHERE retired_at IS NULL`. Without that the two would drift and the reported count would stop matching what was skipped.

  The unrepairable fixture strips the owner's `IS_PART_OF` edges outright: no branch reads the owner as live, so the candidate is unretained, yet nothing records when it stopped being readable, so neither `owner_gone_at` nor `owning_closed_at` resolves. A second test asserts an ordinary orphan reports `Skipped 0`, so the signal cannot be satisfied by always printing a number.
...
- [ ] T049 [US3] Document that `m077` mutates existing data on upgrade — closing edges and hard-deleting vertices with no linked node — and that it is irreversible, in the upgrade documentation under `docs/docs/`
