...
| Head commit | `333c7faef`, plus this report's own commit |
...
### Review-fix pass 1 (not a task chunk)

- Commit: `704e94013458d3b2ea2350cf37362bfbb0406c47`
- Five review findings fixed: one high, four medium. Detail in section 5.

### Review-fix pass 2 (not a task chunk)

- Commit: `333c7faef8ed73fa76be75c4f69a716d54f8526a`
- Four findings fixed: three medium, one low. Detail in section 5. The second review pass also
  verified every pass 1 fix and adjudicated the one finding pass 1 had left contested.
...
|---------|------|-------------|----------------------|---------------------|--------------------|
| `backend/tests/component/git/test_utils.py` (all 7: 2 pre-existing modified for `-global-` removal, 3 added by chunk 2, 2 further modified plus the `branch_registry_restored` autouse fixture added by review pass 1, 1 more added by review pass 2) | component | `env -u INFRAHUB_GIT_IMPORT_SYNC_BRANCH_NAMES INFRAHUB_USE_TEST_CONTAINERS=true uv run pytest backend/tests/component/git/test_utils.py -n 0 -p no:randomly -v` | 2026-09-18T10:54:49Z (final, post review pass 2) | testcontainers Neo4j; common environment above | `======================= 7 passed, 16 warnings in 23.95s ========================` |
| `backend/tests/component/git/test_utils.py::test_get_repositories_commit_per_branch_repository_created_on_a_branch` (added by review pass 2) | component | as the row above | 2026-09-18T10:54:49Z | testcontainers Neo4j; common environment above | `backend/tests/component/git/test_utils.py::test_get_repositories_commit_per_branch_repository_created_on_a_branch PASSED [ 57%]` |
...
Run after every chunk and again after each review pass, all clean at head `333c7faef`:
...
Two review passes ran, each with four agents in parallel across `2db4b622f..HEAD` (code quality;
tests; errors and types; comments and simplify). **No critical findings in either pass, and no high
findings in the second.** Pass 2 independently verified all five of pass 1's fixes as landed and
correct, rather than taking the fix claims on trust.

Note on scope: the review extension's `detect-changed-files.sh` diffs against the repository default
branch `stable`, which yields 3037 files - the whole integration branch plus everything merged into
`develop` since `stable`. That scope was overridden to `2db4b622f..HEAD` (11 files), the actual PR
diff, `2db4b622f` being the confirmed merge-base with `origin/cross-branch-repo-status-infp-671`.

### Pass 1, fixed inline (commit `704e94013`)
...
| Medium | `backend/tests/component/git/test_utils.py` (now `:253`) | `count_for(...) <= math.ceil(BRANCH_COUNT / CHUNK_SIZE)` also passes at 0 or 1 queries, so deleting `itertools.batched` would keep it green. Changed to `==`, keeping the `math.ceil(...)` expression. |
...
### Pass 2, fixed inline (commit `333c7faef`)

| Severity | File | Finding |
|----------|------|---------|
| Medium | `backend/tests/component/git/test_utils.py` | No test anywhere created a repository on a non-default branch, so the assumption the whole refactor rests on - that one `NodeManager.query` on the default branch finds every repository, because the node kinds are AGNOSTIC - was asserted nowhere. Had the default-branch query missed such a node, the repository would vanish silently from `sync_remote_repositories` (never cloned, no error) with the suite green. Fixed: added `test_get_repositories_commit_per_branch_repository_created_on_a_branch`, which creates the repository inside a user branch, sets `staging` there, and asserts the staging-on-a-branch / inactive-on-main shape the sync flow keys off. Proven non-vacuous by asserting against an absent repository name and observing the failure. |
| Medium | `backend/infrahub/git/utils.py:39` | The `ref`-is-branch-agnostic error corrected in `git-sync.md` by pass 1 survived in the source docstring that repeats it. Reworded to match: every field other than `commit` and `internal_status` carries the default branch's value. |
| Medium | `dev/knowledge/backend/git-sync.md` | "Every field other than `commit` and `internal_status` therefore holds the default branch's value" read as though the repository node's own `commit` and `internal_status` were per-branch. They are not: the node carries default-branch values for every field, and the per-branch values live only in `branches` and `branch_info`. Reworded. |
| Low | `backend/infrahub/git/utils.py` | The fallback warning bound the substituted value as `internal_status=`, which reads like a value that was resolved. Renamed to `fallback_internal_status=`. Flagged independently by two agents. |

### Refuted, no action

| Claim | Resolution |
|-------|------------|
| Repositories created on a non-default branch and not yet merged now drop out of the result, exposing `PythonTransformComputedAttribute.repository_commit` to a `KeyError` (raised by chunk 1) | **Not reachable.** `CoreRepository`, `CoreReadOnlyRepository` and `CoreGenericRepository` are all `branch=BranchSupportType.AGNOSTIC` (`core/schema/definitions/core/repository.py:32,69,107`), so the node is written on `-global-` and stays visible from the default branch whichever branch created it. `repositories.get(name)` in `gather.py:93` cannot be `None` for that reason. Corroborated by the deleted `-global-` test expectations, which only arise for agnostic nodes. Pass 2 closed this with a regression test, listed above. |
| The `internal_status is None` warning at `backend/infrahub/git/utils.py` fires on an expected absence (a branch forked before the repository was added) and will train operators to ignore it (raised by pass 1's errors agent, contested by its code agent) | **Refuted by pass 2, with evidence; keep the code as written.** `core/attribute.py:687-689` forces a `BranchSupportType.LOCAL` attribute on an AGNOSTIC node onto the global branch at creation, so `internal_status`'s creation edges land on `-global-`. `core/query/repository.py:115,129` accept `r.branch IN [branch_name, $global_branch_name]` with no fork-point narrowing on the global clause, so that edge resolves on every branch, including branches forked before the repository existed. `core/schema/definitions/core/repository.py:164-165` declares the attribute `optional=False` with default `inactive`, so the resolved value is never NULL. The fallback therefore cannot fire for any repository that exists on the default branch, which is the only source of the repository list. The one reachable path left - a branch in the `registry.branch` cache but absent from the graph - is rare and the warning is genuinely useful there. |
...
| Medium | `backend/infrahub/git/utils.py` | The `internal_status is None` fallback is not merely untested but **untestable**: the reader is constructed inside the function, so no test double can omit the attribute, and mocking is banned. The sibling consumer does this correctly - `graphql/queries/repository_branch_status/resolver.py:65` takes an injected `build_source` callable. Closing it means injecting the source into a shared helper called from two Prefect flows, a design change well beyond the flagged lines. Deferred, and weakened by the adjudication above: the path is effectively unreachable. |
| Medium | `backend/tests/component/git/test_utils.py` (now `:179-182`) | Two of the three provenance assertions (`default_branch`, `location`) are vacuous, since those fields are AGNOSTIC and cannot diverge. Genuine default-branch provenance is proven only for `ref`. This is a limitation of T051's text, not of the implementation. |
...
| Low | `backend/tests/component/git/test_utils.py:20` | Hardcodes `"node_get_list"` while the neighbouring import uses `RepositoryBranchAttributesQuery.name`. Using `NodeGetListQuery.name` would fail at import on a rename instead of as a confusing `0 == 1`. |
| Low | `backend/tests/integration_docker/test_computed_attributes.py:494` | Asserts `== ["swe-sth-router-1"]` for all main devices, encoding state left by earlier cases in the class; adding any case that creates a main device breaks it. |
| Low | `backend/tests/component/git/test_utils.py` (now `:180`) | Covers `commit.value is None` but not the other arm (`commit is None`, attribute absent from the reader result). |
| Low | `backend/tests/component/git/test_utils.py:26-33` | The autouse `branch_registry_restored` fixture snapshots `registry.branch` before `default_branch` runs `reset_registry`, because a function-scoped autouse fixture with no dependencies sets up first. Its teardown therefore leaves the registry empty rather than restored, which its docstring overstates. Harmless for the 7 tests in this module, and still strictly safer than the 200-branch leak it replaced. |
| Low | `backend/tests/component/git/test_utils.py:22` | `BRANCH_COUNT = 200` is an exact multiple of `REPOSITORY_BRANCH_READ_CHUNK_SIZE`, so a partial final chunk is never exercised. Near-zero risk while `itertools.batched` does the slicing; 201 branches would pin it. |
| Low | `backend/tests/component/git/test_utils.py:253-254` | Only two query names are counted, so total query volume is unasserted and `CountingInfrahubDatabase.rows_for`, provided for exactly this, is unused. Per-branch row growth in the reader is not pinned. |
| Low | `backend/tests/component/computed_attribute/test_gather.py:315` | The new gather test adds branches to `registry.branch` without save-and-restore, the same leak fixed one module over. Consistent with the pre-existing tests in that file, so pattern-level rather than newly introduced. |
| Low | `backend/infrahub/git/models.py` | `get_staging_branch()` has no test anywhere, and neither does `sync_remote_repositories`, while this change alters both the key set they iterate and its values. Mitigated by the new utils-level assertions guarding the input. |
| Low | `changelog/+repository-sync-single-read.changed.md` | Attributes the bounded read to the repository sync only; the same read also backs the Python computed-attribute gather (`gather.py:66,153`). |
| Low | `dev/knowledge/backend/git-sync.md` | "always costs `1 + ceil(N / 100)`" is not always: with zero repositories the function returns after the single node query. |
...
7. **The review's own file-detection script was overridden.** `detect-changed-files.sh` diffs against
   the repository default branch `stable` and returned 3037 files, which would have reviewed work
   merged by other people rather than this change. Scope was set to the PR's real diff instead. See
   the note at the top of section 5.
8. **The untestable fallback was left alone.** Pass 2 argued for injecting
   `RepositoryBranchAttributesSource` into `get_repositories_commit_per_branch` so the fallback path
   becomes reachable from a test. That is a design change to a shared helper called from two Prefect
   flows, beyond the lines the review flagged, and the same pass established the path is effectively
   unreachable in production. Recorded as deferred rather than acted on.

---

## 7. Suggested next steps

1. **Run `/pre-ci`** at `333c7faef` or later. This is the first half of T056 and has not been run.
