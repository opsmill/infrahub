...
| MEDIUM | `repository.py:85` | `default_window` omits the `at > branched_from` guard that `branch/models.py:314` applies, so a historical read at a time before a branch was created gets a too-permissive default-branch window. | Deferred. Unreachable from GraphQL now that `at` is rejected there, but the primitive still advertises `at`. |
