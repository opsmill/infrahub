...
- [ ] T044 [P] [US1] Add `changelog/+repository-branch-status-query.added.md` with the `creating-changelog-entries` skill, describing the new query as it landed (grep the diff for the exact argument names before writing). Nothing about `InfrahubBranch` belongs here: T007 was dropped, so this feature never adds a filter to it
