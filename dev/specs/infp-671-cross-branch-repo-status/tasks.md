...
| --- | --- | --- | --- |
| **PR 1** | 1, 2 (T001 to T011, including T009a and T009b) | [IFC-3125](https://opsmill.atlassian.net/browse/IFC-3125) | Branch-list plumbing: `sync_with_git__value` filter, unpaged `Branch.get_list`, total-ordered standard-node reads, branch-name default ordering |
...
- [X] T007 Add a `sync_with_git__value=Boolean()` argument to `InfrahubBranchQueryList` and pass it through `infrahub_branch_resolver` into `BranchListFilters` in `backend/infrahub/graphql/queries/branch.py`. The `__value` suffix is what its sibling filters (`name__value`, `status__value`) use for a value-wrapped branch field, and renaming it after publication would be a breaking schema change
...
- [X] T009b Add `OrderByField.NAME` in `backend/infrahub/constants/enums.py` with its `ORDER BY` arm in `StandardNodeGetListQuery.query_init`, and default `standard_node_ordering_from_order_input` to name ascending instead of database id, so an unordered `InfrahubBranch` read agrees with the `order_rows` default in T018. Only `Branch` of the four `StandardNode` subclasses has a `name`, so this changes the branch query's default and not the shared `StandardNodeOrdering()` default. Apply the ordering direction to the `ID` arm in the same pass; it was silently dropped
- [X] T010 [P] Component tests in `backend/tests/component/graphql/queries/test_branch.py`: `sync_with_git__value: true` returns only syncing branches, `false` only non-syncing, unset returns both, and `count` follows the filter on the paginated `InfrahubBranch` query; an unordered read comes back in branch-name order; `offset` without `limit` returns the T009a `ValidationError`
- [X] T011 [P] Component tests in `backend/tests/component/core/test_branch.py`: with `config.SETTINGS.database.query_size_limit` lowered to 3 for the test and 10 branches saved, `Branch.get_list(limit=None, node_ordering=<created_at desc>)` returns all 10 exactly once and in a stable order across two calls (pins T008 and T009 together); and the generated `order_by` of `BranchNodeGetListQuery` carries the id tiebreaker after each of the `created_at`, `updated_at` and `name` sort keys, which is the assertion that fails without T009
...
**Checkpoint**: `InfrahubBranch` accepts `sync_with_git__value` and defaults to branch-name order;
unpaged branch reads are total-ordered.
...
- [ ] T026 [US1] Regenerate and commit generated files: `uv run invoke schema.generate-graphqlschema`, diff `schema/schema.graphql` against `contracts/graphql-repository-branch-status.graphql` (field names, argument defaults, descriptions, the `sync_with_git__value` argument on `InfrahubBranch`), `uv run invoke schema.validate-graphqlschema`, then `cd frontend/app && pnpm codegen` and commit `frontend/app/src/shared/api/graphql/generated/types.ts`. Any divergence from the contract is fixed in code, not in the contract, unless the plan is wrong, in which case the contract file and this tasks.md are updated in the same commit
...
- [ ] T030a [P] [US1] Add the changelog fragments for the user-visible `InfrahubBranch` changes that shipped in PR 1, with the `creating-changelog-entries` skill: `changelog/+branch-list-sync-with-git-filter.added.md` for the `sync_with_git__value` argument, and a `changed` fragment for the default ordering moving from database id to branch name ascending. Grep the diff for the exact argument name first. The new query itself stays unlogged while its values are fabricated
