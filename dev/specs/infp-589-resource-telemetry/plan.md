...
Add a per-component `resources` block to the daily telemetry payload reporting logical CPU cores (available + assigned) and memory (available + used) for the **database**, the **API server**, and the **worker fleet**, so a deployment can be audited against its contracted tier from a single snapshot — including offline/air-gapped deployments.
...
| III. Type Safety & Explicit Contracts | ✅ Pass | New `resources` block is typed Pydantic (`int \| None` fields), never an untyped dict. Mirrors `TelemetryDatabaseSystemInfoData`. |
...
├── models.py            # + TelemetryComponentResources, TelemetryWorkerResources,
│                        #   TelemetryResourcesData; + resources field on TelemetryData
...
├── database.py          # reuse existing JMX system-info numbers for a NEW resources.database
│                        #   row (existing `database`/`system_info` kept unchanged);
│                        #   read server.cypher.parallel.worker_limit (SHOW SETTINGS) for cores_assigned
└── tasks.py             # gather: build the resources block; aggregate server + workers
