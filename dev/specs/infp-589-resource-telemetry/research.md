...
- **Correction (comparability finding, post-implementation)**: `psutil.cpu_count(logical=True)` is not container-aware, while the `Runtime.availableProcessors()` behind the database figure has been since JDK 10 — under a cgroup CPU quota the JVM reports the quota (rounded up) and psutil the host. Measured on an 18-core host with `--cpus=2`: JMX 2, psutil 18. The same field therefore meant *usable cores* for the database and *host cores* for server/workers, which defeats FR-002. Fixed by capping the server/worker `processor_available` at the enforced quota (`min(host logical count, quota)`, the JVM's rule), so `processor_available` is the CPUs a component can actually use for every component. `processor_assigned` is unchanged — the quota itself, `null` when unenforced — and may exceed `processor_available` when a quota is set above the host's count. The uncapped host count is no longer in the payload; `infrahub telemetry probe-resources` reports it next to the reading for comparison. A `cpuset` restriction (e.g. `docker run --cpuset-cpus` with no `--cpus`) pins the process to specific CPU IDs without setting a quota, so the quota cap alone misses it and the JVM's own container support does react to it; `processor_available` is also capped by the process's CPU-affinity mask (`psutil.Process().cpu_affinity()`, `None` — no additional restriction — where unsupported, e.g. macOS).
...
- **Correction 2 (review finding, post-implementation)**: a limit file that exists but fails to read (permission denied, I/O error) was treated the same as one that is absent, so the ancestor walk silently kept going and could return a looser ancestor's value in place of the leaf's real, unreadable one — understating the enforced restriction for an audit that must never overstate what's available. Fixed by distinguishing "file does not exist at this level" (`FileNotFoundError`, the expected case for most levels) from any other read failure: the latter now stops the walk and reports the metric as unknown (`null`), with a warning logged, instead of falling through.
...
## D12 — Cache the resolved cgroup path and host CPU count; re-read the limit values every heartbeat

- **Decision**: `host`, the resolved cgroup directory list, and the host's logical CPU count (`psutil.cpu_count(logical=True)`) are constant for a process's lifetime — none of them can change without a container restart — so they are read once (at process start / first heartbeat) and cached. `processor_available`, `processor_assigned`, `memory_total`, and `memory_available` are re-read on every heartbeat.
- **Correction (review finding, post-implementation)**: the original decision below cached all four figures for the process's lifetime, on the reasoning that they never change. That is false for a *live* limit reconfiguration — `docker update --cpus=...`, a Kubernetes in-place pod resize, a systemd unit reload — which changes the content of `cpu.max`/`memory.max` at an already-resolved cgroup path without restarting the process. A cached reading made such a change invisible until the process restarted, which defeats the feature's purpose: an audit against a currently-contracted tier should reflect a limit raised or lowered after the worker started, not require a restart to notice. Fixed by splitting what is genuinely static — the cgroup *path* (resolving it means walking `/proc/self/cgroup` and its ancestors, which cannot move without a restart) and the host's logical CPU count — from what can be reconfigured live — the *values* inside `cpu.max`/`memory.max` at that path. The path stays cached; the values are re-read each call.
- **Rationale (what stays cached)**: re-walking `/proc/self/cgroup` and the ancestor directories on every heartbeat (~15 s) would be wasted work for a path that cannot change short of a restart, so that resolution — and the host CPU count — is still read once.
- **Rationale (what is now re-read)**: the cost of re-reading a handful of small cgroup files (`cpu.max`, `memory.max`, `memory.current`, typically 1-3 directories deep) every ~15 s is a few extra file reads, not a hot path — negligible next to the benefit of reflecting a live limit change in the next snapshot instead of requiring a process restart.
