...
- **FR-008**: System MUST support optional TLS encryption for TCP-based syslog connections. TLS configuration MUST follow the same approach as the existing HTTP adapter: an optional CA certificate path for validating the syslog server's certificate against a custom or private CA. Mutual TLS (mTLS) is out of scope for v1.
...
- **FR-026**: Syslog export MUST be available as an enterprise edition feature; the underlying event pipeline architecture MUST be available in all editions. The enterprise gate MUST be enforced using Infrahub's existing license/edition check mechanism by marking the syslog configuration section as an enterprise feature in the configuration layer.
...
- **Log Forwarding Destination**: A configured target for log forwarding, with attributes: name (unique identifier used in all observability output), type (in v1: `syslog`), host, port, transport protocol (TCP/UDP), syslog format (RFC 5424/RFC 3164), TCP framing method (newline/octet-counting), TLS enabled flag, optional CA certificate path (for validating the server's certificate against a custom/private CA, following the HTTP adapter pattern), queue size limit, maximum reconnection interval, shutdown drain timeout, application log forwarding enabled flag, and minimum log severity level.
...
- **Event Consumer (extensibility interface)**: The abstract interface through which the event pipeline dispatches to sinks. The Syslog Service implements this interface. This is new infrastructure introduced by this feature; the implementation pattern to follow is the existing Prefect-side event consumer code (how Prefect handles events after Infrahub emits them). Designed so that future integrations (webhooks, custom event hooks) can be added as additional consumers without changes to the core emission logic.
...
- The audit event payload emitted to syslog contains no sensitive field values. The event pipeline masks secrets before the payload is produced; syslog therefore receives the same sanitized data as Prefect with no additional sanitization step required at the syslog layer.
...
- Q: Does the audit event JSON exported to syslog require any sanitization or field omission beyond what is currently sent to Prefect? → A: No additional sanitization needed — the event pipeline already masks sensitive field values before producing the payload; syslog receives the same sanitized data as Prefect.
- Q: How should the enterprise edition feature gate be enforced for syslog export (FR-026)? → A: Use Infrahub's existing license/edition check mechanism; the syslog configuration section must be marked as an enterprise feature in the config layer.
- Q: What does TLS certificate configuration cover for syslog destinations? → A: Server CA certificate validation only (no mTLS), following the same approach as the existing HTTP adapter TLS configuration.
- Q: Does an event pipeline / consumer dispatch mechanism already exist in Infrahub that the Syslog Service will hook into, or must this infrastructure be built from scratch? → A: New infrastructure built from scratch; the implementation pattern to follow is the Prefect-side event consumer code that handles events after Infrahub emits them.
