...
- **REVISED 2026-07-08**: `fallback-version` is now the **static sentinel `0.0.0.dev0`** in both packages — never bumped, identical on every branch. This supersedes the data-model.md invariant "fallback sorts strictly above the latest release" and the earlier /cut-release fallback-bump step: a per-release moving fallback re-creates the exact stable→develop merge conflict this feature removes (it went stale twice during this PR's own review). Tag-less builds resolve `0.0.0.devN+g<hash>` — unmistakably a non-release; the release.yml fallback guard (which reads the fallback base from pyproject.toml at run time) and the strict-`>` propagation guards keep it unpublishable and inert.
