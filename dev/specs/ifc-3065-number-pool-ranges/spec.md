...
- **FR-006**: Every pool that exists before this change MUST become a pool with one range covering its old span and no weight. What it handed out and every figure it reports MUST stay the same. The migration MUST be idempotent. *(PRD FR-005)*
...
- **SC-003**: A deployment that upgrades and changes nothing sees no change: every pre-existing pool reports the same handed-out numbers, the same utilization and the same `start_range` / `end_range` values as before the upgrade. *(PRD SC-009 spirit, applied to P1)*
