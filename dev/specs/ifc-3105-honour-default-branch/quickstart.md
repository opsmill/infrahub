...
| Repository unreachable or erroring during an operation on a non-default Infrahub branch | `operational_status` on the repository, plus the failing run in its task history. The status is **not** per branch: the attribute is `BranchSupportType.AGNOSTIC`, so one value is shared across branches whichever branch the write names. Which branch of the graph reported the failure is readable from the linked task run, not from the status. Recorded in the spec's Assumptions. |
