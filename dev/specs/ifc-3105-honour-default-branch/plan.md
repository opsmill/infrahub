...
| Redundant graph reads on the paths that already hold the node (Principle V) | The periodic sync already holds the `CoreRepository` node and today forwards its trunk into the factory. FR-003 forbids any flow from supplying the trunk, so the factory re-reads a node the caller has in memory. **Two** such reads per read-write repository per sync cycle, not one: `bootstrap_local_repository` and the sync child flow each construct. At 50 repositories on a one-minute cron that is ~100 extra round trips a minute. Three further sites pay one uncached read per event — `merge_git_repository`, the two branch-lifecycle fan-outs — and `TransformWebhook.compute_payload` pays one per webhook delivery. The 16 `get_initialized_repo` callers are amortised by the existing 30 s cache. | Letting bootstrap keep forwarding the value is precisely the carrier FR-003 exists to delete, and every such carrier is a place the next caller can forget, which is how this defect arose. A carried trunk is also a snapshot: these payloads cross a bus and can be retried, so an operator edit between enqueue and execution makes them silently wrong in the same shape as the defect. Recorded so the carrier is not reintroduced as an optimisation in review; the follow-up that removes the waste without a carrier is in T072. |
| Call sites that resolve a trunk they cannot use (Principle VII) | `git_branch_create` and `git_branch_delete` need only local clone and worktree operations, but the only way to obtain an object that can perform them is to construct the full read-write repository, which requires the trunk and the staging status. Neither value can affect either operation: the branch being created is never Infrahub's default branch, so the mapping returns its input, and deleting a remote branch goes through no mapping at all. | Making the fields optional again is the shape this feature exists to delete. The real remedy is smaller objects — the repository class bundles local git operations, branch-name mapping, graph write-back, object import and staging decisions, and callers that want one of those pay for all five. That is a structural change well beyond a bug fix, and it belongs with `infp-546`; recorded there against its Story 3, and in T072. |
