...
- [x] T031 [P] [US1] Unit test (D4): the factory populates `infrahub_branch_name`, and `_update_operational_status` names that branch in its mutation for an operation running on a non-default Infrahub branch. This pins the one operator-visible move riding along with the fix
