...
- [X] T001 Add `selective_execution_after_merge` to `MainSettings` in `backend/infrahub/config.py` (mirror `delete_branch_after_merge`, description + env `INFRAHUB_SELECTIVE_EXECUTION_AFTER_MERGE`). Ships `default=False` on this increment — the default-True rationale (D9) assumes the US3/cascade fallbacks, which are not yet built; flip to `True` when they land.
