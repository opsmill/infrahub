...
- [ ] T006 Capture in `BranchMergeOrchestrator.merge` (`backend/infrahub/core/merge/orchestrator.py`): after changelog collection (~:101), before `freeze_diffs_for_branch` (:146), build summary + write cache in an **inner try/except** that yields `merge_diff_cache_key = diff_root_uuid` or `None` on failure and never re-raises (critique E7). Depends on T004, T005
