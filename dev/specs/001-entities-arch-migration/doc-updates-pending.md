...
- **Flat `nodes/` UI-only sub-modules**: `edit-form-hook`, `object-items`, `object-item-details`, `object-item-meta-edit` are UI/helper files sitting flat at the module root (no `api/domain/ui`). Like `object-template`, several are object-adjacent UI. Decide per-module whether to fold into the owning entity's `ui/` (e.g. `nodes/object/ui/`) or give them a `ui/` layer. `object-template` was folded into `nodes/object/ui/object-template/` as the first example.
