...
- [X] T021 [US2] `test_reimport_advances_consumer` — **`xfail(strict)`, UNVERIFIED on current stable.** The subagent observed (on the stale pre-#9601 image) that an explicit `InfrahubReadOnlyRepositoryImportLastCommit` returns `ok` but the read-only consumer's recorded commit never advances. #9601 fixes the periodic *default_branch* path, NOT the read-only *reimport* path, so this is **plausibly a real gap** — but it could not be re-confirmed on a current image (local image build network-blocked). **Needs a full-stack CI run to confirm before it counts as a blocker.** Kept `xfail(strict)`.
