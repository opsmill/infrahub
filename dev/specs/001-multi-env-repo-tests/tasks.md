...
- [X] T019b [US2] `test_nonmain_default_periodic_sync_advances` — **NEW confirmed defect, `xfail(strict)`.** A read-write `CoreRepository` whose git `default_branch` is not the primary branch **cannot pull a new commit via the every-minute periodic full-stack sync**: `sync` → `pull(branch_name="develop")` raises `ValueError: Unable to identify the worktree for the branch : develop` (the worktree is keyed by the mapped primary branch, not the git default branch), so the recorded commit stays frozen at the initial import. Reproduced deterministically in a single stack. **Distinct from anything the deterministic prong covers** — the in-process prong drives `sync_remote_repositories()` and asserts the commit advances (green); the full-stack periodic path fails. This is the multi-environment "one non-`main` branch per environment" pattern silently freezing after the first import.
...
- [X] T021 [US2] `test_reimport_advances_consumer` — **`xfail(strict)`.** An explicit `InfrahubReadOnlyRepositoryImportLastCommit` **does not advance** the read-only consumer's recorded commit to the ref's latest commit on the shared remote within the poll window; the recorded commit stays at the initially imported commit. Reproduced in a single consumer stack (mutation returns `ok`, recorded commit does not move). Whether this is the same root cause as T019b or a distinct read-only-import gap was not fully isolated.
