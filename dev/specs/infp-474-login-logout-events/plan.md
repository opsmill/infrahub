...
**Constraints**: Event emission failure MUST NOT block authentication (FR-001/003/005)
...
| **VI. Security** | ✅ PASS | `attempted_identifier` stored as-is, never interpolated into queries. `AuthenticationError` does not expose internal details in HTTP responses. |
...
│   ├── events/
│   │   ├── __init__.py                  # + export 3 new event classes
│   │   └── auth_action.py               # NEW: AccountLoggedInEvent, AccountLoginFailedEvent, AccountLoggedOutEvent
│   ├── api/
...
│   ├── core/
│   │   └── constants/__init__.py        # + 3 EventType enum values
│   └── graphql/
│       └── types/
│           └── event.py                 # + 3 new ObjectType classes + EVENT_TYPES entries
└── tests/
    ├── unit/
    │   └── event/
