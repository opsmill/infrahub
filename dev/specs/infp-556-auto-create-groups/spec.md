...
As an Infrahub administrator reviewing my groups list, I want to tell at a glance which groups were created manually by me and which were auto-created from external authentication (and from which provider), so I can audit, attach permissions to the right groups, and trust the provenance of the data.
...
**Independent Test**: Trigger auto-creation of a group via an OIDC login routed through a configured provider (e.g., `oidc_provider1`) and a separate auto-creation via a native LDAP login. Open the group list (UI and/or API). Verify the OIDC-triggered group has `origin = oidc_provider1` and the LDAP-triggered group has `origin = ldap`. Manually create a group via the UI/API. Verify it has `origin = manual`. Verify existing groups present before the migration have their `origin` attribute set to `manual`. Attempt to set or change `origin` via the UI, GraphQL mutation, REST API, or a schema load on any group — verify the operation is rejected (or the supplied value is ignored).

**Acceptance Scenarios**:

1. **Given** a group auto-created from an OIDC or OAuth2 login, **When** an admin views the group through the API or UI, **Then** the `origin` attribute reads the corresponding provider value (one of `oidc_provider1`, `oidc_provider2`, `oidc_google`, `oauth2_provider1`, `oauth2_provider2`, `oauth2_google`).
2. **Given** a group auto-created from a native LDAP login, **When** an admin views the group through the API or UI, **Then** the `origin` attribute reads `ldap`.
3. **Given** a manually-created group (via UI, API, or schema load), **When** an admin views it, **Then** the `origin` attribute reads `manual`.
4. **Given** an Infrahub instance upgraded from a pre-feature version, **When** the schema migration runs, **Then** the `origin` attribute on every pre-existing group is set to `manual` and no group is left without an `origin` value.
5. **Given** any existing group, **When** an admin attempts to set or change `origin` through the UI, a GraphQL mutation, the REST API, or a schema load, **Then** the operation is rejected (or the supplied value is ignored) — `origin` is read-only from all external write paths and may only be set by Infrahub itself.
...
- **Same effective name from two providers**: Two configured external authentication providers each emit a claim that resolves to the same effective local name. The local group is deduplicated to one. The `origin` value of that single group is the value set at first creation; subsequent contributions from other providers do not change `origin` (provenance for additional contributions lives exclusively in the auto-creation event log).
...
- **FR-012**: System MUST add an `origin` attribute to `CoreAccountGroup` modeled as a regular schema attribute (kind: enum, static value set), so the existing schema-driven UI and API surface it without dedicated UI work — admins can view and filter groups by `origin` through the standard group list mechanism. The full, fixed enum value set is: `oidc_provider1`, `oidc_provider2`, `oauth2_provider1`, `oauth2_provider2`, `ldap`, `manual`, `system`, `oidc_google`, `oauth2_google`. The enum is fixed at the schema level; adding a new auth-provider configuration at runtime does NOT extend the enum.
- **FR-013**: System MUST set the `origin` attribute on every auto-created group to the value that matches the originating authentication flow: one of `oidc_provider1`, `oidc_provider2`, `oidc_google`, `oauth2_provider1`, `oauth2_provider2`, `oauth2_google` for groups created from an OIDC or OAuth2 login (mapped from the configured provider that authenticated the login), and `ldap` for groups created from a native LDAP login. System MUST set the `origin` attribute to `manual` on every group created by admin-facing routes other than auto-creation (UI-created, API-created, schema-loaded). System MUST set the `origin` attribute to `system` on groups seeded by Infrahub's own internal/bootstrap paths (e.g., platform-seeded default groups) so admins can distinguish platform-seeded groups from admin-created ones.
- **FR-014**: System MUST migrate existing `CoreAccountGroup` rows on upgrade so every group has a non-null `origin` value; pre-existing groups MUST be set to `origin = manual`.
...
- **FR-021**: System MUST treat `origin` as read-only from every external write path. The schema MUST mark `origin` as system-managed so that GraphQL mutations, REST writes, schema loads, and UI submissions cannot set or modify it; user-supplied values for `origin` on a create or update operation MUST be ignored or rejected with a validation error. Only Infrahub's internal group-creation paths (auto-creation flow, the standard manual-creation flow that defaults to `manual`, the platform bootstrap path that sets `system`, schema migration) may set the value, and once set the value is immutable from external paths.

### Key Entities

- **Account Group (`CoreAccountGroup`)**: An existing Infrahub entity that represents a named group of accounts and acts as the carrier of role/permission assignments. This feature adds an `origin` attribute (kind: enum, static value set, read-only from all external write paths) whose value is one of `oidc_provider1`, `oidc_provider2`, `oauth2_provider1`, `oauth2_provider2`, `ldap`, `manual`, `system`, `oidc_google`, `oauth2_google`. Existing relationships to Roles, Permissions, and Account members are unchanged.
...
- **SC-005**: 100% of groups auto-created from an OIDC or OAuth2 login have their `origin` attribute set to the corresponding provider value (one of `oidc_provider1`, `oidc_provider2`, `oidc_google`, `oauth2_provider1`, `oauth2_provider2`, `oauth2_google`), 100% of groups auto-created from a native LDAP login have `origin` set to `ldap`, 100% of platform-seeded/bootstrap groups have `origin` set to `system`, and 100% of pre-existing (admin-created) groups have `origin` set to `manual` after migration; an audit query that groups `CoreAccountGroup` rows by their `origin` attribute returns the expected partition with no nulls.
...
- **Origin filter / visual marker in the UI** *(resolved 2026-04-30, refined 2026-05-05 — see Clarifications)*: No dedicated UI work is in scope. Because `origin` is a regular schema attribute of `CoreAccountGroup` (FR-012), Infrahub's schema-driven UI auto-renders it as a field/column on the group view and the generic filter mechanism already supports filtering by it. The "nice-to-have" UI request is therefore satisfied by the schema definition itself, with no bespoke UI work.
...
- **Schema migration**: Adding the `origin` attribute to `CoreAccountGroup` is a non-destructive schema migration that runs as part of the 1.10 upgrade path. The migration seeds `origin = manual` on every pre-existing group so no row is left without an `origin` value.
