...
**Independent Test**: Trigger auto-creation of a group via an OIDC login routed through a configured provider (e.g., `oidc_provider1`) and a separate auto-creation via a native LDAP login. Open the group list (UI and/or API). Verify the OIDC-triggered group has `origin = oidc_provider1` and the LDAP-triggered group has `origin = ldap`. Manually create a group via the UI/API. Verify it has `origin = manual`. Verify existing groups present before the migration are tagged `manual`. Attempt to set or change `origin` via the UI, GraphQL mutation, REST API, or a schema load on any group — verify the operation is rejected (or the supplied value is ignored).
...
4. **Given** an Infrahub instance upgraded from a pre-feature version, **When** the schema migration runs, **Then** all pre-existing groups are tagged `origin = manual` and no group is left without an `origin` value.
...
- **FR-013**: System MUST tag every auto-created group with the `origin` value that matches the originating authentication flow: one of `oidc_provider1`, `oidc_provider2`, `oidc_google`, `oauth2_provider1`, `oauth2_provider2`, `oauth2_google` for groups created from an OIDC or OAuth2 login (mapped from the configured provider that authenticated the login), and `ldap` for groups created from a native LDAP login. System MUST set `origin = manual` on every group created by admin-facing routes other than auto-creation (UI-created, API-created, schema-loaded). System MUST set `origin = system` on groups seeded by Infrahub's own internal/bootstrap paths (e.g., platform-seeded default groups) so admins can distinguish platform-seeded groups from admin-created ones.
...
- **SC-005**: 100% of groups auto-created from an OIDC or OAuth2 login carry the corresponding provider `origin` value (one of `oidc_provider1`, `oidc_provider2`, `oidc_google`, `oauth2_provider1`, `oauth2_provider2`, `oauth2_google`), 100% of groups auto-created from a native LDAP login carry `origin = ldap`, 100% of platform-seeded/bootstrap groups carry `origin = system`, and 100% of pre-existing (admin-created) groups carry `origin = manual` after migration; an audit query that counts groups grouped by `origin` returns the expected partition with no nulls.
