...
| tasks.md Phase | Jira | Status (as of 2026-05-14) |
|---|---|---|
| Phase 1 — Setup (Shared Infrastructure) | [IFC-2586](https://opsmill.atlassian.net/browse/IFC-2586) | Draft |
| Phase 2 — Foundational (Blocking Prerequisites) | [IFC-2587](https://opsmill.atlassian.net/browse/IFC-2587) | Draft |
| Phase 3 — User Story 1 (MVP) | [IFC-2588](https://opsmill.atlassian.net/browse/IFC-2588) | Draft |
| Phase 4 — User Story 2 | [IFC-2589](https://opsmill.atlassian.net/browse/IFC-2589) | Draft |
| Phase 5 — User Story 3 | [IFC-2590](https://opsmill.atlassian.net/browse/IFC-2590) | Draft |
| Phase 6 — User Story 4 (renamed to "Audit provenance of auto-created groups" in spec.md 2026-05-14; Jira ticket title still reads the old "Distinguish auto-created groups from manually-created ones" — Jira update pending) | [IFC-2591](https://opsmill.atlassian.net/browse/IFC-2591) | Draft |
| Phase 7 — User Story 5 | [IFC-2592](https://opsmill.atlassian.net/browse/IFC-2592) | Draft |
| Phase 8 — Polish & Cross-Cutting Concerns | [IFC-2593](https://opsmill.atlassian.net/browse/IFC-2593) | Draft |
| (Not a phase) Validation instructions | [IFC-2522](https://opsmill.atlassian.net/browse/IFC-2522) — Write validation instructions | Draft |
...
- [X] T002 [P] Create test directories `backend/tests/unit/auth_groups/`, `backend/tests/functional/auth_groups/`, `backend/tests/integration_docker/auth_groups/`, each with an empty `__init__.py`
...
- [X] T010 [P] [US1] Unit test `FilterEvaluator` matching + named-capture extraction + no-capture fallback (FR-005 to FR-008) in `backend/tests/unit/auth_groups/test_filter.py` — 8 tests, all pass. (Class renamed from `FilterPattern` → `FilterEvaluator` to disambiguate from `re.Pattern`.)
...
- [X] T012 [P] [US1] Functional test happy-path auto-creation in `backend/tests/functional/auth_groups/test_autocreate_flow.py` covering US1 acceptance scenarios 1, 2, 3, plus FR-018 dedup-within-login plus the filter-off / filter-on no-op cases. (Acceptance scenario 5 — CoreGroup-name collision fallback — is exercised by the production logic in `_find_or_create_group` and remains a follow-up test once a `GenericGroup` fixture is available.) Six tests written; execution requires Docker — they import cleanly but were not run in this session.
- [X] T013 [P] [US1] Concurrent first-logins test in `backend/tests/functional/auth_groups/test_concurrent_first_logins.py` issuing N simultaneous logins for the same brand-new claim and asserting exactly one group is created and every login succeeds (FR-011, SC-008). **Placement deviation**: written under `functional/auth_groups/` rather than `integration_docker/auth_groups/` because the existing SSO test suite uses the `db`/`default_branch` fixtures from `component/` style tests, and the existing `integration_docker/` tests all go through the SDK `InfrahubClient` against a fully-running infrahub stack — much heavier scaffolding than this single-process concurrency test needs. Execution requires Docker; imports cleanly.

### Implementation for User Story 1

- [X] T014 [P] [US1] Implement `FilterEvaluator` dataclass + `evaluate(claim) -> effective_name | None` + `evaluate_many(claims) -> list[str]` (first-match-per-claim, in declared order, with within-login dedup) in `backend/infrahub/auth_groups/filter.py` (FR-005 to FR-008, FR-018). Class renamed from the plan's `FilterPattern` to `FilterEvaluator` since the stored objects ARE `re.Pattern` instances — the evaluator wraps them.
...
- [X] T016 [US1] Implement `autocreate_groups_for_login(db, account, provider_name, claims, settings)` in `backend/infrahub/auth_groups/service.py` using a 2-layer find-or-create pattern (fast-path lookup → distributed lock `lock.registry.get(name=..., namespace="auto-create-group")` with under-lock re-check + create). The 3rd layer (catch `UniqueConstraintViolation` and re-fetch) was deemed unnecessary given the lock already serializes; can be added later if benchmarks expose a race. Sets `origin = provider_name` verbatim, dedupes effective names within the login, skips claims whose effective name fails `CoreAccountGroup` identifier validation. **Sso-default-group fallback** is NOT implemented inside this function — it lives in `signin_sso_account` and runs only on the legacy / disabled-filter path; coordinating the two (US3) is a separate task. (FR-005 to FR-011, FR-013, FR-017 skip behavior, FR-018, clarification 2026-05-14) — depends on T014.
- [X] T017 [US1] Hook `autocreate_groups_for_login` into `signin_sso_account` in `backend/infrahub/auth.py:310`. When `config.SETTINGS.security.auto_create_groups_enabled` is true, the new path runs and the legacy exact-name lookup-and-add is intentionally bypassed (FR-001/FR-005 — non-matching claims are skipped). When the filter is unset/empty, the legacy path runs unchanged. The call site passes `external_identity.provider_name` verbatim into the service (FR-013, clarification 2026-05-14). Lazy-imports `autocreate_groups_for_login` inside the function body to avoid making `auth_groups.service` import-time-eager from `auth.py` — depends on T016.
