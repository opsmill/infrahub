...
**Decision (current)**: The renderer emits a **single quantified-path-pattern (QPP) MATCH** for both default-branch and user-branch requests, parameterized by a planner-derived `$allowed_path_maps` (nested `{start_kind: {rel_name: [end_kind, ...]}}`). The QPP body uses *undirected* `-[:IS_RELATED]-` arrows so it can match all three `HopDirection` storage orientations in one pattern; `$allowed_path_maps` enforces the structural `(start_kind, rel_name, end_kind)` constraint per iteration. Source and target-by-uuid endpoints are resolved through their active `IS_PART_OF` edge to `Root` with `ORDER BY ... LIMIT 1` to handle kind migrations safely. The two branch-conditional pieces are:
