...
- Return data is exposed through frozen dataclasses (`PathData`, `PathHopData`, `PathNodeData`) that now live in `backend/infrahub/graph_traversal/results.py`. `PathData` carries a required `start_node: PathNodeData` plus `hops: list[PathHopData]` (the start node is *not* in `hops`); `PathHopData.relationship_identifier` is a required `str` because every hop traverses an edge, and `PathHopData.from_direction` is a required `RelationshipDirection` read back from the orientation of the hop's two stored `IS_RELATED` edges. `PathNodeData` carries just `uuid` and `kind` — the GraphQL resolver loads richer display metadata separately via `NodeManager.get_many` and merges it at response time. Path extraction lives in `backend/infrahub/graph_traversal/_extract.py` as the free function `extract_path_from_result(result) -> PathData | None`, consumed by `PathTraversalQuery.get_paths`; `ReachableNodesQuery._extract_path_data` still consumes a Neo4j Path (and constructs the new `start_node` field from the first vertex) until Phase 4 rewires it onto the planner shape.
