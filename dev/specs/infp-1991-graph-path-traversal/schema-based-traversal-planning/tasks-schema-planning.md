...
> - **`TerminalByKinds` constraint moved to the outer path-MATCH's final node** — `MATCH path = (source) (...){1, max} (target:$any($terminal_kinds))`. Earlier drafts pre-matched every node of those kinds upfront, then path-joined; this lets Neo4j drive target selection from the path search itself. The constraint binds only on the path's final node so it doesn't collide with intermediates that share a terminal label.
> - **QPP quantifier uses literal `plan.max_depth`**, not `$max_path_length` — Cypher rejects parameters in QPP `{m, n}` quantifier bounds.
> - **Cypher returns four separately-labelled values, not a Neo4j Path.** The renderer projects `head(nodes(path)).uuid AS start_node_uuid`, `head(nodes(path)).kind AS start_node_kind`, `[i IN range(0, depth - 1) | {relationship_identifier, uuid, kind, from_direction}] AS hops`, and `depth` — so only the properties the caller actually reads cross the driver, and hops/start_node are distinguishable at the projection level. `return_labels` is `("start_node_uuid", "start_node_kind", "hops", "depth")`.
> - **`PathData` / `PathHopData` / `PathNodeData` live in `backend/infrahub/graph_traversal/results.py`** (new non-private module), not in `path.py` or `_cypher.py`. `PathData` carries a required `start_node: PathNodeData` plus `hops: list[PathHopData]` (hops excludes the start); `PathHopData.relationship_identifier` is a required `str` (no `None`). `PathNodeData` was simplified to just `uuid` and `kind` — `label` / `display_label` / `hfid` were never read (the GraphQL resolver loads display metadata separately via `NodeManager.get_many` and merges it at response time).
> - **Path extraction is a free function in a sibling private module to `_cypher.py`** — `extract_path_from_result(result) -> PathData | None` in `backend/infrahub/graph_traversal/_extract.py`, called by `PathTraversalQuery.get_paths`. Returns `None` when a row lacks a start node or has no hops so the caller can skip degenerate rows. The module owns the `_HopRow` `TypedDict` describing the per-hop shape and uses a new `QueryResult.get_as_list_of_type(label, return_type=_HopRow)` accessor to recover typed hop rows without a `cast`. `ReachableNodesQuery._extract_path_data` still consumes a Neo4j Path (and constructs the new `start_node` from the first vertex) until it's refactored onto the planner shape — at which point it can call `extract_path_from_result` directly.
> - **`backend/tests/unit/graph_traversal/test_cypher.py` was removed** (T039). The renderer is exercised end-to-end through the Query-class tests instead of in isolation; bounds and module-boundary checks didn't justify a standalone file.
...
- [X] T032 Implement the `RenderedCypher` frozen dataclass in `backend/infrahub/graph_traversal/_cypher.py` per [contracts/query-generator.md §Surface](contracts/query-generator.md#surface): `text: str`, `params: dict[str, Any]`, `return_labels: tuple[str, ...]`.
- [X] T033 Implement `render_plan_to_cypher` in `backend/infrahub/graph_traversal/_cypher.py`: validates `plan.is_empty` and `max_results` bounds; emits the unified QPP shape (T034/T035 collapsed into this — see banner). Reads `plan.adjacency` directly to build `$allowed_path_maps`.
- [X] T034 ~~Implement `_render_default_branch`.~~ **Superseded by T033's unified QPP** (banner item 3).
- [X] T035 ~~Implement `_render_user_branch`.~~ **Superseded by T033's unified QPP** (banner item 3). User-branch differences (`$valid_branches` extension + two `NOT EXISTS` deletion clauses) are branch-conditional pieces of the single template.
- [X] T036 ~~Implement label-name interpolation safety `_label` regex helper.~~ **Dropped** (banner item 4).
- [X] T037 ~~Update `backend/infrahub/graph_traversal/planning/__init__.py` re-exports.~~ **Dropped** (banner item 5).
- [X] T038 ~~Update `backend/infrahub/graph_traversal/__init__.py` re-exports.~~ **Dropped** (banner item 5).

### 2G — Unit tests for Cypher rendering

- [X] T039 ~~Write `backend/tests/unit/graph_traversal/test_cypher.py`.~~ **Removed** — see banner. The renderer is now exercised through `test_path_traversal_query.py` and the component tests rather than in isolation.
...
- [X] T040 [US1] Refactor `PathTraversalQuery` in `backend/infrahub/graph_traversal/path.py` per [contracts/planner.md §Caller Contract](contracts/planner.md#caller-contract) and [contracts/query-generator.md §Caller integration](contracts/query-generator.md#caller-integration-pathtraversalquerysquery_init-reachablenodesqueryquery_init): `__init__(*, plan: Plan, source_id: str, branch: Branch, at: Timestamp, max_paths: int)` raises `ValueError` on empty `plan.routes`; `query_init` renders Cypher via the module-top-imported `render_plan_to_cypher` (no in-function imports, no short-circuit branch). Delete the now-unused Cypher-construction code from the old implementation. **Deviations from original wording** (see banner): the dataclasses `PathData` / `PathHopData` / `PathNodeData` moved out to `results.py` (with `PathData.start_node` added, `PathHopData.relationship_identifier` made required, and `PathNodeData` dropping its unused `label` / `display_label` / `hfid` fields); path extraction became a free function `extract_path_from_result` in `backend/infrahub/graph_traversal/_extract.py` (sibling to `_cypher.py`) consuming the four-label projection (`start_node_uuid`, `start_node_kind`, `hops`, `depth`) rather than a private static method over a Neo4j `Path`.
- [X] T041 [US1] Refactor `path_traversal_resolver` in `backend/infrahub/graphql/queries/path.py` per [contracts/planner.md §Caller Contract](contracts/planner.md#caller-contract): orchestrate plan construction — resolve source/destination kinds via `NodeManager.get_one(...)`, build the `PermissionResolver` with `PermissionLoader(account_session=...).load(db=db, branch=branch)` followed by `PermissionResolver(permissions=..., default_branch_name=registry.default_branch)`, construct the `SchemaPlanner(schema_branch=..., branch=..., permission_resolver=...)`, call `plan()`. Short-circuit at the resolver level when `plan.routes` is empty (return the existing empty-shape result without instantiating `PathTraversalQuery`), and only otherwise construct `PathTraversalQuery(plan=plan, ...)` and `await query.execute(db=db)`. Convert the existing GraphQL `PathTraversalInput` fields into a `UserFilters` via `UserFilters.from_graphql_input(data)`. All imports at module top. **Deviation**: reuses the already-loaded `graphql_context.active_permissions.resolver` (a `PermissionResolver` built by `PermissionManager.load_for_account()` during `prepare_graphql_params`) instead of constructing a fresh one — avoids reloading permissions. The `_wildcard_allow_resolver()` transitional helper was removed along with the unused `ObjectPermission` / `PermissionDecisionFlag` / `PermissionResolver` imports.
- [X] T042 [P] [US1] Extend `backend/tests/unit/graph_traversal/test_path_traversal_query.py`: existing constructor-validation tests still pass; add `__init__` raises `ValueError` for `Plan(routes=())`; the Query holds `self.plan` after construction.
- [X] T043 [P] [US1] **File-placement deviation**: tests that exercise the GraphQL resolver via `prepare_graphql_params` / `graphql()` were placed in `backend/tests/component/graphql/queries/test_path_traversal.py` rather than the originally-named Query-class file, since that is where the rest of the resolver-level tests live (e.g. `test_branch.py`, `test_event.py`). The Query-class file under `tests/component/graph_traversal/` continues to hold direct-execution scenarios. Acceptance scenarios from [spec §User Story 1](spec-schema-planning.md#user-story-1---plan-aware-path-discovery-between-two-objects-priority-p1):
   - Source/destination kinds with no schema route → empty result. Implemented via `test_resolver_short_circuits_when_no_schema_route` using a TestCar source + TestCar destination with `excluded_kinds=["TestPerson"]` (the only schema bridge), so the planner returns an empty adjacency and the resolver never builds a `PathTraversalQuery` (whose `__init__` would raise on an empty plan — that backstop makes the short-circuit observable as a regression). **Deviations from original wording**: the explicit `time.perf_counter` < 100 ms assertion was dropped (latency in a GraphQL-over-pytest test is dominated by `prepare_graphql_params` / `PermissionManager.load_for_account` DB hits, so a 100 ms gate would be flaky and uninformative); and the "instrument `query.execute`" assertion was dropped because instrumenting a Query-class method requires either monkey-patching (forbidden by the no-mocking rule) or an adapter layer in production code (out of scope here). Reliance on the `__init__` ValueError backstop satisfies the same regression-detection intent.
   - Source/destination kinds with one or more routes → `test_resolver_returns_paths_when_route_exists` (TestCar → TestPerson direct hop via `owner`) asserts the resolver returns paths whose first/last hops match the source/destination. **Deviation**: did not reuse `jack_with_blue_tag` for this scenario because the resolver applies the default `excluded_namespaces` set (which includes Builtin) and `UserFilters.from_graphql_input` no longer allows reducing that set — so any TestPerson → BuiltinTag plan is unconditionally empty at the resolver layer. Used a Test-namespace fixture (`two_cars_one_owner`) instead. `jack_with_blue_tag` is retained for the Query-class-level baseline tests where the planner is constructed with `excluded_namespaces=frozenset()`.
   - User without permission on an intermediate kind → **dropped** at the component layer. Building a restricted-permission account session requires creating Account + AccountGroup + AccountRole + ObjectPermission rows wired together with the right specificity rules, which is large enough that an isolated component test is out of scope here. `backend/tests/unit/graph_traversal/planning/test_permissions_filter.py` already covers `can_view`-driven pruning at the planner layer, and `test_resolver_returns_paths_when_route_exists` proves the resolver wires `graphql_context.active_permissions.resolver` into the planner end-to-end (admin sees the route).
- [X] T043a [P] [US1] FR-015 error-semantics parity tests live in `backend/tests/component/graphql/queries/test_path_traversal.py` (same file-placement deviation as T043). Messages pinned as module-level constants (`MISSING_SOURCE_MESSAGE_TEMPLATE`, `MISSING_DESTINATION_MESSAGE_TEMPLATE`, `SAME_SOURCE_DESTINATION_MESSAGE`); three tests assert `errors[0].message == ...` for each of the three error paths.
- [X] T044 [P] [US1] **Replaced by branch-isolation Query-class tests** in `backend/tests/component/graph_traversal/test_path_traversal_query.py`: `test_user_branch_edge_is_invisible_on_default_branch`, `test_default_branch_edge_deleted_on_user_branch_is_hidden`, `test_default_branch_edge_remains_visible_on_user_branch_when_not_deleted`. These execute against real Neo4j to verify the branch-filter semantics (`$valid_branches` + user-branch deletion-supersedes filter) rather than asserting on rendered Cypher substrings. The mirror set lives in `test_reachable_nodes_query.py`. **Deviation from task wording**: the originally-suggested rendered-Cypher substring assertions (`.from <= $at`, absence of `$allowed_path_maps`) don't match the actual unified-QPP renderer (which uses `$allowed_path_maps` on both branches), and assertions on rendered strings drift with every formatting change. The branch-isolation tests verify the same intent — strategy A vs strategy B — through observable behavior.
...
- [X] T045 [US2] Refactor `ReachableNodesQuery` in `backend/infrahub/graph_traversal/reachable.py` analogous to T040: `__init__(*, plan: Plan, source_id: str, default_branch_name: str, max_results: int)` raises on empty plan; `query_init` calls `render_plan_to_cypher`; old Cypher construction deleted. `ReachableNodeData` shape preserved (one entry per reachable node + path). `get_reachable_nodes()` now reads the QPP projection via `extract_path_from_result`, treating the last hop of each path as the reachable target (no separate `target_uuid` / `target_kind` columns needed — they're recovered from `path.hops[-1].node`).
- [X] T046 [US2] Refactor `reachable_nodes_resolver` in `backend/infrahub/graphql/queries/reachable.py` analogous to T041: builds the planner with `TerminalByKinds(kinds=frozenset(target_kinds))`, calls `UserFilters.from_graphql_input(data)`, short-circuits at the resolver layer when `plan.is_empty`. Validates every requested `target_kind` exists in the schema (raises `GraphQLError("Unknown target kind: …")`) before constructing the planner. Reuses `graphql_context.active_permissions.resolver` (same deviation as T041).
- [X] T047 [P] [US2] Rewrote `backend/tests/unit/graph_traversal/test_reachable_nodes_query.py`: three tests covering empty-plan rejection, valid construction, and the default `max_results=50`. The pre-refactor `max_depth` / `excluded_namespaces` constructor-validation tests were removed (those parameters live on `Plan` / `UserFilters` now, not on the Query class, and their validation is covered by the planner / Plan tests).
- [X] T048 [P] [US2] **File-placement deviation** (same as T043): resolver-level tests live in `backend/tests/component/graphql/queries/test_reachable_nodes.py`; Query-class branch-isolation tests live in `backend/tests/component/graph_traversal/test_reachable_nodes_query.py`. The User-Story-2 acceptance scenarios in the resolver file:
   - `test_resolver_short_circuits_when_no_route_to_target_kind`: TestCar source + `target_kinds=["BuiltinTag"]` (Builtin is in default excluded namespaces) → empty result.
   - `test_resolver_returns_reachable_targets`: TestCar source + `target_kinds=["TestPerson"]` against `car_with_owner_and_driver` fixture → both owner and driver appear in dependencies.
   - `test_resolver_supports_multiple_target_kinds`: `target_kinds=["TestPerson", "TestCar"]` → at least TestPerson kinds appear among results.
   - `test_resolver_excludes_targets_only_reachable_via_forbidden_intermediate`: dropped permission-restricted-account variant for the same reason as T043; the fixture provides only one TestCar so the empty result is deterministic.
- [X] T048a [P] [US2] FR-015 error-semantics parity tests in `backend/tests/component/graphql/queries/test_reachable_nodes.py`, pinned to module-level templates (`MISSING_SOURCE_MESSAGE_TEMPLATE`, `UNKNOWN_TARGET_KIND_MESSAGE_TEMPLATE`). **Deviation**: pre-refactor `ReachableNodesQuery` had no unknown-target-kind validation — Cypher would just return empty for unknown labels — so there is no historical message to match byte-for-byte. The new message (`"Unknown target kind: {kind}"`) is the post-refactor canonical form, asserted by the test to detect future drift. Missing-source error message preserves the pre-refactor wording verbatim.
