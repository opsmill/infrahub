...
> - **Cypher returns four separately-labelled values, not a Neo4j Path.** The renderer projects `head(nodes(path)).uuid AS start_node_uuid`, `head(nodes(path)).kind AS start_node_kind`, `[i IN range(0, depth - 1) | {relationship_identifier, uuid, kind}] AS hops`, and `depth` — so only the properties the caller actually reads cross the driver, and hops/start_node are distinguishable at the projection level. `return_labels` is `("start_node_uuid", "start_node_kind", "hops", "depth")`.
