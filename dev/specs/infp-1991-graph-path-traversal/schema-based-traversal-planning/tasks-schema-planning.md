...
> - **Cypher returns a structured list, not a Neo4j Path.** The renderer projects `[n IN nodes(path) | {uuid, kind, name}] AS path_data, depth` so only the three properties the caller actually reads cross the driver. The Query class's extractor consumes that list directly.
> - **`PathData` / `PathHopData` / `PathNodeData` live in `backend/infrahub/graph_traversal/results.py`** (new non-private module), not in `path.py` or `_cypher.py`. `PathNodeData` was simplified to just `uuid` and `kind` — `label` / `display_label` / `hfid` were never read (the GraphQL resolver loads display metadata separately via `NodeManager.get_many` and merges it at response time).
> - **`extract_path_data` is a private `@staticmethod` on each Query class** (`PathTraversalQuery._extract_path_data`, `ReachableNodesQuery._extract_path_data`) rather than a shared importable function. `PathTraversalQuery`'s consumes the structured `path_data` list; `ReachableNodesQuery`'s still consumes a Neo4j Path until it's refactored onto the planner flow.
