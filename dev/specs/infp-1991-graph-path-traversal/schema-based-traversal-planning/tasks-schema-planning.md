...
- [ ] T001 Create new package directories: `backend/infrahub/graph_traversal/`, `backend/infrahub/graph_traversal/planning/`, `backend/tests/unit/graph_traversal/`, `backend/tests/unit/graph_traversal/planning/`, `backend/tests/component/graph_traversal/`. Each gets an empty `__init__.py` so Python sees them as packages.
- [ ] T002 [P] Add empty placeholder modules so subsequent tasks have stable import targets: `backend/infrahub/graph_traversal/path.py`, `backend/infrahub/graph_traversal/reachable.py`, `backend/infrahub/graph_traversal/_cypher.py`, `backend/infrahub/graph_traversal/planning/models.py`, `backend/infrahub/graph_traversal/planning/planner.py`, `backend/infrahub/graph_traversal/planning/permissions.py`.
- [ ] T003 [P] Audit every import site that references the old module paths. Run from the repo root: `rg "from infrahub\.core\.query\.path|from infrahub\.core\.query\.reachable|infrahub\.core\.query\.path|infrahub\.core\.query\.reachable" backend/ python_sdk/` — record the full list in the PR description. The list must reduce to the GraphQL resolvers in `backend/infrahub/graphql/queries/path.py` and `reachable.py` plus the tests being moved (and any helpers internal to the moved files). Any additional caller blocks Phase 2.
- [ ] T004 [P] Create an empty placeholder Towncrier fragment file at `changelog/+infp-1991-schema-planning.changed.md` so CI picks up the path. T056 populates it with the final user-facing text from [quickstart §Where to write changelog entries](quickstart-schema-planning.md#where-to-write-changelog-entries).
...
- [ ] T056 [P] Fill in `changelog/+infp-1991-schema-planning.changed.md` with the final user-facing text describing the perf improvement and permission-aware filtering, per [quickstart §Where to write changelog entries](quickstart-schema-planning.md#where-to-write-changelog-entries).
