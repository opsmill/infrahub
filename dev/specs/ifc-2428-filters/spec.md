...
1. **Given** one or more filters are active, **When** the user looks below the filter button area, **Then** each active filter is displayed as a tag showing the field name, condition (e.g., "is any of", "contains", "after"), and value.
...
5. **Given** one or more filters are active, **When** the user opens the filter menu, **Then** the menu items corresponding to active filters are visually distinguished from inactive ones.
...
- **FR-005b**: Filter menu items that have an active filter MUST be visually distinguished (e.g., highlighted, checked, or showing the current value) so users can see which filters are currently applied directly in the menu.
- **FR-006**: The filter forms displayed on hover MUST reuse the existing attribute filter and relationship filter components.
- **FR-007**: When a filter is applied, the system MUST display it as an active filter tag below the search bar / filter button area. The tag MUST show the filter condition (e.g., "is any of", "contains", "after", "before", "empty", "not empty") alongside the field name and value.
...
- The backend already supports filtering by metadata fields via `node_metadata__` prefixed GraphQL arguments (e.g., `node_metadata__created_at__after`, `node_metadata__created_by__ids`). Confirmed in `schema/schema.graphql` and `backend/infrahub/graphql/manager.py`.
