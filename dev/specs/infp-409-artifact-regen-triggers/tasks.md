...
- [ ] T033 [US1] Integration_docker test in `backend/tests/integration_docker/proposed_change/test_artifact_regen_e2e.py`: end-to-end PC against a `CoreRepository` with a real git worktree replaying the same scenario set as T032. **DEFERRED.** Confirmed independent of the `watch:` SDK work (T045/T046/T073): the closure builders do pure auto-detection and never reference `watch`. **REVISED 2026-06 — the original "no helper can make a follow-up commit" premise was wrong; see "Deferred e2e tests — harness findings" below.** The multi-commit/multi-branch fixture this needs already exists (`tests/helpers/file_repo.py::FileRepo` applies `initial__main` + `pr*__<branch>` fixture dirs as branch commits; `MultipleStagesFileRepo` does multi-commit-per-branch), and `TestInfrahubApp` already drives the full PC pipeline in-process against it (no docker) — see `tests/integration/proposed_change/test_proposed_change_repository.py`. The real blocker is that that comparable test is `@pytest.mark.xfail` for undocumented GitHub Actions flakiness, so new tests built the same way risk landing as `xfail` (no CI signal). Realize as a `TestInfrahubApp` + `FileRepo` test, not the SDK `GitRepo`. The placement path above (`integration_docker/`) is likely wrong — prefer `tests/integration/proposed_change/` alongside the existing repo-PC test.
...
- [ ] T049 [US3] Functional test in `backend/tests/functional/proposed_change/test_artifact_regen_watch.py` mirroring quickstart.md Scenario 6: declares `watch.files: [templates/partials/]` on a Jinja2 transform; asserts regeneration on declared-closure-file edit and non-regeneration on unrelated-file edit. **DEFERRED — see "Deferred e2e tests — harness findings" below.** Realize on `TestInfrahubApp` + `FileRepo` with an `initial__main` carrying a `watch:`-declared Jinja2 transform and a `pr*__<branch>` editing a file under the watched directory (and a second editing an unrelated file). The backend integrator import seam this exercises (T047 watch-exclusion from `validate_data_against_schema` / payload) currently has **no automated coverage** — this is the test that would prove a `watch:` transform is not silently dropped on import. Until then, validate that seam manually (see T070).
- [ ] T050 [US3] Integration_docker test in `backend/tests/integration_docker/proposed_change/test_artifact_regen_watch_e2e.py` exercising `watch.files` on a Python transform that imports from a sibling top-level package (US3 acceptance 3). **DEFERRED — see findings below.** Same harness as T049; the Python sibling-package case is a fixture-content variant (transform + sibling helper under a watched directory). Likely belongs in `tests/integration/` on `FileRepo`, not a separate `integration_docker` path.

### Documentation

- [X] T051 [P] [US3] Add the `watch:` schema reference under `docs/docs/reference/infrahub-yml/` per spec.md "Documentation Deliverables": strict object form with the `files:` key, recursive directory matching, note that future keys (`strict:`, `exclude:`) will live under `watch:`, one Python and one Jinja2 worked example.
...
- [X] T066 [P] Add the "Where to find the why trail" section to `docs/docs/topics/proposed-change.mdx` per spec.md Documentation Deliverables: points users at the repository task log as the canonical place to see which file/query/relationship change caused a regeneration; includes one example log line.
...
- [X] T072 [P] Remove the dead `FILE_CHANGES` selection-gate constant from `backend/infrahub/proposed_change/tasks.py` (its only consumers were the gates T013/T014 replaced, then T030/T031 removed the residual `has_file_modifications` clause). Before deleting, run `git grep FILE_CHANGES` to verify no other consumers remain; also remove any helper functions (e.g. `has_file_modifications`) whose only callers are now gone. **No removal performed — premise was incorrect.** `git grep` confirmed both symbols are still live: `DefinitionSelect.FILE_CHANGES` drives the *generator* dispatch gate (`tasks.py:385-386`, separate from the artifact gate), is rendered by `DefinitionSelect.log_line` (`tasks.py:1373`), and is reused as the flag name for the artifact gate's `_transform_changed` clause (`tasks.py:1560`); `ProposedChangeRepository.has_file_modifications` (`message_bus/types.py:153`) is consumed by that same generator dispatch (`tasks.py:386`). T030/T031 only removed the legacy clause from the *artifact* gate; nothing was orphaned.
...
## Deferred e2e tests — harness findings (revised 2026-06)

Applies to the seven deferred end-to-end tests: **T033, T049, T050, T056, T057, T064, T065**.

### Correction to the original deferral rationale

The earlier notes deferred these on the premise that "no test helper can make a follow-up commit on a branch (`infrahub_sdk.testing.repository.GitRepo` does one initial commit + import and stops), so there is no two-commit diff for the gate to react to." **That premise was wrong.** It is true of the *SDK* `GitRepo`, but that is not the helper these flows use. The required infrastructure already exists:

- `backend/tests/helpers/file_repo.py::FileRepo` builds a repo from an `initial__<branch>/` fixture dir and applies each `pr*__<branch>/` dir as a **branch + commit** — i.e. a baseline commit plus a follow-up commit on a branch, which is exactly the two-commit diff the gate consumes.
...
- The fixture convention is already established: see `backend/tests/fixtures/repos/infrahub-test-fixture-01/` (`initial__main` + `pr__0001__branch01` + …).
- `TestInfrahubApp` already drives the **full PC pipeline in-process** (no docker) against a `FileRepo` — see `backend/tests/integration/proposed_change/test_proposed_change_repository.py` (asserts the artifact/generator validators ran and produced output) and `backend/tests/integration/git/test_readonly_repository.py` (read-only repo, `sync_with_git=False`, multi-step branch/commit progression).

So the placement paths on the seven tasks (`tests/functional/…` and `tests/integration_docker/…`) are mostly wrong. Prefer `backend/tests/integration/proposed_change/` on `TestInfrahubApp` + `FileRepo`/`MultipleStagesFileRepo`, alongside the two existing PC-repo tests. The `ArtifactRegenTestBase` extracted into `tests/component/proposed_change/conftest.py` remains useful for fast *selection-only* slices, but the real-git replays ride on `TestInfrahubApp`.

### The actual blocker

`test_proposed_change_repository.py` — the nearest comparable test — is `@pytest.mark.xfail(reason="Works locally but it's failing in GitHub Actions")` with **no documented root cause**. New tests built the same way inherit that flakiness and would land as `xfail` (no CI signal). De-risking that flakiness (almost certainly async/timing around git sync + pipeline completion) is the real gate, not missing fixture machinery. Until it is resolved, the import seam these tests would protect is validated only manually (T070) — see below.

### Highest-value gap right now

The backend **integrator import seam** added in T047 (excluding `watch` from `validate_data_against_schema` and the `InfrahubRepositoryJinja2.payload` path) has **no automated coverage**. A `watch:` block that is not excluded is silently dropped at import. T049 is the test that would prove this; until it exists, confirm manually per T070 that a `watch:`-declared transform imports and its node shows the watched files in `dependencies` with `dependencies_complete=true`.

### Suggested order when this is picked up

1. **T057** first — import-only, no second commit, does not depend on the CI-flakiness fix.
2. Author scenario fixtures (`initial__main` + `pr*`) and write **T033 / T049 / T050 / T056** on `TestInfrahubApp` + `FileRepo` (T056 must null `dependencies` post-import to fake the pre-feature state).
3. **T064 / T065** on the `test_readonly_repository.py` pattern; T065 settles the missing-pinned-commit edge.
4. De-flake `test_proposed_change_repository.py` in parallel — it gates real CI signal for everything in steps 2–3.
