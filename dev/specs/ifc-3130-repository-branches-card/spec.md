...
> **32 requirement statements**: FR-001…FR-028 plus FR-003a, FR-010a, FR-011a and FR-018a. Refer to
...
- **FR-019**: The division between the two cards MUST be derived from each **field's** branch-support declaration in the schema, falling back to the node's own declaration when the field does not declare one. It MUST NOT be a list of field names held in this feature. **`BranchSupportType` has three values, and both `aware` and `local` are branch-scoped**; only `agnostic` is repository-wide. This is not a detail: on `CoreRepository`, `commit` and `sync_status` are both declared `local`, so treating only `aware` as branch-scoped would file the two values this feature exists to disambiguate under repository-wide, making SC-004 actively false. The rule MUST also cover **relationships**, which `ObjectDataDisplay` renders alongside attributes and which would otherwise appear in both cards. _Verify_: unit test with one case per `BranchSupportType` value plus the node-level fallback; component test supplying a schema with an invented attribute and asserting it lands in the card its declared branch support dictates; component test asserting each relationship label appears exactly once on the page.
...
- **FR-025**: Every status value, state and marker on the branches card and on both details cards MUST be identifiable from a text label or an accessible name alone, with colour disregarded. Component tests MUST query by accessible name rather than by class or colour. This extends to three things the card introduces: the **paging controls** (the active page carries `aria-current`, and a page change is announced), the **truncated commit hash** (its accessible name carries the full value, since table cells offer no copy affordance), and the **branch-name caption** on the branch-scoped details card (part of that card's accessible name, not decorative text). _Verify_: component tests locating every chip, state, the default-branch marker, each paging control and the commit hash by accessible name; no test may depend on a colour or a class to identify one. Matches the bar the sibling epic sets for the Commits tab, so two cards on one page are not held to different standards.
...
- **The design's `Showing 5 of 12 · Load more` footer is replaced by page controls.** A "load more" affordance cannot express a position in the URL, so it is incompatible with FR-011. This is **one of six knowing divergences from the canvas**, not the only one — see the divergence register in [plan.md](plan.md), which also covers the two-card split (the canvas explicitly uses one card), the schema-derived column labels, the search toolbar on the read-only card, and the dropped ref-kind pill. All six should be taken to the designer together, on T094 in IFC-3101, **before work unit 1 starts** — pagination is built first and would otherwise be rejected last.
...
- **This work merges into the epic branch, not into `develop`.** The PR targets `cross-branch-repo-status-infp-671`, where IFC-3126 already sits and IFC-3127 will land. (An earlier revision of this spec assumed no pull request would be produced at all; that is superseded.) The practical consequence is that **the preview window's fabricated attribute values reach no user** while this lives on the epic branch — which is why the card carries no preview banner. If the epic branch is ever released with IFC-3127 still outstanding, that decision must be revisited: plausible-looking fake commit hashes with nothing marking them are worse than showing nothing. Recorded as open question Q2 for the epic owner in [plan.md](plan.md).
