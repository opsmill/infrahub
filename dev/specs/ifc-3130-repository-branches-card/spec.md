...
- Q: Default page size and the sizes offered? → A: Default 20, selectable from 10, 20 and 50. **Revised 2026-09-16 to default 10**, after seeing the card against real data: 20 rows made the card dominate the page, and reserving 20 rows of height against a short last page (FR-011b) left far too much dead space.
...
- **FR-010**: The user MUST be able to move between pages using previous/next controls and direct page selection, and MUST be able to change how many rows a page holds — defaulting to 10, selectable from 10, 20 and 50. Rows MUST be replaced page by page, not accumulated. _Verify_: component test asserting a page change and a page-size change each issue a new request with the corresponding window, and that the previous page's rows are no longer rendered after a page change.
...
- **FR-011b**: Moving between pages MUST NOT change the height of the card at the default page size. A last page holding fewer rows than a full one otherwise shortens the card, moving everything below it and taking the window's scrollbar with it — the same class of layout jump FR-023 forbids while loading, and equally disruptive. The table MUST therefore reserve height whenever more than one page exists, and MUST reserve nothing when every row fits on one page, so a short table carries no dead space. The reservation is **the smallest offered page size**, not the selected one: reserving a full 50 rows would leave more dead space than the jump costs, so a user who chooses a larger page knowingly trades stability for density. _Verify_: component test asserting the reserved height for a set larger than one page, and its absence for a set that fits one.
