...
- [X] T016 [P] [US2] Extend `test_schema_diff_constraints.py` to cover a second value-intrinsic family beyond kind — pick one with a `parameters.*` identifier (regex or a length bound) — so the unrestricted-scope guarantee is not pinned on `attribute.kind.update` alone
