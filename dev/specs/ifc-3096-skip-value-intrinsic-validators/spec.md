...
4. **Given** a reviewer reading the pull request, **When** they look for what the classification change actually did, **Then** they find it as an explicit, line-by-line list — the diff of the pinning test's expected mapping — rather than having to infer it from scattered logic, so each entry can be checked against its justification.
...
- Rebase gates the schema-diff producer on the branch's cached schema hash, while merge deliberately recomputes the diff, with a code comment stating this is so a schema change is never missed at merge time. Today's data-path scheduling incidentally masks that asymmetry on rebase. **Resolved during planning — not a blocker.** Research R3 settled it: the asymmetry is real (and wider than stated — Proposed Change has no gate at all), but removing data-path scheduling does not turn it into a correctness gap, because the safety argument turns on the candidate schema rather than the gate. A value-intrinsic violation requires the candidate's guarded property to differ from what the data was validated against at write time, and the candidate takes its value from the source branch, which is the same branch the data was written on. The one residual — the gate fails open when a schema hash is absent — is a startup-ordering edge that today's data-path cover addresses only partially and coincidentally. Recorded as follow-up: align rebase's gate with merge's recomputed check.
