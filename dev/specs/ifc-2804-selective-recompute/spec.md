...
- A deleted transform (**deleted**) tears down the per-attribute automation(s) so nothing keeps firing for a transform that no longer exists.
...
### User Story 5 - Deleting a transform removes its recompute automation (Priority: P2)

An operator deletes a Python transform (or removes it from a repository so it is deleted on import). The per-attribute automation that fired that transform's recompute is torn down, so nothing keeps firing for a transform that no longer exists.

**Why this priority**: A dangling automation for a deleted transform is a resource leak and a source of spurious or failing jobs. Teardown keeps the automation set aligned with the transforms that actually exist.

**Independent Test**: Import a transform feeding a computed attribute (automation created). Delete the transform and import. Verify the recompute automation tied to that transform's attribute(s) no longer exists and no further recompute fires for it.

**Acceptance Scenarios**:

1. **Given** an imported Python transform feeding a computed attribute, with its recompute automation in place, **When** the transform is deleted and the deletion is imported, **Then** the automation(s) that fired recompute for that transform's attribute(s) are removed.
2. **Given** a deleted transform whose automation has been removed, **When** subsequent commits are imported, **Then** no recompute is triggered for the attribute(s) the deleted transform used to feed.
...
- **Transform feeding no computed attribute**: a transform that feeds no computed attribute produces no automation and no recompute; its fingerprint changes are inert for this feature.
...
- **FR-005**: When a transform is **deleted**, the system MUST tear down the per-attribute recompute automation(s) associated with that transform, so no automation survives for a transform that no longer exists.

#### Change signal and scoping

- **FR-006**: The system MUST use the transform's branch-aware `fingerprint` attribute (delivered by IFC-2844) as the change signal; an unchanged fingerprint MUST mean the transform's output cannot have changed and its attributes MUST NOT be recomputed on that account.
- **FR-007**: The update trigger MUST be filtered to the `fingerprint` attribute changing; changes to other, non-output-affecting attributes of the transform MUST NOT trigger a recompute.
- **FR-008**: Recompute triggered by a transform change MUST be scoped to only the attribute(s) that transform feeds; it MUST NOT recompute attributes fed by other transforms (no branch-wide full recompute).
- **FR-009**: The system MUST NOT trigger recompute of Python transform computed attributes from the commit event on a linked repository once this feature is in place (the commit-driven full-sweep trigger for these attributes is removed).

#### Live-edit-only reaction

- **FR-010**: The recompute trigger MUST react only to live edits. It MUST NOT fire for fingerprint changes replayed by a branch merge or rebase, which are handled by their own recompute path.
- **FR-011**: The recompute trigger MUST NOT fire for change events produced by a recompute write itself, so a single change cannot loop into repeated recomputes.

#### Null and no-watch safety (the invariant)

- **FR-012**: A null `fingerprint` MUST be treated as "unknown" and therefore as changed; the transform's attribute(s) MUST be recomputed rather than skipped when the fingerprint is null.
- **FR-013**: The first import of a transform after upgrade MUST stamp its fingerprint (a null-to-value transition), and that transition MUST itself be an update event that recomputes the transform's attribute(s) exactly once (the self-heal path).
- **FR-014**: A transform that declares no `watch` MUST keep per-commit recompute behaviour (its fingerprint folds in the commit id and changes on every commit), but the recompute MUST be scoped to only that transform's own attribute(s).
- **FR-015**: Any fallback, indeterminate, or error path in the trigger or scoping logic MUST default toward recompute, never toward skipping a recompute that may be needed (over-regenerate, never under-regenerate).

#### Scope and parity

- **FR-016**: This feature MUST apply only to Python transform-based (`TRANSFORM_PYTHON`) computed attributes; it MUST NOT change the recompute behaviour of USER-kind or JINJA2-kind computed attributes.
- **FR-017**: The import path MUST stamp fingerprints for read-only and pinned-commit repositories identically to writable repositories, so the create / update / delete triggers behave the same regardless of repository mode.
- **FR-018**: The system MUST NOT recompute on a GraphQL query edited directly through the API (the fingerprint refreshes only at import); this matches today's commit-based behaviour and is a documented, deferred limitation, not addressed by this feature.

#### Rollout

- **FR-019**: The upgrade MUST NOT cause a full branch-wide recompute per commit; the only added cost of adopting fingerprints MUST be one recompute pass per transform at that transform's first post-upgrade import.
- **FR-020**: The one-time first-import recompute cost and the deferred API-query-edit limitation MUST be documented in release notes.
...
- **Recompute automation**: the per-attribute mechanism that fires the recompute for a transform's attribute(s). Created when a transform feeding an attribute is imported; removed when the transform is deleted. Which automations exist per computed attribute is an implementation detail confirmed in planning; the requirement is that none survives a deleted transform.
...
- **SC-007**: Deleting a transform removes its recompute automation 100% of the time; after deletion, subsequent commits produce zero recompute for the attribute(s) the deleted transform fed.
...
- **Each Python transform computed attribute is fed by exactly one transform**, so resolving a changed transform to the attributes it feeds is well defined.
...
- **The set of recompute automations per computed attribute** is an implementation detail to confirm in planning; the requirement is that deleting a transform removes every automation that fired recompute for its attribute(s).
