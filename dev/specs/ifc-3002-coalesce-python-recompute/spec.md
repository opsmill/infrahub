...
- **SC-001**: For a merge changing N nodes tied to Python transform computed attributes, the number of background jobs created per affected attribute-and-kind pair is at most the number of chunks the submission limit requires, instead of one per changed node. **Measured at 100 and 1000 changed nodes**, giving one job and four; 2000 was dropped, because the two points already show the count following the chunk limit rather than the node count and a third adds cost without adding an answer. The shape is also asserted at 20 nodes, on merge and on rebase, in the dispatch test.
- **SC-002**: The recompute a merge or rebase dispatches does not grow with the changed-node count. Measured on `release-1.11` at 20 changed nodes, the current behaviour dispatches 20 transform runs plus 60 reader-resolution runs on a merge, and 20 plus 100 on a rebase. The coalesced pass dispatches about 2 and 0, refreshing the same readers. **Stated as a ratio of dispatched work, not as a duration.** Two earlier attempts at a duration target, 90% and then 75%, failed to survive contact with the hardware: the same metric moved between 62.9 s and 121.0 s across identical runs, so a timing bar is not a criterion, it is a coin toss. Run counts do not vary.
- **SC-003**: For the same merge, the set of nodes written and their final values are identical between the current behaviour and the coalesced behaviour. **Checked at 50 changed nodes**, comparing every owner's stored value across the two modes. Not repeated at the larger scales: the comparison is of one mapping against another and does not change shape with size, while each run needs its own stack because the switch is read when the container starts. Node coverage alone, which is the weaker half of this, is confirmed at 100 and 1000 by the timing runs.
...
- **SC-005 (UNVERIFIED)**: Throughout the background window that follows the merge, the API answers every request without an error and without a timeout. **Nothing in this work checked it.** No request was issued against an instance while a recompute window was draining, at any scale. The expectation is that it holds, since the pass replaces one job per changed node with a bounded few and so lowers the load rather than adding to it, but that is an argument and not a measurement. Treat this criterion as open until someone drives API traffic through the window.
...
- **SC-007 (fail criterion)**: If the transform execution count exceeds the current behaviour, or the dispatched recompute still grows with the changed-node count, the suppression is reverted and the feature is reconsidered rather than shipped. **Gated on counts rather than on the window**, for the reason given under SC-002: a duration bar on this hardware would fire or not fire on noise. A regression that fragmented a merge into roughly twenty times more runs was caught by counts while the window looked acceptable.

The baseline was measured first, as planned. Results and method are in [baseline.md](./baseline.md).
Note that its earliest figures, including the 498.2 s settle time, were taken before a junk-trigger fix landed in `release-1.11`, so part of what they measured has since been addressed elsewhere. The 20-node comparison against today's `release-1.11` is the figure to trust.

## What this does and does not speed up

The work scales with the number of derived values a change affects, not with the number of nodes that read them. That decides whether a given merge gets faster, so it is stated here rather than left to be inferred.

- **Many changed nodes.** The current behaviour starts one recompute per changed node. This is the case the feature exists for, and the one where it collapses the work to a bounded few jobs.
- **Few changed nodes, many readers.** The current behaviour already resolves those readers in one pass, so there is nothing to collapse and both paths do about the same work. A merge of this shape will not get faster, and should not be expected to.

A production comparison on a workload of the second shape measured the two as equal. That is the correct result for that shape. It is not evidence against the feature; it is evidence that the feature is narrower than a plain reading of these criteria suggests.
