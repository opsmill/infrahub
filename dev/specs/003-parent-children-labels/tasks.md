...
- [X] T004 [P] [US1] Route the label through `getRelationshipDisplayLabel` in `frontend/app/src/entities/nodes/object/ui/object-details/object-data-display/object-data-row.tsx` (line ~23). Add `useSchema(fieldSchema.peer)` for the relationship branch (guarded by `"peer" in fieldSchema`) and pass the peer schema in.
