...
- **Same opportunity filed concurrently**: two pull requests that file the same opportunity at the same moment can each create an item, because the tracker offers no atomic create-if-absent. The per-PR concurrency group and the dedup label lookup cover the sequential case; SC-004 measures any duplicate that slips through (FR-011).
