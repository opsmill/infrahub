...
| Field | Derivation |
|---|---|
| `effective_verdict` | Strictest of: every `PackageFinding.verdict`, the overall `verdict`, and `review-required` when any of these holds: the report is malformed; the report names another pull request; the report is missing or stale (it analysed another head) once the analysis run of the head completed or 60 minutes after the head commit's commit date ("analysis did not run", "analysis did not complete"); the analysis run that produced a current report cannot be found, is unfinished 60 minutes after the head commit's commit date ("analysis did not complete"), or completed without success ("analysis run did not succeed"); the lockfile diff adds a package; a changed lockfile could not be read, so the new-package check is unverified ("new packages could not be checked"); any commit is not authored by `dependabot[bot]`; CI is red on the head commit ("CI failed on the head commit") |
