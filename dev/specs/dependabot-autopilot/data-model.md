...
| Field | Derivation |
|---|---|
| `effective_verdict` | Strictest of: every `PackageFinding.verdict`, the overall `verdict`, `review-required` if the lockfile diff adds a package, `review-required` if the report is missing, malformed or stale |
...
Dedup identity: the `dbap-…` label among issues whose status category is not Done.
