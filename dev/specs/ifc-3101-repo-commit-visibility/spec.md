...
- Paging is by page size and offset, matching the existing convention for lists in the product. The PRD considered and rejected a cursor as a bespoke pattern for no gain. The accepted cost is that paging is not stable: commits arriving between two page requests shift the window, so a user paging through a busy repository can see a row twice or miss one. The window moves whenever the answering worker's copy is updated, which is any sync tick, any refs check, or any on-demand check a user triggers, rather than on a fixed interval. What it never does is misreport a commit's state or either marker, since both come from the answer rather than from position. Revisit if paging deeply through an active repository becomes a real workflow rather than a theoretical one.
