...
- **Branch not synchronised with Git.** On a read-write repository the branch is not part of the branch list's row set at all. A mismatch there would legitimately mean the branch needs rebasing, not that an import failed, so the branch list omits its row entirely (FR-006, FR-021). The exclusion is deliberately conservative rather than a restatement of what the sync does: `sync_with_git` gates the **outbound** side only (creating the branch on the remote, merging and deleting it there), and on a default deployment - `git.import_sync_branch_names` empty - the sync imports every remote branch whatever the flag says, so such a branch can genuinely carry an imported commit. The commit view therefore does **not** follow the branch list here: it maps the branch through whenever a sync would import it, via `remote_branch_is_imported`, and reports it as not tracked only when it would not. The two surfaces disagree on exactly this branch. That gap is a defect in the sync rather than in either resolver, and it is tracked in opsmill/infrahub#10684: the decision taken there is that `sync_with_git=False` blocks inbound import in every configuration, which makes the branch list's rule the correct one and brings the commit view into line with it.
...
- **FR-003**: System MUST report, per Infrahub branch, both the commit it has imported and the latest commit the answering worker has seen on the remote branch or tracked ref. The second value is the worker's remote-tracking state, not a live read of the remote: the read path never fetches (FR-007 carries its age), so "latest" means latest known rather than latest that exists. _Verify_: advance a fixture remote without importing; assert both values are returned and differ.
...
- Paging is by page size and offset, matching the existing convention for lists in the product. The PRD considered and rejected a cursor as a bespoke pattern for no gain. The accepted cost is that paging is not stable: commits arriving between two page requests shift the window, so a user paging through a busy repository can see a row twice or miss one. The window moves when new commits actually reach the answering worker's copy, not on any fixed interval and not on every check: a check against an unchanged remote transfers no content (FR-018) and shifts nothing, and neither does a sync tick that found nothing to fetch. What it never does is misreport a commit's state or either marker, since both come from the answer rather than from position. Revisit if paging deeply through an active repository becomes a real workflow rather than a theoretical one.
...
| Authentication / authorization changes | Ruled out as an authorization change: no new permission, and no new authorization rule. It is not "no new surface" - the refs-check mutation is a new GraphQL operation, covered by the schema gate above. It gates on update permission for the concrete kind, exactly as `ReadOnlyRepositoryImportLastCommit` already does, and the two queries reuse the existing repository view permission. |
...
| New graph query for per-branch repository values | Reads existing data and changes no schema, but it is new graph-query surface that the sibling epic extends and the periodic sync later depends on. Reviewed on its own merits, with the per-branch resolution pinned by test. **Carries a deferred consolidation (T101):** PR #10606 adds `RepositoryBranchAttributesQuery` for the same attributes over the same edges, and whichever merges second takes an add/add conflict on the same new file. The consolidation is not a free choice between the two - that query windows the default branch unconditionally, while this one goes through `Branch.get_branches_and_times_to_query_global` and so honours `origin_branch`, which differ on any deployment whose branches do not all fork from the default branch. The surviving query must keep this one's window rules, and `test_branch_that_never_imported_inherits_from_its_origin_not_the_default_branch` fails if it does not. |
