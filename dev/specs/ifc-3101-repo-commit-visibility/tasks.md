...
- [ ] T028 [P] Walk quickstart.md "Phase A" against a running stack (`uv run invoke dev.start`) and record any step whose expectation the implementation contradicts, amending quickstart.md rather than the assertion. **Partly done 2026-09-08.** Step 1 ran clean, and step 3's row expectation is amended in quickstart.md: the drift rows arrive with the per-branch graph query's pull request, not this one. Steps 2, 4 and 5 have not been walked against a live stack — the host already runs an `infrahub` compose project holding the ports a second stack would need. Their assertions are covered against a real database by `backend/tests/component/graphql/queries/test_repository_git_state.py` and `backend/tests/component/graphql/auth/test_repository_git_state_permissions.py`; what those do not exercise is the HTTP GraphQL route and the permission-checker pipeline in front of the resolvers. Walk them before PR 2 raises
