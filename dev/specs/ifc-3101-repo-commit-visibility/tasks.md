...
- [x] T029 [US1] Add `timeout: float | None = None` to `InfrahubMessageBus.rpc` in `backend/infrahub/services/adapters/message_bus/__init__.py` and implement it in `rabbitmq.py` and `nats.py` by wrapping the reply future in `asyncio.timeout`, and in `local.py` by accepting and ignoring it. `None` resolves to the adapter's own injected `BrokerSettings.rpc_timeout`, not a module-global `config.SETTINGS` read, so a bus built with explicit settings honours them; do not reintroduce the global lookup. On expiry, discard the correlation entry in the same `finally` that raises, so an abandoned future cannot accumulate for the life of the process, and make both adapters' reply callbacks return quietly on a correlation id they no longer hold rather than raising. A worker that answers after the bound is now an ordinary event, not an error: without both halves the first timeout leaks a future and the late reply raises inside the callback
- [x] T030 [US1] Add `rpc_timeout: int = 30` (`ge=1`, env `INFRAHUB_BROKER_RPC_TIMEOUT`) to `BrokerSettings` in `backend/infrahub/config.py`
- [x] T031 [US1] Add `WorkerTimeoutError` to `backend/infrahub/exceptions.py` (HTTP 504), the `WORKER_TIMEOUT` entry to `infrahub.errors.catalogue::CATALOGUE` with stability `evolving`, `WorkerTimeoutData(operation, timeout_seconds)` to `backend/infrahub/errors/payloads.py`, and the payload case to `infrahub.graphql.error_formatter::_build_payload`. The message names the routing key only, never a worker identity or a path. Done: `retry_after_seconds` stays, because IFC-3148 names the retry hint in its own acceptance criteria; it was briefly dropped during review on the argument that it only ever carries the same value as `timeout_seconds` with no backoff policy behind it, and restored once the ticket was checked. That argument still stands as future work: give it a distinct figure and set the HTTP `Retry-After` header with it, since the REST envelope drops the typed payload and only GraphQL callers see the hint today. The exception keeps the true `float` bound so its message reports what was actually waited, and the whole-second rounding happens where the published `int` payload is built
...
- [x] T099 [US1] Handle the new timeout on the third existing `rpc` caller, in `RepositoryFinalizer.post_create` (`backend/infrahub/repositories/create_repository.py`; the class this file called `InfrahubRepositoryCreate` is named `RepositoryFinalizer` in the tree): the connectivity check there deletes the repository it just created and raises `ValidationError` when the worker reports failure, and a `WorkerTimeoutError` bypasses that branch entirely, so a slow or unreachable remote now leaves a repository row created, never cloned, and un-retryable behind a 504. Route the timeout into the same delete-and-raise path, with a message naming the timeout rather than a connectivity verdict the worker never gave, and cover it in `backend/tests/component/` with a bus double that never replies, asserting the repository is gone afterwards. This is unlike T032, which was descoped because it fixed a defect this feature does not create: this is a regression introduced by T029/T030 and belongs in the same pull request. Done: the delete runs and the `WorkerTimeoutError` is then re-raised rather than converted to `ValidationError`, so the caller keeps the catalogued 504 and its retry hint instead of a validation failure reporting a connectivity verdict no worker gave. Covered by `backend/tests/component/repositories/test_create_repository.py`, whose never-replying bus is the real RabbitMQ adapter with only its publish and reply plumbing replaced, so the bound, its cleanup and the error are the production ones
- [x] T033 [P] [US1] Create `backend/tests/unit/services/adapters/message_bus/test_rpc_timeout.py`: with a bus adapter that never replies, `rpc` raises `WorkerTimeoutError` within the configured bound. Assert the whole error message with `==`. **Unit, not component, and the path above is amended from the one this file first named**: the never-replying doubles subclass the two adapters but never call `new()`, so no connection is opened and the whole suite passes with no broker, database or container running. The catalogue rendering it also specified now lives with its peers in the parametrised table in `backend/tests/unit/graphql/test_error_formatter.py`, including the `retry_after_seconds` hint this ticket's acceptance criteria require, which gained a completeness check so no future code can be added to the catalogue without a case. Both adapters are covered here, `nats.py` included
- ~~T034~~ **Retired 2026-09-04**, with T032: it tested that fix and travels with it to the new ticket
- [x] T035 [US1] Regenerate and commit `schema/error-catalogue.json`, `frontend/app/src/shared/api/errors/catalogue.generated.ts`, `docs/docs/reference/error-catalogue.mdx` and `docs/docs/reference/configuration.mdx` (`uv run invoke docs.generate`), add the `INFRAHUB_BROKER_RPC_TIMEOUT` mapping to `development/docker-compose.yml` and the root compose env block, and confirm `uv run invoke release.validate-dockercomposeenv` and `uv run invoke docs.validate` pass
- [x] T036 [P] [US1] Add `changelog/+ifc-3101-bounded-worker-rpc.changed.md` covering the bounded wait and the new setting. Not the file endpoint: that fix left this feature with T032. The `+` orphan prefix is the one T027 explains
