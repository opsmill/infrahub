...
| 3 | **Per-branch graph query.** `RepositoryBranchValuesQuery` and the tests pinning its inheritance. Block 2.5 | T021-T024; **T097 closed as superseded and T101 lands after this pull request, not in it** | PR 2. T021/T022 may merge either side of checkpoint 2A; T023/T024 edit files PR 2 creates and cannot precede it |
...
Depends on block 2.1. Split, because only half of it is independent: **T021 and T022** touch only
new files, so they can be built and reviewed in parallel with blocks 2.2 to 2.4 and merged either
side of checkpoint 2A. **T023 and T024 cannot** - they edit the drift
...
**T101 is deliberately not part of this pull request, and T097 is closed as superseded by it.** The
query this block adds has a near-twin on the IFC-3104 side, and the two can only be consolidated
once both have reached `develop`. Nothing here waits for that: see T101 for why deferring is safe
and what the eventual merge has to reconcile.

- [x] T021 Create `backend/infrahub/core/query/repository.py` with `RepositoryBranchValuesQuery` following `dev/knowledge/backend/query-pattern.md`: parameters `repository_id`, an explicit `branch_scopes` list and an explicit `attribute_names` set; one statement that `UNWIND`s the scopes and applies the standard edge-activity predicate per scope with `ORDER BY branch_level DESC, from DESC, status ASC LIMIT 1` plus the active-status check; `get_data()` returns frozen `RepositoryBranchValue` rows carrying `source_branch`. Shape after `core/diff/query/artifact.py` for the multi-branch value read and `core/query/diff.py::DiffCountChanges` for the row-per-branch return and its Python-side backfill. **`BranchScope` carries `windows: tuple[BranchReadWindow, ...]` rather than the `(branch_names, time_base, time_tip)` this task first specified**: a branch reads its origin at the fork point and itself at query time, and flattening the two into one name list applied at one time either breaks branch isolation or admits the branch's own name at the base time, which is not what `Branch.get_query_filter_path` does. `from_branch` now transcribes `get_branches_and_times_to_query_global` entry for entry, so the fork-point rule stays owned by `Branch` and the Cypher is one `any(window IN scope.windows ...)`. The node match and the attribute expansion sit above the `UNWIND` behind `WITH DISTINCT`, so neither repeats per branch. `self.limit` is the exact row bound floored at 1, and `limit`/`offset` are refused outright, because the read has no meaningful page and a falsy limit is read as unpaginated
- [x] T022 Create `backend/tests/component/core/query/test_repository_branch_values.py` pinning the resolution before anything depends on it: a branch with its own `commit` reports it; a branch that never imported reports its origin branch's fork-point value with `source_branch` naming that branch, still does after the default branch imports a newer commit, and reports the newer value after a rebase; a read-only repository reports each branch's own `ref`; and the recorded database query count is identical for a fixture with 5 branches and one with 200, asserted with `backend/tests/helpers/db_query_counter.py` against a pinned positive number. **The count case is 5 and 20 branches under a lowered `query_size_limit`, not 5 and 200 at the default**: at the 5000-row default both counts are 1 whether or not the query carries its own bound, so the assertion passed a read that had lost it, and 200 branches bought nothing over 5 that the lowered limit does not. It also asserts `rows_for` and a pinned value, because the Python-side backfill makes a row count alone true even when the statement returns nothing. Added with them: a branch that resolves no edge at all (a read-only repository, whose `AWARE` attributes the fork point hides - a read-write `commit` is `LOCAL`, so its creation edge is global and visible from every branch however early it forked), the stored null marker reaching a caller as `None`, and the two refusals
- [x] T023 Replace the row source in the `InfrahubRepositoryBranchDrift` resolver with `RepositoryBranchValuesQuery`, so each row carries the `tracked_commit` and `git_ref` its own branch resolves, `remote_head` null, `NOT_TRACKED` where nothing is tracked or inherited, and the column-level unavailable state still set. Leave `infrahub.git.utils::get_repositories_commit_per_branch` untouched. A row that does track something reports `UNAVAILABLE`, which agrees with the column-level state until the worker read path ships. The read-write branch mapping T013 already resolves is extracted as `_resolve_remote_branch` so the per-branch path shares it rather than restating it, and `quickstart.md` step 3 loses the paragraph saying the rows are absent. The repository kind is dispatched on **once**, in `_drift_read`, which returns the row set, the attributes to read and whether the ref is per-branch; three separate dispatches would have let a third kind raise in two places and silently pick the wrong attribute set in the third. The resolver passes `graphql_context.at` rather than the current time, which every other query resolver in the package already does and this one did not: without it a `?at=` request answered as of now
- [x] T097 **Closed as superseded by T101, 2026-09-21.** This asked for a scope conversation before the merge: whether `RepositoryBranchValuesQuery` is the primitive IFC-3104 widens, and whether reading the tracked commit and ref here encroaches on the Branches card's other graph-resolved values. Neither is an open question any more. The other side built `RepositoryBranchAttributesQuery` in PR #10606 before this one opened, so the boundary is no longer a scope negotiation but a concrete add/add merge of two existing queries, which is what T101 is. Holding this pull request for the conversation would serialise it behind an unrelated epic and would not change what either side ships. What T097 was protecting - two teams not carrying duplicate per-branch reads - now lives entirely in T101, including the window rules the merge must not lose. What remains is not a decision but a message: the IFC-3104 owner has to acknowledge T101 as their work too, which T084 records as part of the hand-off. Original wording follows. Confirm the `RepositoryBranchValuesQuery` boundary with the IFC-3104 owner **before this block's pull request lands**, not after: that this single-repository per-branch read is the primitive that epic widens to many repositories with server-side filters, ordering and paging, and that reading the tracked commit and ref here does not encroach on the Branches card's other graph-resolved values. The PRD assigns every graph-resolved per-branch value to that epic, so this is the one scope boundary this feature moves and it needs the other side's agreement rather than a unilateral note. Record the outcome in `spec.md`'s Deviations entry. T084 remains the post-implementation hand-off record. T101 carries what the other side has already built, which is the concrete form this conversation now has to take
- [ ] T101 **Consolidate the two per-branch repository reads, once both have landed on `develop`.** PR #10606 (IFC-3127) adds `backend/infrahub/core/query/repository.py` with `RepositoryBranchAttributesQuery`, resolving the same attributes across the same edges for the same reason; this feature's `RepositoryBranchValuesQuery` is the single-repository case of it, and `source_branch` carries strictly more than its `own_value`. Neither can see the other yet: #10606's base is `cross-branch-repo-status-infp-671`, whose own pull request to `develop` (#10513) still has open children, so waiting for it would serialise this feature behind two unrelated frontend pull requests. Both branches therefore add the same new file and the second to merge takes an add/add conflict. Resolution is mechanical - keep `RepositoryBranchAttributesQuery`, delete `RepositoryBranchValuesQuery` and `BranchScope`, and repoint `_resolve_drift_rows` at it with `repository_ids=[repository.get_id()]` - but it needs the branch-window rules reconciled first, and that reconciliation is **not** a free choice between the two. That query windows the **default** branch on `branched_from` unconditionally; this one goes through `Branch.get_branches_and_times_to_query_global` and so honours `origin_branch` and the deprecated `is_isolated`. They differ on a deployment whose default branch is not named `main`: `INFRAHUB_INITIAL_DEFAULT_BRANCH` is configurable, while `Branch.origin_branch` defaults to the literal `"main"`, so every branch there has an origin that is not the default branch, and a window keyed on the default branch reads the wrong one. On `is_isolated` the two do agree, because `BranchCreator` pops the field and the GraphQL input is deprecated, so no supported path creates a non-isolated branch. So the consolidation keeps the window logic from this query even if it keeps the multi-repository shape of the other, and it needs a test on a non-`main` default branch before either is deleted; `test_branch_that_never_imported_inherits_from_its_origin_not_the_default_branch` pins that property from this side already. Supersedes T097
- [x] T024 Extend `backend/tests/component/graphql/queries/test_repository_git_state.py` with the drift row set: read-write branches without `sync_with_git` are absent, `MERGED` and `DELETING` branches and the global branch are absent, every branch of a read-only repository is present, and a read-only branch with nothing imported or inherited reports `NOT_TRACKED`. `test_drift_answers_the_infrahub_side_fields` is amended with it: it asserted empty edges and now asserts the default branch's row. Added beyond the list: a synced read-write branch mapping to its own remote branch, which is the only case that exercises the per-branch mapping and which `create_branch` hides by defaulting `sync_with_git` to false where the model defaults it true; a `MERGING` branch being **kept**, so the terminal-status boundary is pinned from both sides; the `?at=` answer; and the refusal for a kind with no git state. The expected `condition` is passed to the row helper rather than derived inside it, which was the resolver's own rule restated and so could not fail
...
- [ ] T084 [US3] Record the hand-off on the IFC-3104 epic: the Branches card consumes `InfrahubRepositoryBranchDrift` for its drift column, and one per-branch primitive serves both sides. **This task was written expecting `RepositoryBranchValuesQuery` to be that primitive "rather than writing a second one", which is no longer the situation to record**: the other side built `RepositoryBranchAttributesQuery` in parallel, so the hand-off note says which of the two survived and points at T101 for the consolidation, rather than claiming a single query was agreed up front. Sharpen the Dependencies section of `dev/specs/ifc-3101-repo-commit-visibility/spec.md` if its wording needs it
...
- **Phase 2 (Foundational)**: blocks all three stories. Internally: 2.1 → 2.2 → 2.3 → 2.4 → 2.6, with
  2.5 branching off 2.1. Only part of 2.5 is free of that chain: T021 and T022 may merge either
...
- Block 2.5: T021 and T022 alongside blocks 2.2 to 2.4; T023 and T024 only after 2.3 and 2.4 (T097 superseded by T101, which lands after both sides merge)
