...
Ninety-eight tasks is a file-level count, not an effort-level one: a dozen of them add a single enum,
...
| 2 | **Contract and resolvers.** Enums, value objects, cache keys, reader seam, graphene types, branch-mapping extraction, both resolvers, the mutation, workflow stubs, permission tests, schema and frontend codegen. Blocks 2.1 to 2.4 and 2.6 | T002, T003-T005, T007-T020, T025-T028, T100 | PR 1 |
| 3 | **Per-branch graph query.** `RepositoryBranchValuesQuery` and the tests pinning its inheritance. Block 2.5 | T021-T024, T097 | PR 2. T021/T022/T097 may merge either side of checkpoint 2A; T023/T024 edit files PR 2 creates and cannot precede it |
| 4 | **Bounded RPC.** Shared path: `rpc(timeout=)`, the setting, `WORKER_TIMEOUT`, and the one existing caller whose failure handling the bound breaks. Block 3.1 | T029-T031, T033, T035, T036, T099 | PR 2 |
...
| 8 | **Read-only refs check.** Configuration, the shared body, both callers, component and integration tests. Blocks 4.1 to 4.3 | T061-T071, T098 | PR 2 |
...
- [x] T002 [P] **Done 2026-09-08. All four clean, no pre-existing drift**: `uv run invoke schema.validate-graphqlschema`, `uv run invoke docs.validate`, `uv run invoke release.validate-dockercomposeenv`, and `cd frontend/app && pnpm codegen && git diff --exit-code src/shared/api/graphql/generated` each left the tree unchanged, so every regeneration diff from block 2.6 onward is unambiguously ours. One environment note for anyone picking this up in a fresh worktree: `uv sync --all-groups` alone is not enough. Run `git submodule update --init --recursive` first, and if the editable install was already built against an empty `python_sdk/`, `uv sync` reports everything satisfied while `infrahub_sdk` stays unimportable, because the `.pth` maps only the roots that existed at build time. `uv sync --all-groups --reinstall-package infrahub-server` rebuilds it
...
- [x] T003 [P] Add `RepositoryGitCondition`, `RepositoryCommitState` and `RepositoryGitUnavailableReason` as `InfrahubStringEnum` subclasses in `backend/infrahub/core/constants/__init__.py`, with exactly the members and string values in data-model.md. Constants only, no helper functions in that module
...
- [x] T008 Create `backend/infrahub/git/state/factory.py` with `build_repository_git_state_reader(...) -> RepositoryGitStateReader`, returning `UnavailableRepositoryGitStateReader()` in this phase, following `task_manager/flow_run/service.py::build_prefect_task_service`. This factory is the only place that will ever read a setting or pick an implementation, so block 3.4 changes its body and nothing else. It takes the message bus as a required parameter from the start, unused until T046 and carrying a `noqa: ARG001` until then, so that swap really is one body and no call site. The injection seam for the tests is `config.OVERRIDE.repository_git_state_reader`, checked first in the factory body, alongside the existing `config.OVERRIDE` entries for the message bus, cache and workflow adapters — which is what lets the component tests inject a double with no patching
- [x] T009 [P] Create `backend/tests/helpers/repository_git_state.py` with `RecordingRepositoryGitStateReader` (appends each request to an ordered list and replays queued results) and `FailingRepositoryGitStateReader`, following `StaticBlobResolver` in `backend/tests/unit/git/fingerprint/conftest.py`. The failing double takes the exception to raise as a required constructor argument rather than hard-coding `WorkerTimeoutError`: that exception arrives with T031 in PR 4, and this helper ships in PR 2. T048 and T081 pass `WorkerTimeoutError` once it exists. Not `backend/tests/adapters/`, which mirrors `services/adapters/`. These are the third and fourth implementations of the protocol and the reason the component tests need no patching: the recorded requests are what make "no worker request was made" and "the request carried this branch and this flag" assertable as full frozen-dataclass equality
...
- [x] T100 [P] **Done 2026-09-09.** Approved by Patrick Ogenstad, who owns both this feature and [INFP-670](https://opsmill.atlassian.net/browse/INFP-670), so the two sides of this boundary are the same person and no cross-team hand-off was needed. Recorded in the Deviations entry in `spec.md` and in the corresponding Risks row in `plan.md`, both of which previously only claimed the coordination had happened. The agreement is the extraction as built: all three inputs required, the `default_branch` property's `or registry.default_branch` fallback left in place so this change stays clear of that epic's P1, and `_get_mapped_target_branch` untouched. Original task text follows. Confirm the `git/branch_mapping.py` extraction with the INFP-670 owner **before PR 2 raises**, not after: that lifting the mapping rule out of `InfrahubRepositoryBase._get_mapped_remote_branch` with all three inputs required is the seam that epic's configured-default-branch work builds on, and that leaving the `default_branch` property's `or registry.default_branch` fallback in place keeps this change clear of its P1. This is the second of the two scope boundaries this feature moves, it is an explicit override of the PRD's "without refactoring it", and it lands on a file that epic owns, so it needs the other side's agreement rather than a unilateral note. Record the outcome in `spec.md`'s Deviations entry, which currently only claims the coordination happened. T089 remains the documentation record of the seam
...
- [x] T013 Add the `InfrahubRepositoryCommits` resolver to `backend/infrahub/graphql/queries/repository_git_state.py`: build the reader with the T008 factory, resolve the imported commit from `commit.value` on the request branch, resolve `git_ref` with an exhaustive `match` on the repository kind rather than a predicate chain (the T011 function over the repository's own `default_branch.value` and `registry.default_branch` for the read-write kind, `ref.value` for the read-only kind, with no fallback if either is missing), returning `git_ref: null` with `condition: NOT_TRACKED` when the kind's source value is absent **or when a read-write branch is not synchronised with Git**, since such a branch may share a name with a real remote branch and mapping it through unconditionally would report drift for a branch Infrahub deliberately never imports, which is why the contract makes the field nullable; validate `limit` to 1..100 and `offset` to `>= 0`, gate `pending_count` on selection with `extract_graphql_fields`, then delegate to the reader and map its result onto the graphene types
- [x] T014 Add the `InfrahubRepositoryBranchDrift` resolver to the same module: resolve the row set from `registry.branch` (branches with `sync_with_git` for the read-write kind, every branch for the read-only kind, excluding `MERGED`, `DELETING` and the global branch), delegate to the reader, and return the column-level unavailable state with empty edges until block 2.5 supplies the rows. **Landed without the reader call and without the row set**, which is the half of this task that contradicted T048: with no per-branch tracked commit until T021/T023, a request built from the request branch's values would carry the wrong commit for every other branch, and the bus reader has no `branch_heads` implementation until T079 anyway. So the resolver returns the repository id and its own `UNAVAILABLE / NOT_IMPLEMENTED` column state with no rows, which is exactly the short-circuit T048 relies on. T023 brings the row set in with the query that can populate it, and T079 removes the short-circuit
- [x] T015 [P] Add `ReadOnlyRepositoryCheckRefs` to `backend/infrahub/graphql/mutations/repository.py`, requiring `PermissionAction.UPDATE` on the concrete kind the way `ReadOnlyRepositoryImportLastCommit` does, refusing a non-read-only repository, and submitting `GIT_READ_ONLY_REPOSITORY_CHECK_REFS`. Returns `ok` and `task`. Submits `{"repository_id": ...}` for now: the `GitReadOnlyRepositoryCheckRefs` model is T062, and the tracked refs it carries are resolved by the shared body in block 4.2, so T067 changes the flow signature and this one parameter dict with it. The mutation's published type prints as `ReadOnlyRepositoryCheckRefs`, after the graphene class name the way `ReadOnlyRepositoryImportLastCommit` already does, rather than the `InfrahubReadOnlyRepositoryCheckRefs` the contract sketches; the *field* on `Mutation` carries the `Infrahub` prefix as the contract requires
- [x] T016 [P] Add the three `WorkflowDefinition` constants `GIT_REPOSITORY_WARM_UP`, `GIT_READ_ONLY_REPOSITORIES_CHECK_REFS` (`cron="* * * * *"`, `concurrency_limit=1`, `ConcurrencyLimitStrategy.CANCEL_NEW`) and `GIT_READ_ONLY_REPOSITORY_CHECK_REFS` to `backend/infrahub/workflows/catalogue.py`, with flow bodies in `backend/infrahub/git/tasks.py` that log and return in this phase. The definitions must exist for `backend/tests/unit/workflows/test_catalogue.py` to pass. The stubs take plain scalars (`repository_id`, and nothing for the cron flow) because the Pydantic models they will take are T042 and T062; T042, T066 and T067 replace both the signature and the body. `WORKFLOWS` stays sorted by constant name, which `test_workflows_sorted` enforces
- [x] T017 Register the two query fields and the mutation in `backend/infrahub/graphql/schema.py` (`InfrahubBaseQuery`, `InfrahubBaseMutation`) and export the resolvers from `backend/infrahub/graphql/queries/__init__.py`
...
- [x] T019 [P] Extend `backend/tests/component/graphql/auth/` with the permission cases: a user holding `Core/Repository/view` reads both queries; a user without it is denied with the same error as querying `CoreRepository` directly; the mutation requires update permission. Reload the repository after the denial and assert nothing changed
- [x] T020 [P] Extend `backend/tests/component/graphql/mutations/test_repository.py` with the check-refs mutation using `WorkflowRecorder`: the recorded submission names `GIT_READ_ONLY_REPOSITORY_CHECK_REFS` with the repository id, a read-write repository is refused, and the returned `task.id` is the submitted run's

### Block 2.5 [PR 3]: the per-branch graph read (own pull request, reviewable on its own)

Depends on block 2.1. Split, because only half of it is independent: **T021, T022 and T097** touch
only new files and the sibling epic, so they can be built and reviewed in parallel with blocks 2.2
to 2.4 and merged either side of checkpoint 2A. **T023 and T024 cannot** - they edit the drift
resolver from T014 and the test module from T018, both of which block 2.3 and 2.4 create. If this
block ships as its own pull request, that pull request depends on PR 2; if the query needs to land
first, T023 and T024 travel with PR 2 instead.
...
- [x] T025 Regenerate `schema/schema.graphql` with `uv run invoke schema.generate-graphqlschema` and confirm `uv run invoke schema.validate-graphqlschema` passes. Diff the new SDL against `contracts/repository_git_state.graphql` field by field and assert the schema exposes no `count`, no `author_email` and no `answered_by`. Done: every one of the seven new types matches the contract field for field, and none of the three trimmed names appears on any of them. The one naming difference is the mutation payload type, recorded under T015
- [x] T026 Regenerate the frontend types: `cd frontend/app && pnpm codegen` for `src/shared/api/graphql/generated/{graphql-env.d.ts,graphql-cache.d.ts,types.ts}`, and commit them
- [x] T027 [P] Add a towncrier fragment describing the new commit-log and branch-drift queries and the on-demand refs-check mutation, in user-facing terms. Named `changelog/+ifc-3101-repository-commits-api.added.md`: without the `+` orphan prefix towncrier renders the filename as a GitHub issue link, so a bare `ifc-3101-...` name ships a link to an issue that does not exist. The fragment also states that the git-derived values report as unavailable until the worker read path ships, because that is what this pull request actually gives a user. The same `+` prefix applies to T036, T058, T074 and T083
...
- [ ] T029 [US1] Add `timeout: float | None = None` to `InfrahubMessageBus.rpc` in `backend/infrahub/services/adapters/message_bus/__init__.py` and implement it in `rabbitmq.py` and `nats.py` by wrapping the reply future in `asyncio.timeout`, and in `local.py` by accepting and ignoring it. `None` resolves to `config.SETTINGS.broker.rpc_timeout`. On expiry, discard the correlation entry in the same `finally` that raises, so an abandoned future cannot accumulate for the life of the process, and make both adapters' reply callbacks return quietly on a correlation id they no longer hold rather than raising. A worker that answers after the bound is now an ordinary event, not an error: without both halves the first timeout leaks a future and the late reply raises inside the callback
...
- [ ] T099 [US1] Handle the new timeout on the third existing `rpc` caller, in `InfrahubRepositoryCreate` (`backend/infrahub/repositories/create_repository.py`): the connectivity check there deletes the repository it just created and raises `ValidationError` when the worker reports failure, and a `WorkerTimeoutError` bypasses that branch entirely, so a slow or unreachable remote now leaves a repository row created, never cloned, and un-retryable behind a 504. Route the timeout into the same delete-and-raise path, with a message naming the timeout rather than a connectivity verdict the worker never gave, and cover it in `backend/tests/component/` with a bus double that never replies, asserting the repository is gone afterwards. This is unlike T032, which was descoped because it fixed a defect this feature does not create: this is a regression introduced by T029/T030 and belongs in the same pull request
...
- [ ] T037 [P] [US1] Create `backend/infrahub/git/state/classification.py` with `classify(facts)` and `classify_commit(hash, is_ancestor_of_imported, facts, condition)`: `ORPHANED` first of all, when the imported hash is present but `imported_resolvable` is false, so the ancestry branches are never reached with a hash git cannot resolve; then `IN_SYNC` when head equals imported, `BEHIND` when imported is an ancestor of head, `REWRITTEN` when it is not, `NO_REMOTE` when there is no head, `NOT_TRACKED` when nothing is imported; per-commit precedence in the order data-model.md fixes it: the condition first, so under `REWRITTEN` and `ORPHANED` every commit but the head is `UNRELATED` even when it is still an ancestor of the imported commit and would otherwise read as `HISTORY`; then `IMPORTED` before `HEAD`; then `HEAD`; then `PENDING` / `HISTORY`. The condition-first rule is the one that matters: a rewritten ref usually shares most of its history, so without it those shared ancestors have two valid labels
...
- [ ] T048 [P] [US1] Extend `backend/tests/component/graphql/queries/test_repository_git_state.py` with `FailingRepositoryGitStateReader`: `InfrahubRepositoryCommits` surfaces `WORKER_TIMEOUT` with its retry hint rather than an `UNAVAILABLE` result. Only the commit query: the drift resolver still short-circuits to its own `NOT_IMPLEMENTED` column state at this point and never reaches the reader, so a failing reader proves nothing about it here. The drift query's timeout behaviour is T081, once T079 makes it actually call the reader
...
- [ ] T051 [P] [US1] Create `frontend/app/src/entities/repository/domain/rules/is-git-state-available.ts`, a pure predicate over the condition and unavailable reason that decides whether to keep polling. All three unavailable reasons keep polling, `TIMEOUT` included: it is the drift column's only signal that a worker may answer later, since that column carries no retry hint
...
- [x] T061 [P] [US2] Add `read_only_refs_check_interval_mins: int = 15` (`ge=1`, env `INFRAHUB_GIT_READ_ONLY_REFS_CHECK_INTERVAL_MINS`) to `GitSettings` in `backend/infrahub/config.py`. No disabling value: lengthening the interval is the control, matching `CacheSettings.clean_up_deadlocks_interval_mins`
- [x] T062 [P] [US2] Add `GitReadOnlyRepositoryCheckRefs` and its `TrackedRef` to `backend/infrahub/git/models.py` per data-model.md. **Revised after review**: `TrackedRef` carries no commit at all. It originally held `commit: str | None`, read when the request was built, but the convergence broadcast takes the repository lock long after that, so an import landing in between would have been undone by a broadcast pinned to the older value. The commit is now read through `TrackedCommitReader` with the lock held, and a branch that answers `None` there is left out of the broadcast rather than broadcast with a null pin, which would move every worker onto the latest upstream head. **`RepositoryBranchInfo` in the same file gained `ref`**, populated by `get_repositories_commit_per_branch`, because two Infrahub branches may pin different refs and that function was the only per-branch read available to T066 (`RepositoryBranchValuesQuery` is PR 3 and not merged). Read with `getattr`, since the read-write kind has no `ref` attribute
- [x] T063 [P] [US2] Add the `INFRAHUB_GIT_READ_ONLY_REFS_CHECK_INTERVAL_MINS` mapping to `development/docker-compose.yml` and the root compose env block, regenerate `docs/docs/reference/configuration.mdx` with `uv run invoke docs.generate`, and confirm `uv run invoke release.validate-dockercomposeenv` and `uv run invoke docs.validate` pass. Both validators are `git diff --exit-code` over the regenerated files, so they report the uncommitted change as a failure and pass once it is committed; regenerating produced exactly the one compose line and the one configuration-reference row written by hand, and nothing else
...
- [x] T064 [US2] Implement the per-repository body in `backend/infrahub/git/tasks.py` as an injectable component with a single entry point, in this order: claim the repository by writing this flow's run id to the in-flight key from `git/state/cache_keys.py` with `not_exists=True` and a ceiling derived from the same constant as T066's per-repository timeout plus a margin (a shorter ceiling would expire mid-run and admit a second check), and on a failed claim return the run id already recorded without contacting the remote; resolve the tracked refs and read the local `origin/<ref>` or tag SHA with no lock; validate every ref with `git check-ref-format --allow-onelevel` and refuse on failure; run `git ls-remote origin -- refs/heads/<ref> refs/tags/<ref>` with no lock held and with `GIT_HTTP_LOW_SPEED_LIMIT` and `GIT_HTTP_LOW_SPEED_TIME` in the subprocess environment, then apply the same branch-then-tag precedence used for the local read: fully qualified refs are required because a bare name matches every namespace, so a repository whose tracked name exists as both a branch and a tag returns two lines and the check cannot tell which moved; return if nothing moved; only then take `lock.registry.get(name=<repository_name>, namespace="repository")` around `InfrahubRepositoryBase.fetch()` and the `RefreshGitFetch` broadcast pinned to the tracked commit for each Infrahub branch pinning that ref. Release the in-flight key in a `finally`. Never write `commit` or `ref`, and never call `InfrahubReadOnlyRepository.get_commit_value` or `update_latest_commit`, both of which fetch or write. **Landed in a new package, `backend/infrahub/git/refs_check/`, not in `tasks.py`**: `git/sync.py` is the established home for an injectable git component consumed by a flow (`RepositoryAdder`, `RepositorySyncer`), and the design rule keeps a flow body a thin composition root. It is five modules after `backend/infrahub/api/admission/`, which the design rule names as its worked example: `checker.py` (`ReadOnlyRepositoryRefsChecker`, `RefNameValidator`, `RefsCheckScheduler` - T066's due check, extracted so it is testable without a database), `gateway.py` (the `RepositoryRefsGateway` protocol and its production implementation, the only module importing GitPython, translating every git exception into `RepositoryError` so the decision logic names no library type), `models.py`, `constants.py` and `factory.py`; `__init__.py` is empty and callers import the module they need. `tasks.py` keeps the two flows. Two further deviations: the validator refuses an option-like name in Python before reaching `git check-ref-format`, because that command has no `--` separator to hide a leading `-` behind; and the convergence fetch runs `origin.fetch(..., force=True)` directly rather than `InfrahubRepositoryBase.fetch()`, because git refuses to update an existing tag without it and a moved tag is precisely the case this check exists to notice. Forcing is safe for the reason FR-020 already states. **Post-review fix**: the `ls-remote` pattern list also has to carry `refs/tags/<ref>^{}`. An annotated tag publishes both a tag object and the commit it peels to, under different hashes, and the pattern filter was dropping the peeled line, so the listing returned the tag object while the local read returned the commit. They can never agree, so every check of an annotated tag reported movement, took the lock, force-fetched and broadcast to the whole pool, on every cycle, with no fetch able to converge it. **Post-review fix**: the global branch is excluded from the tracked refs. It is present in `registry.branch`, carries a per-branch value like any other, and names no branch a worker could check out, so it was producing a `RefreshGitFetch` for `-global-`; the drift resolver excludes it for the same reason
- [x] T065 [US2] Add failure handling and observability to the same body: catch `GitCommandError` and the repository errors per repository, record the failure with the repository name and the reason, make the repository due again rather than treating it as checked, write the last-checked key with the current timestamp on success and on failure alike, and emit one structured record per detected movement carrying repository, ref, previous head and new head. **Two corrections against this wording, recorded in T065b and T065e**: the caught type is `GitError`, not just its `GitCommandError` leaf; and the due key is shortened rather than deleted, because deleting it means "retry on the next tick" and the cron runs every minute
- [x] T065b [US2] **Post-review corrections to the failure handling.** Three, all found by the review of block 4.2: the `finally` released the claim *after* the best-effort check-time write, so a cache blip on that write skipped the release, discarded the result the `except` had just built, and replaced the reason a check failed with the reason the stamp failed - the release now goes first and the stamp is guarded and logged. The caught git exception was `GitCommandError`, which is one leaf of the git error tree rather than its root, so a corrupt clone or a worker image with no git binary escaped as a crash; it is now `GitError`. And `ValueError` was caught alongside it, which turned any programming error inside the body into a per-cycle "this remote failed" that retried forever instead of surfacing; it is no longer caught
- [x] T065c [US2] **Post-review: the wall-clock bound moved off the Prefect task and onto the listing.** `timeout_seconds` on the per-repository task cancels through an anyio scope, so a deadline reached inside the convergence makes the lock release itself raise. `repository.<name>` is created with no TTL, and `InfrahubLockRegistry.get` caches a lock by name so the first caller's TTL is the one every later caller in that process gets - meaning the lock cannot simply be given an expiry here without also shortening it for the imports that legitimately hold it longer. So the bound is applied inside the component, around the remote listing only, which is the step that can wait on an unresponsive host and the step where abandoning the work costs nothing. Convergence is left to finish. This also gives the on-demand path the same ceiling, which it had none of while the bound lived on the cycle's task
- [x] T065d [US2] **Post-review: the result and the cycle record say what happened rather than implying it.** `RefsCheckResult` carried a `contacted_remote` boolean that nothing read, plus two independent optionals, so thirteen meaningless combinations were constructible and a repository skipped because another run held its claim was counted and logged as checked. It now carries a `RefsCheckOutcome` of completed / skipped_claimed / failed, validated in `__post_init__` the way the sibling `git/state/models.py` results already validate theirs. `RefsCheckCycleSummary` is frozen and derives every count from its results, so `checked` and `failed` cannot disagree with them and nothing is set by assignment after construction; a repository whose task raised now contributes a real failed result carrying its name and `repr(exc)`, where `record_incomplete()` discarded both and `str(TimeoutError())` printed nothing at all. The cycle also re-raises anything that is not an `Exception`, so cancellation belongs to whoever raised it
- [x] T065e [US2] **Post-review: a failed repository backs off instead of retrying on the next tick.** The failure path deleted the due key, and with a per-minute cron that meant a permanently broken repository took a concurrency slot every sixty seconds at the expense of repositories that could still be checked. `RefsCheckScheduler` now owns both halves of the policy - `select_due` and `retry_soon` - so the rule lives in one place rather than being spelled out in the checker and again in the flow, and the checker takes the scheduler as a collaborator. Also in this pass: `build_check_refs_model` skips a branch on which the repository is not active, since its first import has not completed and there is no local copy to compare against; the gateway does its filesystem work inside `asyncio.to_thread` rather than opening the repository on the event loop and threading only the git call after it; a local copy with no remote raises instead of returning silently, which had it re-detecting and re-broadcasting the same movement forever; and a ref absent from the remote and a branch with nothing imported yet are both recorded rather than passed over in silence
- [x] T065g [US2] **Post-review-bot corrections.** The branch id travelling in the convergence broadcast was `Branch.get_id()`, which is the database element id, where every other sender of that message passes the branch UUID and the recipient creates a worktree under whatever it is given; it is now `str(branch.get_uuid())` on both paths. The claim was released unconditionally in the `finally`, so a check outliving its own lease deleted the claim a later run had since taken, admitting exactly the overlap the claim prevents: the release is now conditional on the stored value still being this run's, and the lease is taken again before convergence, which is the unbounded step. `git_check_ref_format` caught only `GitCommandError`, so a missing git binary escaped the validator as a raw git exception; it catches `GitError`. The retry delay is clamped to the configured interval, since a sub-five-minute interval made a failed repository wait *longer* than a healthy one. And ref validation moved ahead of the listing so a refused ref is no longer counted among the repositories the cycle contacted
- [x] T065f [US2] **Post-review refinements, no behaviour change except where noted.** Types: `RefMovement` refuses a movement whose two heads are equal; the check request is frozen, holds a tuple, and requires at least one ref and a non-empty location; the cycle counts are named `*_count` so they no longer collide with the booleans of the same name on a single result; field docstrings added where a name alone did not carry the meaning. Simplification: the remote listing parses straight to a `dict[str, str]`, dropping a dataclass that existed only to be rebuilt into that dict a line later, and `_tracked_ref_names` is `dict.fromkeys`. Comments: docstrings no longer name their own call sites or the alternative not taken, and the three things a reader could not have known are now stated - why `--allow-onelevel` is load-bearing, why the local read's branch-before-tag order has to match the listing's, and that the low-speed settings bind HTTP only, so an SSH remote is bounded by the caller's ceiling instead. One behaviour change: the mutation now refuses a repository with no ref or no location rather than coercing the absent value with `str()`, which would have sent the literal `"None"` as a ref and reported a clean check for a repository it never looked at catch `GitCommandError` and the repository errors per repository, record the failure with the repository name and the reason, delete the due key so the next tick retries rather than treating the repository as checked, write the last-checked key with the current timestamp on success and on failure alike, and emit one structured record per detected movement carrying repository, ref, previous head and new head
- [x] T066 [US2] Implement `check_read_only_repositories_refs` in `backend/infrahub/git/tasks.py`, replacing the T016 stub: resolve read-only repositories via `get_repositories_commit_per_branch(kind=READONLYREPOSITORY)`, apply the due check by writing the due key with `expires=<interval seconds>, not_exists=True` and skip a repository whose write fails, then run each due repository's body as a Prefect `@task` under this flow run with bounded concurrency and a per-repository timeout. Record checked, moved, failed and duration for the tick, and never fail the flow run because one remote is unreachable. Concurrency is bounded by an `asyncio.Semaphore` around the task calls, taken outside the task so a queued repository does not spend its own budget waiting. **The per-repository timeout is not the task's `timeout_seconds`**: T065c moved it inside the checker, as an `asyncio.timeout` around the remote listing only, and the task decorator carries none - cancelling a Prefect task part-way through releasing the repository lock would leave that lock held for good. The tick gathers with `return_exceptions=True` so a repository that times out is counted as failed and made due again rather than failing the run. **T016's `cron="* * * * *"` was missing from `GIT_READ_ONLY_REPOSITORIES_CHECK_REFS` and is added here**, without which nothing would ever schedule this flow
- [x] T067 [US2] Implement `check_read_only_repository_refs` in `backend/infrahub/git/tasks.py` for the on-demand path: no due check, same shared body. The mutation always returns the run it submitted; a run that loses the claim exits without contacting the remote, which is what makes repeated clicks idempotent. Do not attempt to claim in the resolver so the mutation can return the earlier run's id: the claim belongs to the body both paths share, and a request admitted before the first run claims the repository is a race no resolver-side claim closes without a second key. The flow now takes `GitReadOnlyRepositoryCheckRefs` and the mutation's parameter dict changes with it, as T015 and T016 anticipated. **The mutation builds that model from the request branch alone**: it already holds that branch's `ref`, where covering every branch would be one graph query per branch inside a resolver. It carries no commit - the tracked commit is read under the repository lock at broadcast time (T062) - and it refuses a repository that is not active on the request branch, which is the same eligibility rule the scheduled path applies in `build_check_refs_model`. The scheduled cycle is what covers every branch, and an action offered on one branch's page checking that branch's ref is the reading a user would expect anyway

### Block 4.3 [PR 8]: tests

Depends on block 4.2, except T098, which touches an existing suite this feature does not otherwise
edit and can land at any point in PR 8. Five separate files, parallel.

- [x] T068 [P] [US2] Create `backend/tests/component/git/test_check_refs.py` with recording adapters: a repository not yet due is skipped and one that is due is checked; an unchanged remote produces a refs listing and no fetch and no broadcast; the recorded lock timeline shows the lock taken only around the fetch and the broadcast and never around `ls-remote`; a second trigger while one is in flight performs no remote call and reports the claim it found; a crashed run releases its in-flight key; a repository whose remote is unreachable is recorded with its reason, does not abort the cycle, has its due key deleted, and does not delay a concurrent import of the same repository; the tick record carries checked, moved and failed; `RefreshGitFetch` is broadcast once per Infrahub branch pinning the moved ref with `commit` pinned to the tracked commit rather than the new head; and the due key is written with a TTL taken from the configured interval, so a changed setting spaces the next cycle differently without any schedule rewrite (SC-014). The claim-aware cache double these tests need lives in `backend/tests/adapters/cache.py` as `ClaimAwareCache`, since `MemoryCache` ignores `not_exists` and would let every caller win a claim. The unreachable-remote case asserts the lock timeline is clear rather than racing a real import, which is the same property stated without a timing dependency; T070 runs the concurrent case for real. **Strengthened after review**: the lock-scope test asserted the lock at the fetch but not at the broadcast, because the bus double wrote nothing to the timeline - dedenting the broadcast out of the lock left it green. The double now marks each publish, so both halves of the claim are checked. Added with the block's other corrections: the backoff TTL and that a backed-off repository is passed over by the next selection, a branch with nothing imported being fetched but not broadcast, a git failure that is not a command failure being recorded rather than escaping, a programming error *not* being recorded as a repository failure, a failing check-time write costing neither the claim nor the reason the check failed, an unresponsive remote being abandoned without the repository lock ever being taken, and the four combinations the result type now refuses to construct
- [x] T069 [P] [US2] Add a unit test in `backend/tests/unit/git/` for the ref validation, including a ref starting with `-`, asserting it is refused before any subprocess runs. Add a case for a tracked name that exists as both a branch and a tag on the remote, asserting the branch head is the one compared and that the tag line does not change the outcome. Landed as `backend/tests/unit/git/test_refs_check.py`; the subprocess is a required constructor parameter of the validator, so a recording double proves it was never reached. The ls-remote cases also pin that an annotated tag is compared through its peeled commit, which is what the local read resolves a tag to. **Extended after review** with the two things this file was missing: `build_check_refs_model`, whose every rule was unexercised (the global-branch exclusion was found this way), and `_list_remote_heads` against a real local repository carrying a branch, a lightweight tag and an annotated tag. The latter needs only the git binary, and it is what catches a pattern list that drops the peeled line - the hand-written listing fixtures cannot, because they supply that line themselves
- [x] T070 [P] [US2] Create `backend/tests/integration/git/test_readonly_refs_check.py` against Gogs: advance the tracked ref, force-push it, move a tag, and delete a tag. Assert throughout that `commit` is unchanged and the imported commit's content is still readable on the worker (its worktree is the reachability root that makes forcing tag updates safe), that a deleted upstream tag yields `NO_REMOTE` and no exception, and that `RecordingLockRegistry` shows no overlap with a concurrent import under `repository.<name>`. The lock assertion is made as its stronger and deterministic form: a check that finds nothing moved acquires the repository lock zero times, so there is nothing for a concurrent import to wait on, and a check that finds movement acquires it exactly once. This test is what caught the forced fetch T064 records: without it a moved tag failed the whole check. **Changed after review** to use an annotated tag, which is the shape a release tag usually takes and the one that distinguishes a tag object from the commit it peels to; a lightweight tag publishes one line and hides that distinction. A step asserting that an *unchanged* annotated tag reports no movement was added alongside it
- [x] T071 [P] [US2] Add a handler-level test that receiving `RefreshGitFetch` with a pinned commit updates the local copy without moving the pin (`update_commit_value=False`), which is the convergence half of this story that no two-worker fixture is used for. Added to the existing `backend/tests/component/message_bus/operations/git/test_refresh_git_fetch.py`. The absence of a commit write is observed through a recording SDK requester rather than by reading the graph, and the test asserts the operational-status write is present so the absence is a decision rather than an unexercised path
- [x] T098 [P] [US2] Extend `test_rabbitmq_initial_setup` in `backend/tests/integration/services/adapters/message_bus/test_rabbitmq.py` with the broadcast binding it currently omits: alongside the exclusive `worker-events-{WORKER_IDENTITY}` queue and the `refresh.registry.*` binding it already asserts, assert the `refresh.git.*` binding from the `infrahub.events` topic exchange to that queue. Assert that binding specifically rather than the queue's whole binding set, so a failure names the broadcast that broke. This is the third link of FR-017 and the only one with no test today: T068 pins that the check publishes to the routing key, T071 pins that a recipient converges without moving the pin, and delivery to *every* worker rather than to one of them follows entirely from the topology - one exclusive queue per worker, each bound to `refresh.git.*` on a topic exchange, so no competing-consumer draw is involved. The existing test reads that topology back from the RabbitMQ management API after setup, sending no messages, so this addition has no timing and cannot flake. It is also what the multi-worker deviation rests on, which is why it belongs in the phase that first depends on convergence rather than in the polish phase
...
- [x] T074 [P] [US2] Add `changelog/ifc-3101-readonly-refs-check.added.md` covering the periodic check, the on-demand action and the new interval setting. Named `+ifc-3101-readonly-refs-check.added.md`, with the `+` orphan prefix T027 records. It describes the on-demand *mutation* rather than the "check remote now" action, because the frontend action is T072 and has not landed; the fragment states what shipped
...
- [ ] T079 [US3] Implement `branch_heads` on `BusRepositoryGitStateReader` in `backend/infrahub/git/state/bus_reader.py` and remove the drift resolver's column-level `NOT_IMPLEMENTED` state in `backend/infrahub/graphql/queries/repository_git_state.py`, so rows carry their remote head. This resolver is the one place that catches `WorkerTimeoutError` instead of letting it propagate: it returns the graph-resolved rows with `unavailable{reason: TIMEOUT}` and `remote_head` null, because the rows do not depend on the git read (FR-022, SC-012). The commit resolver keeps letting it propagate as `WORKER_TIMEOUT`, which is where the retry hint lives; the drift column carries none and relies on the consumer's polling
...
- [ ] T081 [P] [US3] Extend `backend/tests/component/graphql/queries/test_repository_git_state.py` with `BusRecorder`: exactly one `git.branch_heads.get` message for a repository with 200 branches, and, with `FailingRepositoryGitStateReader`, that the drift query returns every graph-resolved row with its `branch_name`, `tracked_commit` and `git_ref` intact, `remote_head` null, and `unavailable{reason: TIMEOUT}` on the payload rather than raising. Assert the whole response, so a regression that drops the rows and raises `WORKER_TIMEOUT` instead fails here
...
- [ ] T088 [P] Add user-facing documentation for the commit view and the read-only refs check under `docs/docs/`, decided with the `audit-docs` skill so it lands in the right layer, and covering that commits are read live and never stored, what the two freshness values mean, and that the check never changes what Infrahub runs. No broker-driver caveat: NATS is not in use, settled 2026-09-08 and recorded in `research.md`
...
- Block 2.3: T010, T011, T015, T016 and T100 together, then T012 → T013 → T014 → T017
- Block 2.4: T018, T019, T020 together
- Block 2.5: T021, T022 and T097 alongside blocks 2.2 to 2.4; T023 and T024 only after 2.3 and 2.4
...
- Phase 4: T061, T062, T063 together; T068 to T071 and T098 together; T095 after T072
...
Track A (backend):  T029 → T030 → T031 → T099 ∥ T033 → T035  (bounded RPC PR)
