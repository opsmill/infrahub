...
- [x] T031 [US1] Add `WorkerTimeoutError` to `backend/infrahub/exceptions.py` (HTTP 504), the `WORKER_TIMEOUT` entry to `infrahub.errors.catalogue::CATALOGUE` with stability `evolving`, `WorkerTimeoutData(operation, timeout_seconds)` to `backend/infrahub/errors/payloads.py`, and the payload case to `infrahub.graphql.error_formatter::_build_payload`. The message names the routing key only, never a worker identity or a path. Done: `retry_after_seconds` was dropped on review, because it only ever carried the same value as `timeout_seconds` with no backoff policy behind it and nothing expressing that the two must agree; reinstate it when there is a distinct figure to put in it, and set the HTTP `Retry-After` header with it rather than the payload alone, since the REST envelope drops the typed payload. The exception keeps the true `float` bound so its message reports what was actually waited, and the whole-second rounding happens where the published `int` payload is built
