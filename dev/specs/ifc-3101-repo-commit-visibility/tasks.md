...
| 3 | **Per-branch graph query.** `RepositoryBranchValuesQuery` and the tests pinning its inheritance. Block 2.5 | T021-T024, T097; **T101 lands after this pull request, not in it** | PR 2. T021/T022/T097 may merge either side of checkpoint 2A; T023/T024 edit files PR 2 creates and cannot precede it |
...
**T101 is deliberately not part of this pull request.** The query this block adds has a near-twin on
the IFC-3104 side, and the two can only be consolidated once both have reached `develop`. Nothing
here waits for that: see T101 for why deferring is safe and what the eventual merge has to reconcile.
