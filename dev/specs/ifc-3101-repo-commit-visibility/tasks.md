...
regenerate a committed artefact, or walk a quickstart. Grouped by review boundary the feature is ten
pull requests. Every block heading below carries its `[PR n]` tag; this table is the authoritative
mapping.
...
| 3 | **Per-branch graph query.** `RepositoryBranchValuesQuery` and the tests pinning its inheritance. Block 2.5 | T021-T024 | PR 2, mergeable either side of checkpoint 2A |
| 4 | **Bounded RPC.** Shared path: `rpc(timeout=)`, the setting, `WORKER_TIMEOUT`, the `get_file` fix. Block 3.1 | T029-T036 | PR 2 |
| 5 | **Worker read path.** Classification, the log reader, the commit-log message pair and handler, the warm-up flow, the factory swap. Blocks 3.2 to 3.4 | T037-T048, T092, T093 | PR 4 |
...
**Keep PR 2 whole.** It is the largest at twenty-two tasks, and splitting it lands
`schema/schema.graphql` either without resolvers or without types, which is precisely what Phase 2
exists to avoid. If a reviewer does insist, the clean cut is T003 to T009 (vocabulary, value objects,
reader seam, test helpers, no GraphQL at all) ahead of the rest.
...
- [ ] T006 [P] **(PR 1, not PR 2: documentation only, lands with this directory's first commit ahead of any code in this block)** Update `dev/specs/ifc-3101-repo-commit-visibility/data-model.md`, `contracts/message_bus.md` and the Source Code tree in `plan.md` with what the tasks below actually build: the `git/state/` package and factory instead of a reader inside the resolver module, dataclass return types on the protocol, the shared cache-key module, `git/state/log_reader.py` (T092) as the owner of every git read with both handlers shallow over it, and the remote-branch mapping resolved on the API side from the graph value through one extracted function. Grep the whole spec directory for `GitCommitLogGetResponseData`, `graphql/queries/repository_git_state.py`, `git/commit_log` and `WorkerRepositoryGitStateReader` so no file keeps a superseded shape, and fix the stale test path in the quickstart's Phase A command (`backend/tests/unit/git/test_commit_log.py`, now `backend/tests/unit/git/state/test_classification.py`). Add to `plan.md`'s tree the files it currently omits: `git/state/{cache_keys,factory,log_reader}.py`, `git/branch_mapping.py`, `backend/tests/helpers/repository_git_state.py`, `backend/tests/component/api/`, the ref-validation unit test and the FR-002 component test. Record one deliberate divergence from the PRD: the warm-up trigger is not declared as its own protocol, because the single-flight behaviour is already testable without patching through a recording cache and `WorkflowRecorder` (T044), which is the property that constraint existed to protect. The cache-key table, FR-025 and quickstart step 8 were already corrected on 2026-09-04; check them rather than re-editing them
...
- [ ] T032 [US1] Call `InfrahubResponse.raise_for_status()` in `infrahub.api.file::get_file` in `backend/infrahub/api/file.py`, so an `RPCErrorResponse` surfaces as its catalogued status instead of deserialising into an empty body behind HTTP 200. It belongs in this pull request because this is already the reviewed change to that call path
...
- [ ] T034 [P] [US1] Add a component test in `backend/tests/component/api/` asserting `GET /api/file/{repository_id}/...` answers the catalogued status when the worker replies with an error, rather than 200 with an empty body
...
- [ ] T036 [P] [US1] Add `changelog/ifc-3101-bounded-worker-rpc.changed.md` covering the bounded wait, the new setting and the file endpoint now reporting worker-side errors

### Block 3.2 [PR 5]: classification (pure, no I/O)

Depends only on T003 and T004, so it can start during Phase 2.

- [ ] T037 [P] [US1] Create `backend/infrahub/git/state/classification.py` with `classify(facts)` and `classify_commit(hash, is_ancestor_of_imported, facts, condition)`: `IN_SYNC` when head equals imported, `BEHIND` when imported is an ancestor of head, `REWRITTEN` when it is not, `NO_REMOTE` when there is no head, `NOT_TRACKED` when nothing is imported; per-commit `IMPORTED` before `HEAD`, then `HEAD`, then `PENDING` / `HISTORY` / `UNRELATED`, with every non-head commit `UNRELATED` under `REWRITTEN`
- [ ] T038 [P] [US1] Create `backend/tests/unit/git/state/test_classification.py` with parametrised dataclass cases over `GitStateFacts` and per-commit inputs, no repository and no fixtures: each condition, the precedence when head equals imported, a non-linear history where list position would mislead, and the absence of a pending count under `REWRITTEN` and `NOT_TRACKED`
...
Track A (backend):  T029 → T030 → T031 → T032 → T035        (bounded RPC PR)
...
- The feature writes nothing to the graph: no schema change, no migration, and no write to `commit` or
  `ref` anywhere. Any task that appears to need one is a misreading of the design
