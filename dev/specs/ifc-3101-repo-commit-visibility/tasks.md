...
- [ ] T088 [P] Add user-facing documentation for the commit view and the read-only refs check under `docs/docs/`, decided with the `audit-docs` skill so it lands in the right layer, and covering that commits are read live and never stored, what the two freshness values mean, that the check never changes what Infrahub runs, and that cross-worker convergence is guaranteed on the RabbitMQ broker driver only, because `refresh.git.*` is not among the NATS stream subjects (opsmill/infrahub#10514) and `INFRAHUB_BROKER_DRIVER` publicly accepts `nats`
