...
- Make sure the hidden marker `<!-- AGENT_FIX_COMPLETE -->` appears
  ...
  and the `bug-agent-e2e-proof` job reads it to select its GREEN phase.
- Use `gh pr edit` to update the title and body.

## Step 9: Push

**Push your fix commits to the PR branch LAST** (after the PR body update; for an e2e
reproduction this order is mandatory -- the `bug-agent-e2e-proof` job reads the
