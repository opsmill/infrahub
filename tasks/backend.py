...
# Top-level keys `GET /api/schema` returns that have no counterpart on the write root. They are
# read-only rather than unknown, so a raw response body resubmitted to the load endpoint warns
# instead of failing. Pinned against the API response model by a backend test.
_ROOT_READ_ONLY_FIELDS = frozenset({"main", "namespaces", "profiles", "templates"})


def _family_field_name(attribute: "SchemaAttribute | dict[str, str]") -> str:
    """Name of a family field, declared either as a SchemaAttribute or as a pre-rendered dict."""
    return attribute["name"] if isinstance(attribute, dict) else attribute.name


def _write_sdk_generated_contract(generated: str, read_only_fields: dict[str, frozenset[str]]) -> None:
    entries = ""
    for class_name, fields in sorted(read_only_fields.items()):
        names = ", ".join(f'"{name}"' for name in sorted(fields))
        entries += f'    "{class_name}": frozenset({{{names}}}),\n'
    content = (
        '# Generated by "invoke backend.generate", do not edit directly\n'
    ...
    Path(f"{generated}/contract.py").write_text(content, encoding="utf-8")


@dataclass(frozen=True)
class SdkVariant:
    """One generated module of the SDK schema family (``write.py`` / ``read.py``)."""

    name: str
    """Name of the generated module, without the ``.py`` extension."""

    suffix: str
    """Suffix appended to every class name generated for this variant."""

    minimum: "Visibility"
    """Lowest field visibility a definition needs to appear in this variant."""

    model_config_args: str
    """Arguments rendered into the ``ConfigDict`` of every generated model."""

    with_version: bool
    """Whether the root model requires the ``version`` field."""

    with_extensions: bool
    """Whether the root model carries the schema-extension field."""

    with_computed_field: bool
    """Whether the module imports pydantic's ``computed_field`` to serialize ``kind``."""

    extra_families: "Callable[[str], list[dict[str, Any]]]"
    """Builder, keyed on the suffix, for the model families unique to this variant."""


class SdkSchemaGenerator:
    ...
    The definitions, the jinja environment and the field building blocks shared by every
    render are held as attributes. The backend and jinja2 imports stay inside the methods
    that need them: ``tasks/backend.py`` must import without the backend installed.
    """
    ...
    def __init__(self, context: Context) -> None:
        ...
        env = Environment(loader=FileSystemLoader(f"{REPO_BASE}/backend/templates"), undefined=StrictUndefined)
        self.template = env.get_template("generate_schema_sdk.j2")
        self.enums_template = env.get_template("generate_schema_sdk_enums.j2")
        self.generated = f"{REPO_BASE}/python_sdk/infrahub_sdk/schema/generated"

        self.enums = _sdk_enum_specs()

        self.attribute_schema = attribute_schema
        self.base_node_schema = base_node_schema
        self.relationship_schema = relationship_schema
        self.node_stripped = node_schema.without_duplicates(base_node_schema)
        self.generic_stripped = generic_schema.without_duplicates(base_node_schema)

        self.write_extra = {"update": UpdateSupport.ALLOWED, "visibility": Visibility.WRITE}

        # The attribute sub-blocks (choices, parameters, computed_attribute) are value models of
        # their own. They are emitted as dedicated data models so the write contract is explicit
        # rather than an opaque mapping. The attribute itself is a discriminated union on its kind:
        # each variant narrows kind to the kinds sharing one parameters shape and carries that
        # parameters model, so a given kind only validates against its own parameters.
        # computed_attribute discriminates on its own kind field.
        self.dropdown_choice_fields = [
            self._field("name", "Text", "Name of the choice, must be unique within the dropdown."),
            self._field("description", "Text", "Description of the choice.", optional=True),
            self._field(
                "color",
                "Text",
            ...
            self._field("label", "Text", "Human friendly representation of the choice.", optional=True),
        ]

        self.list_parameters_fields = [
            self._field(
                "regex", "Text", "Regular expression that each list item value must match if defined", optional=True
            ),
        ]
        self.text_parameters_fields = [
            self._field(
                "regex", "Text", "Regular expression that attribute value must match if defined", optional=True
            ),
            self._field("min_length", "Number", "Set a minimum number of characters allowed.", optional=True),
            self._field("max_length", "Number", "Set a maximum number of characters allowed.", optional=True),
        ]
        self.number_parameters_fields = [
            self._field("min_value", "Number", "Set a minimum value allowed.", optional=True),
            self._field("max_value", "Number", "Set a maximum value allowed.", optional=True),
            self._field(
                "excluded_values",
                "Text",
        ...
        self.computed_user_fields = [
            self._computed_kind_field(ComputedAttributeKind.USER.value),
        ]
        self.computed_jinja2_fields = [
            self._computed_kind_field(ComputedAttributeKind.JINJA2.value),
            self._field(
                "jinja2_template", "Text", "Jinja2 template used to compute the value, required when kind is Jinja2."
            ),
        ]
        self.computed_transform_fields = [
            self._computed_kind_field(ComputedAttributeKind.TRANSFORM_PYTHON.value),
            self._field("transform", "Text", "Python transform name or ID, required when kind is TransformPython."),
        ]

        # The attribute is modelled as a discriminated union on kind. Each variant narrows kind to
        # the set of kinds the backend maps to one parameters shape, and carries that parameters
        # model. The kind -> parameters split is derived from the backend mapping so the union stays
        # in step with it; the generic variant absorbs every kind not claimed by a specific one.
        # Order sets the union member order. Second element is the backend parameters class name
        # (SDK class = name + suffix).
        self.attribute_variant_specs: list[tuple[str, str]] = [
        ...
        self.kinds_by_parameters = _attribute_kinds_by_parameters(
            {parameters for _, parameters in self.attribute_variant_specs}
        )

        self.kind_description = next(attr for attr in attribute_schema.attributes if attr.name == "kind").description
        self.parameters_source = next(attr for attr in attribute_schema.attributes if attr.name == "parameters")

    def generate(self) -> None:
        """Render every variant, the shared enums module and the package ``__init__``."""
        Path(self.generated).mkdir(parents=True, exist_ok=True)

        enum_names = self._render_enums()

        for variant in self._variants():
            rendered = self.template.render(
                pre_families=self._pre_families(variant.minimum, variant.suffix),
                aliases=self._aliases(variant.suffix),
                families=self._families(variant.minimum, variant.suffix) + variant.extra_families(variant.suffix),
                model_config_args=variant.model_config_args,
                root=_sdk_root(
                    variant.suffix,
                    variant.with_version,
                    variant.model_config_args,
                    with_extensions=variant.with_extensions,
                ),
                enum_names=enum_names,
                with_computed_field=variant.with_computed_field,
            )
            rendered = rendered.replace("__VARIANT__", variant.suffix)
            Path(f"{self.generated}/{variant.name}.py").write_text(rendered, encoding="utf-8")

        _write_sdk_generated_contract(self.generated, self._read_only_fields())
    ...
    def _variants() -> list[SdkVariant]:
        ...
        model_config_args = 'extra="ignore", use_enum_values=True'
        ...
        return [
            SdkVariant(
                name="write",
                suffix="Write",
                minimum=Visibility.WRITE,
                model_config_args=model_config_args,
                with_version=True,
                with_extensions=True,
                with_computed_field=False,
                extra_families=_sdk_extension_families,
            ),
            SdkVariant(
                name="read",
                suffix="Read",
                minimum=Visibility.READ,
                model_config_args=model_config_args,
                with_version=False,
                with_extensions=False,
                with_computed_field=True,
                extra_families=_sdk_profile_template_families,
            ),
        ]

    def _render_enums(self) -> list[str]:
        """Render the self-contained enums module and return the sorted enum class names."""
        enums_rendered = self.enums_template.render(
            enums=[
                {"name": name, "members": [(member, repr(value)) for member, value in members]}
                for name, members in self.enums.enum_specs.items()
            ]
        )
        Path(f"{self.generated}/enums.py").write_text(enums_rendered, encoding="utf-8")
        return sorted(self.enums.enum_specs)
    ...
    def _computed_kind_field(self, value: str) -> dict[str, str]:
        return _sdk_kind_field(
            self.computed_kind_description,
            "ComputedAttributeKind",
            [value],
            self.enums.computed_kind_value_to_member,
        )
    ...
    def _parameters_field(self, parameters_name: str) -> dict[str, str]:
        return {
            "name": "parameters",
            "external_type_annotation": f"{parameters_name}__VARIANT__ | None",
            "external_default_definition": self.parameters_source.external_default_definition,
            "description": self.parameters_source.description,
            "external_pattern": self.parameters_source.external_pattern,
            "min": self.parameters_source.min,
            "max": self.parameters_source.max,
        }

    def _attribute_variant_families(self, minimum: "Visibility", suffix: str) -> list[dict[str, Any]]:
        base_name = f"AttributeSchemaBase{suffix}"
        base_fields = [
            attribute for attribute in self._visible(self.attribute_schema, minimum) if attribute.name != "parameters"
        ]
        families: list[dict[str, Any]] = [{"class_name": base_name, "parent": "BaseModel", "attributes": base_fields}]
        for variant, parameters_name in self.attribute_variant_specs:
            kind_field = _sdk_kind_field(
                self.kind_description,
                "AttributeKind",
                self.kinds_by_parameters[parameters_name],
                self.enums.attribute_kind_value_to_member,
            )
            families.append(
                {
                    "class_name": f"{variant}{suffix}",
                    "parent": base_name,
                    "attributes": [kind_field, self._parameters_field(parameters_name)],
                }
            )
        return families

    def _pre_families(self, minimum: "Visibility", suffix: str) -> list[dict[str, Any]]:
        base = f"AttributeParameters{suffix}"
        return [
            {"class_name": base, "parent": "BaseModel", "attributes": []},
            {
                "class_name": f"ListAttributeParameters{suffix}",
                "parent": base,
                "attributes": self.list_parameters_fields,
            },
            {
                "class_name": f"TextAttributeParameters{suffix}",
                "parent": base,
                "attributes": self.text_parameters_fields,
            },
            {
            ...
            },
            {
                ...
                "parent": "BaseModel",
                "attributes": self.dropdown_choice_fields,
            },
            {
                "class_name": f"ComputedAttributeUser{suffix}",
                "parent": "BaseModel",
                "attributes": self.computed_user_fields,
            },
            {
                "class_name": f"ComputedAttributeJinja2{suffix}",
                "parent": "BaseModel",
                "attributes": self.computed_jinja2_fields,
            },
            {
                "class_name": f"ComputedAttributeTransformPython{suffix}",
                "parent": "BaseModel",
                "attributes": self.computed_transform_fields,
            },
            *self._attribute_variant_families(minimum, suffix),
        ]

    def _aliases(self, suffix: str) -> list[str]:
        variant_members = "".join(f"        {variant}{suffix},\n" for variant, _ in self.attribute_variant_specs)
        return [
            (
    ...
    def _families(self, minimum: "Visibility", suffix: str) -> list[dict[str, Any]]:
        return [
            {
                "class_name": f"RelationshipSchema{suffix}",
                "parent": "BaseModel",
                "attributes": self._visible(self.relationship_schema, minimum),
            },
            self._base_node_family(minimum, suffix),
            {
                "class_name": f"NodeSchema{suffix}",
                "parent": f"BaseNodeSchema{suffix}",
                "attributes": self._visible(self.node_stripped, minimum),
            },
            {
                "class_name": f"GenericSchema{suffix}",
                "parent": f"BaseNodeSchema{suffix}",
                "attributes": self._visible(self.generic_stripped, minimum),
            },
        ]

    def _families_for_contract(self, minimum: "Visibility") -> list[dict[str, Any]]:
        """Every generated write family, at the given field visibility."""
        suffix = "Write"
        return self._pre_families(minimum, suffix) + self._families(minimum, suffix) + _sdk_extension_families(suffix)

    @staticmethod
    def _declared_names(families: list[dict[str, Any]]) -> dict[str, set[str]]:
        """Field names each family declares itself, excluding what it inherits."""
        names: dict[str, set[str]] = {}
        for family in families:
            fields = {_family_field_name(attribute) for attribute in family["attributes"]}
            # A computed field is part of the payload only for the variant that serializes it.
            fields |= {computed["name"] for computed in family.get("computed_fields", []) if computed["serialize"]}
            names[family["class_name"]] = fields
        return names

    @staticmethod
    def _inherited_names(families: list[dict[str, Any]], declared: dict[str, set[str]]) -> dict[str, set[str]]:
        """Field names each family accepts, following its parent chain."""
        parents = {family["class_name"]: family["parent"] for family in families}
        resolved: dict[str, set[str]] = {}
        for class_name in declared:
            names: set[str] = set()
            current: str | None = class_name
            while current in declared:
                names |= declared[current]
                current = parents.get(current)
            resolved[class_name] = names
        return resolved

    def _value_model_read_only_fields(self, write_names: dict[str, set[str]]) -> dict[str, set[str]]:
        """Non-settable fields of the value models, taken from their internal counterparts.

        The parameters, choice, computed-attribute and extension models are declared in this
        generator rather than derived from internal.py, so they omit the bookkeeping fields every
        internal schema model carries (``id``, ``state``) and each computed-attribute variant omits
        the fields of its siblings. All of those names appear in a schema dumped from the internal
        models, so they are tolerated with a warning rather than reported as unknown.
        """
        ...
        for variant in ("User", "Jinja2", "TransformPython"):
            internal_counterparts[f"ComputedAttribute{variant}{suffix}"] = ComputedAttribute
        for _, parameters_name in self.attribute_variant_specs:
            internal_counterparts[f"{parameters_name}{suffix}"] = getattr(parameters_module, parameters_name)

        return {
            class_name: set(internal.model_fields) - write_names.get(class_name, set())
            for class_name, internal in internal_counterparts.items()
        }

    def _read_only_fields(self) -> dict[str, frozenset[str]]:
        """Field names a generated write class does not accept but a submitted schema may carry.

        Two sources feed the table: a field the read variant of a class declares and the write
        ...
        write_families = self._families_for_contract(Visibility.WRITE)
    ...
    def _base_node_family(self, minimum: "Visibility", suffix: str) -> dict[str, Any]:
        ...
        attributes = [attribute for attribute in self.base_node_schema.attributes if attribute.visibility >= minimum]
        family: dict[str, Any] = {
...
def _generate_schemas_sdk(context: Context) -> None:
    """Render the user-facing write/read schema models into the Python SDK."""
    SdkSchemaGenerator(context=context).generate()
