...
class TestTutorial4GitIntegration:
    async def test_generate_the_configuration_of_a_device(
        ...
        await admin_page.get_by_test_id("sidebar").get_by_role("button", name="Device Management").click()
        await admin_page.get_by_role("menuitem", name="Device", exact=True).click()
        await expect(admin_page.get_by_text("Generic Device object")).to_be_visible()
        ...
        await admin_page.get_by_label("Description").fill("New description in the branch")
        await save_screenshot_for_docs(admin_page, "tutorial_6_interface_update")
        await admin_page.get_by_role("button", name="Save").click()
        await expect(admin_page.locator("#alert-success-updated")).to_contain_text("InterfaceL3 updated")
