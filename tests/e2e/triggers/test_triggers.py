...
class TestNodeTrigger:
    ...
    async def test_should_create_a_node_trigger(self, admin_page: Page, branch: str) -> None:
        ...
        await admin_page.get_by_role("option", name="Group Action Core").first.click()
        ...
        # The nested "+ Add new ..." forms now open as stacked Sheet dialogs (aria-label
        # "sheet 2", "sheet 3"); scope each step to its sheet so the duplicated Name/Save
        # controls resolve unambiguously.
        sheet2 = admin_page.get_by_role("dialog", name="sheet 2")
        await sheet2.get_by_label("Name *").first.fill("test group action")
        await sheet2.get_by_test_id("new-object-form").get_by_role("combobox", name="Kind").click()
        await sheet2.get_by_role("option", name="Standard Group Core").click()
        await sheet2.get_by_role("combobox", name="Standard Group *").click()
        await sheet2.get_by_role("button", name="+ Add new Standard Group").click()
        sheet3 = admin_page.get_by_role("dialog", name="sheet 3")
        await sheet3.get_by_label("Name *").fill("test standard group")
        await sheet3.get_by_role("button", name="Save").click()
        await expect(admin_page.get_by_text("StandardGroup created")).to_be_visible()
        await sheet2.get_by_role("button", name="Save").last.click()
        await expect(admin_page.get_by_text("GroupAction created")).to_be_visible()
        await admin_page.get_by_role("button", name="Save").last.click()
