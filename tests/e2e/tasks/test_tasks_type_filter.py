...
class TestTasksTypeFilter:
    async def test_selecting_system_reveals_internal_runs_and_narrows_the_count(
    ...
    async def test_the_type_filter_survives_a_reload_through_the_url(
        self, admin_page: Page
    ) -> None:
        await admin_page.goto("/tasks")
        await expect(admin_page.get_by_role("heading", name="Task Overview")).to_be_visible()
    ...
    async def test_internal_runs_leave_the_branch_and_related_node_cells_empty(
        ...
        internal_row = admin_page.get_by_role("row").filter(
            has=admin_page.get_by_role("cell", name=SYSTEM_LABEL)
        )
