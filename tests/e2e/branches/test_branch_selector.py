...
class TestBranchSelectorNotLoggedIn:
    async def test_cannot_create_a_branch_if_not_logged_in(self, page: Page) -> None:
        ...
        await expect(page.get_by_test_id("create-branch-button")).to_be_disabled()

        # to go branch list view
        await page.get_by_role("link", name="View all branches").click()
        await expect(page.get_by_test_id("branches-table")).to_contain_text("main")
    ...
    async def test_no_quick_create_for_non_existent_branch(self, page: Page) -> None:
        ...
        await page.get_by_test_id("branch-search-input").fill(non_existent_branch_name)

        await expect(page.get_by_text("No branch found")).to_be_visible()
    ...
    async def test_search_and_switch_branch(self, page: Page, data_scenario_branches: ScenarioBranchesHandle) -> None:
        await page.goto("/")
        await expect(page.get_by_test_id("branch-selector-trigger")).to_contain_text("main")
        await page.get_by_test_id("branch-selector-trigger").click()
        await expect(page.get_by_test_id("branch-list").get_by_role("option", name="main default")).to_be_visible()

        await page.get_by_test_id("branch-search-input").fill("atl1")
        ...
        await expect(branch_list.get_by_role("option")).to_have_count(1)
        await branch_list.get_by_role("option", name="atl1-delete-upstream").click()
        await expect(page.get_by_test_id("branch-selector-trigger")).to_contain_text("atl1-delete-upstream")
...
class TestBranchSelectorLoggedInAsAdmin:
    async def test_create_a_branch_with_a_name_that_does_not_exist(self, admin_page: Page) -> None:
        ...
        await admin_page.get_by_test_id("branch-search-input").fill("quick-branch-form")
