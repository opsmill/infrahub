...
"Site2" are the two kept sites, atl1 and den1; the children relationship
surfaces the peer kind label "Site" instead of the generic "Children").
"""
...
class TestObjectHierarchyNavigation:
    ...
    async def test_should_display_correctly(self, admin_page: Page, branch: str) -> None:
        ...
        await expect(admin_page.get_by_test_id("object-items")).to_contain_text("Continent")
        await expect(admin_page.get_by_test_id("object-items")).to_contain_text("Country")
        ...
        await expect(object_hierarchy_tree.get_by_text("Canada")).to_be_visible()
        await expect(object_hierarchy_tree.get_by_text("United States of America")).to_be_visible()
        await expect(admin_page.get_by_test_id("object-items")).to_be_visible()
        ...
        await expect(admin_page.get_by_text("Site2")).to_be_visible()
        ...
        await object_hierarchy_tree.get_by_text("North America").click()
