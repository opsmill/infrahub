...
class TestFileUploadCircuitContract:
    ...
    def test_should_successfully_upload_a_file(
        ...
        expect(admin_page.get_by_text("Drag and drop a file here, or click to select")).to_be_visible()
        ...
        expect(admin_page.get_by_text(re.compile(r"\d+\s*(B|KB|MB)"))).to_be_visible()  # File size

        # fill required fields
        fill_circuit_contract_fields(admin_page, contract_number=CONTRACT_UPLOAD)
    ...
    def test_should_validate_required_file_field(
        ...
        expect(admin_page.get_by_text(re.compile(r"required", re.IGNORECASE)).first).to_be_visible()
    ...
    def test_should_update_existing_file(
        ...
        admin_page.get_by_role("link", name=CONTRACT_UPDATE).click()
    ...
    def test_should_handle_different_file_types(
        ...
        for test_file in test_files:
            ...
            expect(admin_page.get_by_text(test_file["name"])).to_be_visible()

            fill_circuit_contract_fields(admin_page, contract_number=f"CONTRACT-{test_file['name']}")
    ...
    def test_should_not_be_able_to_edit_existing_file(
        ...
        read_only_page.get_by_role("link", name=CONTRACT_UPLOAD).click()

        # verify edit button is disabled
        expect(read_only_page.get_by_test_id("edit-button")).to_be_disabled()
