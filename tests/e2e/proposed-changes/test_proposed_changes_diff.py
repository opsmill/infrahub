...
class TestProposedChangesDiff:
    async def test_should_verify_the_diff_data_with_conflicts(self, admin_page: Page, demo_edge_repo: None) -> None:
        ...
        await admin_page.get_by_role("option", name="den1-maintenance-conflict").click()
        await admin_page.get_by_label("Name *").fill("conflict-test")
        await admin_page.get_by_label("Reviewers").click()
        await admin_page.get_by_role("option", name="Admin").click()
        await admin_page.get_by_label("Reviewers").click()
        await admin_page.get_by_role("button", name="Open", exact=True).click()
        ...
        # check diff data
        await expect(admin_page.get_by_text("Infra ›Interface L3")).to_be_visible()
        await expect(admin_page.get_by_role("link", name="Ethernet1")).to_be_visible()
        await expect(admin_page.get_by_text("Infra ›Deviceden1-edge1")).to_be_visible()
        await expect(admin_page.get_by_role("link", name="den1-edge1")).to_be_visible()
        await save_screenshot_for_docs(admin_page, "topics/proposed_change/pc_tab_data")
        await admin_page.get_by_text("Infra ›Interface L3").click()
        await admin_page.get_by_label("diff tree").get_by_text("den1-edge1").click()
        await admin_page.get_by_text(
            "main den1-maintenance-conflictstatusConflictactiveprovisioningmaintenanceChoose"
        ).click()
        hash_value = await admin_page.evaluate("() => window.location.hash")
        highlighted_node_diff = admin_page.locator(f"id={hash_value[1:]}")
        await expect(highlighted_node_diff).to_be_in_viewport()
        await expect(highlighted_node_diff).to_contain_class("ring-2 ring-custom-blue-500")
    ...
    async def test_should_comment_a_proposed_changes(self, admin_page: Page, demo_edge_repo: None) -> None:
        ...
        await admin_page.goto("/proposed-changes")
        await admin_page.get_by_role("link", name="conflict-test").click()
        await expect(admin_page.get_by_role("heading", name="conflict-test")).to_be_visible()
        await admin_page.get_by_text("Data").click()
        await expect(admin_page.get_by_role("button", name="Refresh diff")).to_be_visible()

        # comment proposed changes
        await admin_page.locator("span").filter(has_text="Infra ›Deviceden1-edge1").hover()
        await (
            admin_page.locator("span")
            .filter(has_text="Infra ›Deviceden1-edge1")
            .get_by_test_id("data-diff-add-comment")
            .click()
        )
        await expect(admin_page.get_by_text("Add a comment")).to_be_visible()
        await admin_page.get_by_role("textbox").click()
        await admin_page.get_by_role("textbox").fill("Some comment ")
        await admin_page.get_by_role("button", name="Comment", exact=True).click()
        await expect(admin_page.get_by_test_id("comment").get_by_text("AAdmin")).to_be_visible()
    ...
    async def test_should_delete_the_proposed_change(self, admin_page: Page, demo_edge_repo: None) -> None:
        await admin_page.goto("/proposed-changes")
        await admin_page.get_by_test_id("actions-row-button-conflict-test").click()
        ...
        await expect(admin_page.get_by_text("Proposed changes conflict-test deleted")).to_be_visible()
