...
class TestTemplateWithNumberPool:
    ...
    async def test_create_a_patch_panel_template_with_number_pool(self, admin_page: Page, template_branch: str) -> None:
        ...
        # The badge is now the sole indication of provenance: the value input lives in the other
        # tab, which is unmounted while the pool tab is showing, so it cannot be asserted here.
    ...
    async def test_create_a_patch_panel_from_template_with_number_pool(
        ...
        # A template-inherited pool value shows its provenance as a badge; the old
        # "Allocated by pool" stand-in inside the value input no longer exists.
        await expect(admin_page.get_by_test_id("source-pool-badge")).to_be_visible()
