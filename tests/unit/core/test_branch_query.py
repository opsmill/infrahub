...
async def test_DiffRelationshipQuery(session, base_dataset_02):
    ...
    query = await DiffRelationshipQuery(branch=branch1).execute(session=session)
    ...
    query = await DiffRelationshipQuery(branch=branch1, diff_from=Timestamp(base_dataset_02["time0"])).execute(
        session=session
    )
...
async def test_DiffRelationshipPropertyQuery(session, base_dataset_02):
    ...
    query = await DiffRelationshipPropertyQuery(branch=branch1).execute(session=session)
    assert query.diff_from == Timestamp(base_dataset_02["time_m40"])
    assert query.diff_to > Timestamp(base_dataset_02["time0"])
    ...
    query = await DiffRelationshipPropertyQuery(branch=branch1, diff_from=Timestamp(base_dataset_02["time0"])).execute(
        session=session
    )
    assert query.diff_from == Timestamp(base_dataset_02["time0"])
    assert query.diff_to > Timestamp(base_dataset_02["time0"])
    ...
    query = await DiffRelationshipPropertyQuery(
