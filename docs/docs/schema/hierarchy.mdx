...
- [Groups](../groups/) can be organized in a hierarchy by default which makes it possible to query the members of all sub-groups at once.
...
As an example, the GraphQL query below will query the members of all [groups](../groups/) that are defined as children of `Group1`.
...
- [Groups](../groups/) — Groups use hierarchy to enable nested member queries
