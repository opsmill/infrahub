...
For more detailed explanations on how these events are used within Infrahub, see the [event system](../events/overview.mdx) documentation.
...
| **branch_name** | The Infrahub branch the file request is scoped to | string | None |
...
| **default_branch** | The branch that must exist on the remote; when unset, only connectivity is checked | N/A | None |
<!-- vale on -->
...
<!-- vale off -->
#### Event refresh.settings.response_delay
<!-- vale on -->

**Description**: Broadcast to update the API response delay on every running API worker process.
...
|-----|-------------|------|---------------|
| **meta** | Meta properties for the message | N/A | None |
| **response_delay** | Delay in seconds to add to each API request | integer | None |
<!-- vale on -->
