...
| `object:*:Generic:view:allow_all`             | `*Generic`    | `view`  | `allow_all` | Allows viewing all objects with 'Generic' in their type (e.g., LocationGeneric, DeviceGeneric) |
...
- **Object Type**: The type of object the permission applies to (e.g., `Tag`, `Device`):
  - Can include namespace prefixes (e.g., `Builtin:Tag`)
  - Supports wildcards (`*`) to apply to multiple object types
  - Can use partial matching with wildcards (e.g., `*Generic`)
