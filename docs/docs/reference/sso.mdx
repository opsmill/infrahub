...
| Group memberships | `groups` | `groups` | Read from the `groups` claim by default; override per provider with `groups_claim` when your provider emits memberships under a different key (e.g., `roles`). Applied on every login, additively — memberships are never removed automatically. See [group mapping](../deploy-manage/user-management/sso/advanced-sso.mdx#group-mapping). |
