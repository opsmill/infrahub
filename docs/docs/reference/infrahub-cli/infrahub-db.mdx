...
* `snapshot-dump`: Bootstrap an empty database...
* `snapshot-restore`: Restore a portable graph snapshot into an...
...
```console
$ infrahub db snapshot-dump [OPTIONS] OUTPUT
```

**Arguments**:

* `OUTPUT`: Path to write the snapshot to (.json.gz).  [required]
...
```console
$ infrahub db snapshot-restore [OPTIONS] SNAPSHOT_PATH
```

**Arguments**:

* `SNAPSHOT_PATH`: Path to a snapshot (.json.gz) to restore.  [required]
