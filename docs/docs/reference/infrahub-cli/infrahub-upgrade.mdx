...
* `--rebase-branches / --no-rebase-branches`: In step 6, automatically rebase every branch that needs it. Without this flag, the upgrade reports which branches need rebase and stops; rebase them manually later or re-run with this flag.  [default: no-rebase-branches]
