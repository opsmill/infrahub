...
| [**default_filter**](#default_filter) | Attribute | Default filter used to search for a node in addition to its ID. Use an empty string to remove an existing value. (deprecated: please use human_friendly_id instead) | False |
...
| **Name** | default_filter |
| **Kind** | `Text` |
| **Description** | Default filter used to search for a node in addition to its ID. Use an empty string to remove an existing value. (deprecated: please use human_friendly_id instead) |
| **Optional**  | True |
| **Default Value** |  |
