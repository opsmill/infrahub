...
| `shortest_paths_only` | Boolean | No | true | — | When true, return only the shortest path through each intermediate object. When false, return all loopless paths up to `max_paths`. |
...
| `truncated_at_depth` | Int | `null` when the search completed. Otherwise the depth at which it ran out of budget — the returned paths are complete only for depths below this value, and deeper paths may exist. |
...
Each end is named from the direction the edge is stored with, so both ends are right for every
hierarchy shape, including a hierarchy that leaves its `parent` or `children` peer on the generic.
...
| Query timed out | Search space too large | `InfrahubReachableNodes` aborts and returns an error. `InfrahubPathTraversal` instead returns the paths found so far with `truncated_at_depth` set, rather than failing. In both cases, reduce `max_depth` or lower `max_paths`/`max_results`. To narrow the search space further, use kind/namespace filters in `InfrahubPathTraversal`, or fewer `target_kinds` in `InfrahubReachableNodes` (which does not accept kind/namespace filters). |
