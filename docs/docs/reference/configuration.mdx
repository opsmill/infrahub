...
| Name | Description | Type | Default |
|------|-------------|------|---------|
| `INFRAHUB_POLICY_REQUIRED_PROPOSED_CHANGE_APPROVALS` | Number of approvals required for proposed changes. | integer | 0 |
| `INFRAHUB_POLICY_REVOKE_PROPOSED_CHANGE_APPROVALS` | Boolean indicating whether performing changes on a proposed change branch should revoke existing approvals. | boolean | False |
