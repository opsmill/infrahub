...
| `INFRAHUB_TELEMETRY_DEV_INTERVAL_MINUTES` | Override telemetry interval for development (minutes). When set, telemetry runs at this interval instead of daily. | None | None |
| `INFRAHUB_TELEMETRY_STORAGE_PATH` | Path for local telemetry storage | string | /var/lib/infrahub/telemetry |
| `INFRAHUB_TELEMETRY_STORAGE_RETENTION_DAYS` | Number of days to retain local telemetry files | integer | 90 |
| `INFRAHUB_LICENSE_FILE_PATH` | Path to the license file (JSON) | None | None |
| `INFRAHUB_LICENSE_SIGNING_KEY` | Secret key for license signature verification (PoC only) | None | None |
| `INFRAHUB_LICENSE_SKIP_SIGNATURE_VALIDATION` | Skip license signature validation (development only) | boolean | False |
