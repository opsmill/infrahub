...
| `INFRAHUB_WORKFLOW_SCHEMA_LOAD_PICKUP_TIMEOUT` | Maximum time (sec) the API will wait for a task worker to pick up the schema validation and migration workflows during a schema load. Bounds how long the schema lock can be held when no worker is available. | integer | 60 |
