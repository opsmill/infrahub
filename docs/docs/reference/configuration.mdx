...
| `INFRAHUB_GIT_TLS_CA_FILE` | File path to a CA cert or bundle in PEM format used to verify the certificate of HTTPS git remotes, set as `http.sslCAInfo` in the global git configuration. Falls back to `tls.ca_bundle` when unset. | None | None |
