...
| `INFRAHUB_GIT_TLS_INSECURE` | Skip TLS certificate validation when git connects to HTTPS remotes, by setting `http.sslVerify` to false in the global git configuration. Test and development environments only; never enable in production. | boolean | False |
| `INFRAHUB_GIT_TLS_CA_FILE` | File path to a CA cert or bundle in PEM format used to verify the certificate of HTTPS git remotes, set as `http.sslCAInfo` in the global git configuration. Falls back to `tls.ca_bundle` when unset. | string | None |
...
| `INFRAHUB_DB_TLS_CA_FILE` | File path to CA cert or bundle in PEM format | string | None |
...
| `INFRAHUB_BROKER_TLS_CA_FILE` | File path to CA cert or bundle in PEM format | string | None |
...
| `INFRAHUB_CACHE_TLS_CA_FILE` | File path to CA cert or bundle in PEM format | string | None |
...
| `INFRAHUB_STORAGE_TLS_CA_FILE` | File path to a CA cert or bundle in PEM format used to verify the certificate of the S3 endpoint. Falls back to `tls.ca_bundle` when unset. Cannot be combined with `use_ssl=false`. | string | None |
...
| `INFRAHUB_TRACE_TLS_CA_BUNDLE` | Path to a PEM-encoded certificate authority bundle used to verify the OTLP collector's TLS certificate, for a collector using a private or self-signed certificate. Supported by both exporter protocols; with http/protobuf the endpoint must use https://. Checked at startup. | string | None |
...
| `TLS_CA_BUNDLE` | Path or PEM string for CA bundle to validate syslog server certificate. | string | None |
