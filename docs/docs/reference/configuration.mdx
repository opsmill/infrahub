...
| `INFRAHUB_EXPERIMENTAL_RECOMPUTE_BULK_WRITE` | Persist recomputed display labels, human-friendly ids and Jinja2 computed attributes with one bulk write per family instead of one update flow per value. | boolean | False |
