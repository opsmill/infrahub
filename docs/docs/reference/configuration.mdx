...
| `INFRAHUB_COALESCE_PYTHON_RECOMPUTE_AFTER_MERGE` | When enabled, the coalesced merge and rebase pass recomputes Python transform computed attributes, and their per-node tasks only run for a live write; when disabled, the pass recomputes none of them and those per-node tasks run for a merge and a rebase too. | boolean | True |
