...
| Name | Description | Type | Default |
|------|-------------|------|---------|
| `INFRAHUB_HEALTH_CHECK_TIMEOUT` | Per-dependency health check timeout in seconds | integer | 3 |
...
| `INFRAHUB_DB_GRAPH_TRAVERSAL_QUERY_TIMEOUT` | Server-side transaction timeout in seconds for graph-traversal queries (path traversal and reachable nodes); the query is aborted once it is exceeded. | number | 75 |
