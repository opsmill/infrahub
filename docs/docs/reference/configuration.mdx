...
| `INFRAHUB_COALESCE_PYTHON_RECOMPUTE_AFTER_MERGE` | When enabled, Python transform computed attributes are recomputed by the coalesced merge and rebase pass; when disabled, they fall back to one recompute per changed node. | boolean | True |
