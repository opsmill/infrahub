...
| `INFRAHUB_COALESCE_PYTHON_RECOMPUTE_AFTER_MERGE` | When enabled, the coalesced merge and rebase pass recomputes Python transform computed attributes; when disabled, one task per changed node recomputes them. | boolean | True |
...
| `INFRAHUB_GIT_READ_ONLY_REFS_CHECK_INTERVAL_MINS` | Time (in minutes) between two checks of a read-only repository's remote for movement of its tracked refs. | integer | 15 |
