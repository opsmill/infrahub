...
| `INFRAHUB_GIT_USER_NAME` | User name of the git user. This will be used as the user name when Infrahub commits code to a repository | None | None |
| `INFRAHUB_GIT_USER_EMAIL` | Email of the git user. This will be used as the user email when Infrahub commits code to a repository | None | None |
| `INFRAHUB_GIT_GLOBAL_CONFIG_FILE` | The location of the git config file. This will be set as the system `GIT_GLOBAL_CONFIG` environment variable if the environment variable is not initially set | None | /opt/infrahub/.gitconfig |
| `INFRAHUB_GIT_ALLOW_EXPLICIT_MERGE_COMMIT` | Whether to allow explicit merge commits when infrahub merges branches | boolean | False |
