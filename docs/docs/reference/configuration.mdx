...
| `INFRAHUB_COALESCE_PYTHON_RECOMPUTE_AFTER_MERGE` | When enabled, the coalesced merge and rebase pass also recomputes Python transform computed attributes, alongside the per-node tasks that still run; when disabled, only those per-node tasks recompute them. | boolean | True |
