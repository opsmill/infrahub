...
| `INFRAHUB_STORAGE_TLS_CA_FILE` | File path to a CA cert or bundle in PEM format, or the PEM text itself, used to verify the certificate of the S3 endpoint. Falls back to `tls.ca_bundle` when unset. Cannot be combined with `use_ssl=false`. | string | None |
