...
| `INFRAHUB_GIT_IMPORT_SYNC_BRANCH_NAMES` | Names or regex of branches to be created in infrahub during import e.g. 'main', 'infrahub/.*', 'release/.*', '^branch-'. Note: other branches created with sync with git will be imported also | array[string] | None |
