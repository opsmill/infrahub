...
```json
{
  ...
  "errors": [
    {
      "message": "The target branch has been merged and is permanently read-only.",
      "extensions": {
...
```json
{
  ...
  "errors": [
    {
      "message": "The target branch must be rebased before it accepts new changes.",
      "extensions": {
...
The write was rejected because a branch merge is in progress. The block is transient; retry once the merge completes.
...
```json
{
  ...
  "errors": [
    {
      "message": "The write was rejected because a branch merge is in progress. The block is transient; retry once the merge completes.",
      "extensions": {
        "code": "MERGE_IN_PROGRESS",
        "http_status": 423,
        "data": {
          "branch_name": "example",
          "merging_branch": "example"
        }
      }
    }
  ]
...
The submitted values collide with an existing object on a uniqueness constraint. The constraint members may be relationships as well as attributes.
...
```json
{
  ...
  "errors": [
    {
      "message": "The submitted values collide with an existing object on a uniqueness constraint. The constraint members may be relationships as well as attributes.",
      "extensions": {
...
No worker answered the request within the time allowed. The failure is transient; retry after the number of seconds the payload reports.
...
```json
{
  ...
  "errors": [
    {
      "message": "No worker answered the request within the time allowed. The failure is transient; retry after the number of seconds the payload reports.",
      "extensions": {
        "code": "WORKER_TIMEOUT",
        "http_status": 504,
        "data": {
...
- `UNIQUENESS_VIOLATION` reports `422` rather than a semantically correct `409 Conflict`, for the same REST-parity reason as the entry above. Its `path` also stops at the mutation field instead of extending to the offending input, because the constraint is evaluated deep inside the write and a composite constraint has no single offending field. Use `data.fields` to identify the colliding members.