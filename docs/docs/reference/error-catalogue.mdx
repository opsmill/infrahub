...
No worker answered the request within the time allowed. The failure is transient; the request may be retried.
...
```json
{
  ...
  "errors": [
    {
      "message": "No worker answered the request within the time allowed. The failure is transient; the request may be retried.",
      "extensions": {
        "code": "WORKER_TIMEOUT",
        "http_status": 504,
        "data": {
