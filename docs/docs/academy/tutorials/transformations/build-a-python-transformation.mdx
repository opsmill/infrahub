...
For conceptual background, see [Transformations](../../../transformations/). For a recipe-form how-to without the running example, see [Write a Python transformation](../../../transformations/python).

Within Infrahub a [Transformation](../../../transformations/) is defined in an [external repository](../../../git-integration/connect-repository). However, during development and troubleshooting it is easiest to start from your local computer and run the Transformation using [infrahubctl transform]($(base_url)infrahubctl/infrahubctl-transform).

The tutorial follows these steps:

1. Identify the relevant data you want to extract from the database using a [GraphQL query](../../../development-resources/graphql/), that can take an input parameter to filter the data
...
When running the Transformation, the `data` input variable will consist of the response to the query we created or the webhook payload if using the Transformation with a [Custom Webhook](../../../webhooks/index.mdx).
