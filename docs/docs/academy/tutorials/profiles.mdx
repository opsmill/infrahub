...
By the end of this tutorial you will have created a Profile that defines standard configuration for network interfaces, applied it to multiple interfaces, overridden Profile values for specific cases, updated the Profile and seen changes propagate, and worked with multiple Profiles to understand priority. You'll leave with a working mental model of Profile inheritance.

This tutorial uses a network interface schema so you can follow along end-to-end with a single running example. The same patterns apply to any Profile-enabled schema.

For conceptual information about Profiles, including inheritance rules and priority systems, see [Profiles](../../profiles/index.mdx).
...
From Infrahub 1.7 relationships can also be defined in Profiles. If you are using an earlier version, you can skip the untagged VLAN part. Learn more in the [Understanding Profiles in Infrahub](../../profiles/index.mdx#relationships-in-profiles) topic.
...
Learn more about Profile priorities in the [Profile priority system](../../profiles/priority-and-inheritance.mdx#profile-priority-system) section.
...
- [Understanding Profiles in Infrahub](../../profiles/index.mdx) - Detailed explanation of Profile concepts, inheritance rules, and design decisions
