...
- Omit `name`, `from_time`, and `to_time`, and the comparison covers the branch's `branched_from` timestamp to the present. Once a branch has been merged, that same form returns the last diff stored for it instead, whatever period that diff covers.
