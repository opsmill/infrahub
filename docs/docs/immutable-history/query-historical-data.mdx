...
Then retrieve it by the name you stored it under, using `DiffTree` for the changed nodes or `DiffTreeSummary` when you only need counts. The timestamps do not need to align with a branch point or a Proposed Change, so you can compare any useful period, such as the hours around an incident.

Passing `name` on its own retrieves the diff stored under that name, whatever period it covers. Pass the time range instead when you want a diff for that range whether or not it was stored under a name, and pass both to require the named diff to match the range as well.
...
- Omit `name`, `from_time`, and `to_time`, and the comparison covers the branch's `branched_from` timestamp to the present. Once a branch has been merged or is being deleted, that same form returns the last diff stored for it instead, whatever period that diff covers.
...
- A `null` result means that no diff matching the requested name or period has been calculated, not that nothing changed. A calculated diff with no changes in it comes back as a result with zero counts.
