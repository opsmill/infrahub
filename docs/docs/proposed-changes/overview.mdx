...
Infrahub cannot always detect every file a Transformation depends on, and it says so in two different ways.

A Jinja2 template that pulls in a partial through a runtime variable (`{% include some_variable %}`) leaves the auto-detected closure **incomplete**, recorded as `dependencies_complete = false` on the Transformation. While a closure is incomplete Infrahub cannot tell which file changes are relevant, so it conservatively regenerates that Transformation's artifacts on **any** file change in the repository. The task log explains each such fallback and names the Transformation, and the repository import log records every reference auto-detection could not resolve, naming the template and the location of the unresolved include.

A Python Transformation arrives at the same outcome by a different route. Its closure is always complete, because there is only ever the one file to detect, but imports are never analyzed, so a closure carrying no `watch` declaration cannot be trusted to name everything. Infrahub therefore ties such a Transformation's fingerprint to the repository's current commit: the fingerprint moves on every commit, and the artifacts regenerate. This is not the incomplete-closure fallback above, and `dependencies_complete` stays `true` throughout.

Either route produces the noisy behavior the precise gate is meant to avoid. To restore precise regeneration, do one of the following, then commit so the closure recomputes on the next import:
...
- **Declare the missing files with `watch.files`** on the Transformation entry in `.infrahub.yml`. Listing the files, or the directory that contains them, both adds them to the closure and marks the closure complete. On a Python Transformation, declare the key even with an empty `files` list when there is nothing to add: the declaration itself is what unties the fingerprint from the commit. See [declaring extra dependencies](../git-integration/infrahub-yml#declaring-extra-dependencies-with-watch).
...
The two repository-level rules above apply to Generators as well: editing a repository's `.infrahub.yml` re-runs every Generator in that repository, and read-only repositories participate on the same terms - bumping a `CoreReadOnlyRepository` to a new pinned commit re-runs the Generators affected by that commit, even on branches where `sync_with_git` is disabled.
