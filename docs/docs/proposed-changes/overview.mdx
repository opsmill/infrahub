...
- **Repository Import Check**: Reports when the last object import for a connected Git repository failed on the source branch, so a proposed change cannot merge while a repository's registered objects are out of step with the branch.
