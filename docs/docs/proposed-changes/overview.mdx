...
Infrahub cannot always detect every file a Transformation depends on. A Jinja2 template that pulls in a partial through a runtime variable (`{% include some_variable %}`) leaves the auto-detected closure **incomplete**, recorded as `dependencies_complete = false` on the Transformation. A Python Transformation is in the same position for a different reason: its imports are never analyzed, so until it declares `watch` - with an empty `files` list if it needs no extra files - Infrahub has to assume there is a dependency it cannot see.
