...
- Editing a repository's `.infrahub.yml` does not by itself regenerate a Transformation whose dependency closure is complete. The manifest is not part of that closure, so a comment, a reformat, or an entry belonging to another definition regenerates no artifacts. What does regenerate is the change the edit makes to a definition: changing a Transformation's `template_path`, `file_path`, `class_name`, `convert_query_response`, or `watch.files`, or an artifact definition's `parameters`, `content_type`, `artifact_name`, or `targets`, is a change to the definition itself and regenerates its artifacts. Two cases still re-render on any manifest edit: a Transformation in one of the fallback states below, which re-renders on any file change in the repository, and one that names `.infrahub.yml` in its [`watch.files`](../git-integration/infrahub-yml#declaring-extra-dependencies-with-watch), which puts the manifest back in its closure deliberately.
