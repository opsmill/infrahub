...
After a merge the order is enforced: Infrahub waits for each Generator to finish, then regenerates the artifacts built from the objects that Generator created or changed. Only the pipeline inside the proposed change runs both concurrently.

To review correct artifacts on a proposed change, run the Generator on the branch before the pipeline starts: set `execute_in_proposed_change: false` on the Generator and run it through an [event rule](../events/event-rules) when its input data is created on the branch, then open the proposed change once the Generator has finished. [Running a Generator from events](../generators/overview#running-a-generator-from-events) describes the setup. On a proposed change that is already open, re-running the checks regenerates the artifacts from the branch as it stands.
