...
- **Repository import status**: Reports when the last object import for a connected Git repository failed on the source branch, so a proposed change cannot merge while a repository's registered objects are out of step with the branch.
...
- Editing a repository's `.infrahub.yml` does not by itself regenerate a Transformation whose dependency closure is complete. The manifest is not part of that closure, so a comment, a reformat, or an entry belonging to another definition regenerates no artifacts. What does regenerate is the change the edit makes to a definition: changing a Transformation's `template_path`, `file_path`, `class_name`, `convert_query_response`, or `watch.files`, or an artifact definition's `transformation`, `parameters`, `content_type`, `artifact_name`, or `targets`, is a change to the definition itself and regenerates its artifacts. Two cases still re-render on any manifest edit: a Transformation in one of the fallback states below, which re-renders on any file change in the repository, and one that names the manifest in its [`watch.files`](../git-integration/infrahub-yml#declaring-extra-dependencies-with-watch), which puts it back in the closure deliberately - spelled the way the repository spells the file, `.infrahub.yml` or `.infrahub.yaml`.
...
### Ordering between Generators and artifacts

Within a proposed change, Generator runs and artifact generation start at the same time, and neither waits for the other. The pipeline also selects which artifact definitions to process from the branch diff as it was when the pipeline started. An artifact whose Transformation reads data a Generator creates during the same proposed change can therefore render before that data exists, and which of the two finishes first varies from run to run.

To review artifacts built from Generator output, run the Generator on the branch before the pipeline starts. Set `execute_in_proposed_change: false` on the Generator and run it through an [event rule](../events/event-rules) when its input data is created on the branch, then open the proposed change once the Generator has finished. For the setup, see [Running a Generator from events](../generators/overview#running-a-generator-from-events). On a proposed change that is already open, re-run the checks to regenerate the artifacts from the current state of the branch.
