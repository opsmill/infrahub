...
querying for. A filter will be created for every attribute and relationship defined in the schema.

You can query Infrahub's schema, to discover the available filters for a give Node kind.
...
The `name` of each filter can be used as the argument for the `get` and `filters` method. The `kind` of the filter defines what type of value it accepts. 
...
An `IndexError` exception will be raised, if the query would return multiple Nodes. `infrahub_sdk.exceptions.NodeNotFound` exception will be raised if the query returns no results.
...
It's possible to include or exclude attributes and/or relationships from the query, with the `include` and `exclude` arguments of the filter methods.
