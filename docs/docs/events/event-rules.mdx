...
3. Configure the rule:
   ...
   - Select the action kind. A group trigger rule can only fire a Generator action (example: `Generator Action Core`)
...
To change group membership from an event, use a node trigger rule with a [group action](./event-actions#configure-a-group-action) instead.

## Where trigger rules are stored and evaluated

Infrahub reads trigger rules from the default branch only. A rule created on another branch does not fire, and creating it reports no error: Infrahub skips the rule when it reconciles the trigger rules, and the task worker logs a warning, `Skipping trigger rule with an unresolved relationship`, naming the rule and the relationship it could not resolve. Create rules, together with the groups and Generator definitions they point to, on the default branch, and use `branch_scope` to control which branches they fire on:
