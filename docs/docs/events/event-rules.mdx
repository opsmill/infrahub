...
:::warning
A group trigger rule supports only Generator actions. Attaching a group action to a group trigger rule fails when Infrahub configures the rules, and while such a rule exists no other trigger rule change is applied ([#10463](https://github.com/opsmill/infrahub/issues/10463)). Existing rules keep firing; deleting the rule restores configuration.
:::

## Where rules live and where they fire

Infrahub reads trigger rules from the default branch only. A rule created on another branch is never evaluated, and no error reports this: the rule is stored on the branch, but it does not fire there or anywhere else.

Create rules on the default branch, and use `branch_scope` to control which branches they fire on:
