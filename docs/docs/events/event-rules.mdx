...
| Scope | Fires on |
|---|---|
| `default_branch` (default) | The default branch only |
| `other_branches` | Every branch except the default branch |
| `all_branches` | Every branch |

A rule created without changing this field fires on the default branch only. Set `other_branches` when the automation should run inside branches and proposed changes, so its results are reviewed before they reach the default branch.
...
Create rules on the default branch, together with the groups and Generator definitions they reference, and use `branch_scope` to control which branches they fire on:

- `default_branch` (the default): the rule fires only for events on the default branch.
- `other_branches`: the rule fires only for events on other branches. Use this when the outcome should be reviewed in a proposed change before it reaches the default branch.
- `all_branches`: the rule fires everywhere.
