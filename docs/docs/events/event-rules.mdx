...
Every rule has a `branch_scope` that limits which branches its events are matched on. The scope is matched against the branch the event was emitted on.

| Scope | Label in the UI | Fires on |
|---|---|---|
| `default_branch` | **Default Branch** | The default branch only |
| `other_branches` | **Other Branches** | Every branch except the default branch |
| `all_branches` | **All Branches** | Every branch |

The default is `default_branch`, so a rule created without setting this field never fires inside a branch or a Proposed Change. That is the opposite of what automation running inside a branch needs: set `other_branches` when the results should be reviewed in a Proposed Change before they reach the default branch, as [chained Generators](../generators/modular-best-practices#use-branches-for-cascade-testing) do.

Which scope can match depends on the branch an event is emitted on, and that is not always the branch you changed something on. Webhooks share this field, and [configuring branch types](../webhooks/overview#configuring-branch-types) lists the emission branch for each class of event.
...
3. Configure the rule:
   ...
   - Set the [branch scope](#branch-scope). It defaults to **Default Branch**, which does not fire inside a branch
   - Click **Save**.
...
3. Configure the rule:
   ...
   - Select the action kind (example: `Generator Action Core`). A group trigger rule can only fire a Generator action
...
To change group membership from an event, use a node trigger rule with a [group action](./event-actions#configure-a-group-action).

## Where trigger rules are stored and evaluated

Infrahub reads trigger rules from the default branch only. A rule created on another branch does not fire, and you see no error when you create it. Infrahub skips the rule while it reconciles the trigger rules, and the task worker logs a warning, `Skipping trigger rule with an unresolved relationship`, naming the rule and the relationship it could not resolve.

Create rules on the default branch, together with the groups and Generator definitions they reference, and use [`branch_scope`](#branch-scope) to control which branches they fire on.

## Known limitations

- [10463](https://github.com/opsmill/infrahub/issues/10463) attaching a group action to a group trigger rule stops Infrahub from applying any trigger rule change until that rule is deleted. Existing rules keep firing, so the failure can go unnoticed.
