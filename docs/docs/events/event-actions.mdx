...
[Webhooks](../webhooks) are a related concept — they also fire in response to events — but they live outside the rule/action loop because their setup is more involved. Use a Generator action when the outcome stays inside Infrahub; use a webhook when you need to send a payload to an external HTTP endpoint.
...
- [Webhooks](../webhooks) — a related event-driven outcome that lives outside the rule/action loop
