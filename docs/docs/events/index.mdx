...
[Webhooks](../webhooks) consume events too, but live outside this rule/action loop because their setup is more involved. Use webhooks when you need to send a payload to an external HTTP endpoint; use event rules + actions when the outcome stays inside Infrahub.
...
**Goal**: when a circuit joins the `provisioning_circuits` group, run a [Generator](../generators/) that creates its endpoints.
