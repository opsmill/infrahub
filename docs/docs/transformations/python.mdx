...
When a Python Transformation feeds an artifact, Infrahub regenerates that artifact only when a file the transform depends on changes. On import, Infrahub records the transform's dependency closure as the file named by its `file_path`, and nothing else. Files that merely sit in the same directory are not part of the closure, so several unrelated Transformations can share a directory without each one regenerating whenever any of them is edited.

Infrahub does not analyze your imports, so anything the transform depends on beyond its own file has to be declared with the [`watch` key](../git-integration/infrahub-yml#declaring-extra-dependencies-with-watch) in `.infrahub.yml` - a helper module beside the transform just as much as one in another package such as `shared/`. A `watch` entry naming a directory watches every Git-tracked file beneath it, so a transform with several helper modules alongside it can declare its own directory in one line:
...
**Declare `watch` even when the transform depends on nothing but its own file**, using an empty list:
...
Because imports are never analyzed, Infrahub cannot tell a transform with no further dependencies from one whose dependencies it simply has no way to see. Until the `watch` key is present, it assumes the latter and treats every commit to the repository as a possible change to the Transformation, regenerating its artifacts. Declaring `watch` - whether it lists files or not - is your statement that the closure is complete, and it is what makes regeneration precise. Note that the key has to carry the `files` list: a bare `watch:` with nothing under it reads as no declaration at all.
