...
A Python Transformation processes graph data through user-written Python code, producing JSON output (or any structured data you serialize). Use this when Jinja templating isn't enough — for example, conditional logic, external API calls, or complex aggregation. The steps below cover how to write one.

For conceptual background, see [Transformations](./).
...
Create a `.gql` file with the data your transform needs.
...
For reusable query fragments, see [GraphQL fragments](../guides/graphql-fragment).
...
With `convert_query_response: true`, use `self.nodes[0]` (SDK objects with attribute access like `device.name.value`); otherwise use the raw `data` dict (`data["InfraDevice"]["edges"][0]["node"]["name"]["value"]`).
...
Set `convert_query_response: true` if you want the SDK to convert the response into `InfrahubNode` objects accessible via `self.nodes`.
...
Push the branch and merge it. The Transformation registers with Infrahub when the repository syncs.
...
To cache transform output and tie it to a specific target object, define an artifact that uses this Transformation. See [Use artifacts](../guides/artifact).
...
- [Use artifacts](../guides/artifact) — cache Transformation output
