...
Webhooks are HTTP callbacks that deliver data to external systems in real-time when an [event](../topics/events.mdx) occurs within Infrahub. They allow you to:
...
:::info
The **Webhook System** also powers the [Activity Log](../topics/activity-log.mdx).
:::
...
- `Standard Webhook` sends event information to an external endpoint. See [Create a webhook](./create) for setup.
- `Custom Webhook` allows the ability to tie events to a [Transformation](../topics/transformation.mdx) to configure the payload being sent to an external endpoint. See [Webhook with custom Transformation](./custom-transformation) for the full walkthrough.
...
The `data` object from the payloads above is passed to the `transform` method of your custom Transformation. For the full setup walkthrough, see [Webhook with custom Transformation](./custom-transformation).
