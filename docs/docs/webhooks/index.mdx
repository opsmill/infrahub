...
- `Custom Webhook` allows the ability to tie events to a [Transformation](../transformations/) to configure the payload being sent to an external endpoint. See [Webhook with custom Transformation](./custom-transformation) for the full walkthrough.
