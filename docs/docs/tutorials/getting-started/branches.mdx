...
2. Select the `my-first-tenant` organization (created in the [previous step](./creating-an-object.mdx)).
...
Go back to the detailed page for the Organization `my-first-tenant` and try to switch branches with the branch selection menu at the top.
...
Go back to the detailed page for the Organization `my-first-tenant`.
