...
Building an IPAM with these generics serves a different purpose from using the `IPHost` or `IPNetwork` attribute kinds on other nodes. The IPAM generics use those attribute kinds internally.
...
Network and broadcast addresses count toward the usable total when `is_pool` is `true`, for IPv4 prefixes with a `/31` length (per [RFC 3021](https://datatracker.ietf.org/doc/html/rfc3021)), and for `/32` prefixes.
...
Infrahub also exposes lower-level GraphQL queries — `InfrahubIPAddressGetNextAvailable` and `InfrahubIPPrefixGetNextAvailable` — that return the next free address or subnet without creating a record and without the pool's cross-branch allocation guarantees. Use Resource Pool for production workflows; reach for these only to look up a value without persisting it.
