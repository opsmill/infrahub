...
Read the computed values on IP prefixes and addresses, or walk the hierarchy tree. Use these patterns for reporting, automation, or verification.
...
Filter on a computed attribute the same way as any other attribute — for example, to find fully utilized prefixes:

```graphql
query {
  IpamIPPrefix(utilization__value: 100) {
    edges {
      node {
        prefix { value }
      }
