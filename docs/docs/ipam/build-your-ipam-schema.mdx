...
The Marketplace collection just speeds this up by giving you a working schema to start from — building it yourself means inheriting directly from the built-in generics. Inheriting from `BuiltinIPPrefix` and `BuiltinIPAddress` gets you hierarchy, utilization, and the other generic behavior for free:

```yaml title="schemas/ipam/prefix-and-address.yml"
nodes:
  - name: IPPrefix
    namespace: Ipam
    inherit_from:
