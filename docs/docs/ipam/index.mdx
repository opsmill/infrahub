...
IP address management (IPAM) in Infrahub gives you a structured, version-controlled model for your IP resources. Prefixes and addresses are first-class objects in the graph — they form hierarchies, track utilization automatically, and integrate with the rest of your infrastructure data.
...
- `BuiltinIPNamespace` — models a namespace to isolate IP resources (analogous to a VRF or routing instance)
- `BuiltinIPPrefix` — models a network (supernet, subnet)
- `BuiltinIPAddress` — models a single IP address

By default, Infrahub includes a node called `IpamNamespace` that inherits from `BuiltinIPNamespace`, and a "default" namespace object is created automatically on first start.
