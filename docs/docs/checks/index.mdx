...
Targeted checks are exactly the same as a check; the main difference is that they target a specific [group](../groups/) of nodes in Infrahub. The check will then only be executed against the nodes in that group.
