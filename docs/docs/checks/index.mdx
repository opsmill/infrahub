...
Checks are user-defined logic, stored in an [external repository linked to Infrahub](../git-integration/index.mdx), that run as part of a [proposed change](../proposed-changes/index.mdx). They let users perform any kind of data validation logic during a proposed change. If a check does not complete successfully, the proposed change cannot be merged.
...
Targeted checks are exactly the same as a check; the main difference is that they target a specific [group](../topics/groups.mdx) of nodes in Infrahub. The check will then only be executed against the nodes in that group.
...
See [Groups](../groups/index.mdx) for details on creating target groups for your checks.
...
- [Proposed Changes](../proposed-changes/index.mdx) — where checks run
- [Repository Management](../git-integration/index.mdx) — where check code lives
- [Groups](../groups/index.mdx) — used to target checks at specific subsets of objects
