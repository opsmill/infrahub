...
4. **Merged**: After receiving approval (if required) and passing all automated checks, the change is merged into the target branch. At this point:
   ...
   - The source branch transitions to a frozen `MERGED` state — no further mutations are allowed on it. It can still be viewed and deleted, but rebase, re-merge, and new Proposed Changes against it are blocked.
