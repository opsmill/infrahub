...
- **Isolated Mode**: When operating in isolated mode, the branch won't automatically keep in sync with the main branch. See the dedicated section if the documentation about the isolated mode for more informatin

Branch names are fairly permissive, but must conform to git ref format. For example, slashes (/) are allowed, tildes (~) are not.

A branch can be created via the UI, via GraphQL or via the cli `infrahubctl`.
