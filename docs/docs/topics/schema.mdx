...
- `Group`: Defines a relationship where a node (inheriting from `CoreNode`) is a member or subscriber to a [group](./groups.mdx) (inheriting from `CoreGroup`). These relationships appear in the "Manage Groups" form and enable organizational concepts fundamental to Infrahub's architecture.
