...
Some namespace like `Core`, `Infrahub` and `Builtin` are reserved and can't be used by users to defined new Nodes and Generics.
The complete list is present in this [file](https://github.com/opsmill/infrahub/blob/stable/backend/infrahub/core/constants/__init__.py#L292)

In a similar fashion, some attributes name can't be use, like `attribute` or `relationship` for obvious reasons.
You can find the complete list in the same [file](https://github.com/opsmill/infrahub/blob/deecf2494e8c8dd84293229e230359f99e26b07d/backend/infrahub/core/constants/__init__.py#L28)
