...
The order in which attributes and relationships appear in the Infrahub frontend is controlled by the `order_weight` property. When not explicitly specified, Infrahub automatically assigns default values in increments of 1000:
...
:::tip Field Ordering Best Practices

- Use values below 1000 for high-priority fields that should appear first
- Use values like 1500, 2500 to insert fields between default assignments
- Leave gaps between values to make future reorganization easier
- For comprehensive guidance, see [Understanding Order Weight](./order-weight) and [Customize Field Ordering](../guides/customize-field-ordering)
