...
There are several ways to [load a new schema](../guides/import-schema.mdx).

:::info Editor validation

To help with the development process of a schema definition file, you can leverage the schemas we publish for validation within your [editor](../reference/schema-validation.mdx)
...
- computed attributes can only be used on Nodes, see [computed attribute topic](./computed-attributes.mdx) for more information.
- Generic's properties are propagated to Nodes only during the first import see the section [Inherited properties](./schema.mdx#inherited-properties) below.
...
```yaml title="Default Order Weight Assignment"
nodes:
  - name: Device
    namespace: Example
    attributes:
...
```yaml title="Custom Field Ordering"
attributes:
  - name: asset_tag
    kind: Text
...
<ReferenceLink title="For more details on order weight, refer to the dedicated topic" url="/topics/order-weight" openInNewTab />
...
Relationships of kind `Component` will automatically be covered by the template when enabling template generation on a given schema node. See the [object-template topic](../topics/object-template.mdx) for more information.
...
For detailed information, see the [Relationship Reference Guide](../reference/schema/relationship.mdx#direction)
...
For detailed information, see the [Relationship Reference Guide](../reference/schema/relationship.mdx#identifier)
...
- [Groups](./groups.mdx) can be organized in a hierarchy by default which makes it possible to query the members of all sub-groups at once.
...
As an example, The GraphQL query below will query the members of all [groups](./groups.mdx) that are defined as children of `Group1`.
...
You can read this [guide](../guides/menu.mdx) for a more detailed explanation of how to operate the menu.
...
Schema files can be loaded into Infrahub through the Schema Marketplace page, with the `infrahubctl` command, or directly via the Git integration.

### Schema Marketplace page

The Schema Marketplace page (accessible from the main navigation or from the **Schema Marketplace** tile on the home page) lets you browse the [Infrahub Marketplace](https://marketplace.infrahub.app) and install schemas or collections without leaving the web UI.

The install drawer offers two install methods — the same button, a different toggle.

#### To repository (recommended for editable schemas)

Commits the selected schema files to a writable `CoreRepository`.

1. Open **Schema Marketplace** from the home tile or the sidebar.
2. Pick one or more schemas or collections to select them.
3. In the install drawer, leave **Install method** on `To repository`, pick a repository, and confirm. The Branch field tracks the Infrahub branch you have selected at the top of the page.
4. The Infrahub backend downloads the selected items from the Marketplace and commits them to `schemas/` on the chosen Git branch. Your existing repository-sync pipeline then loads the schema into the graph.

The "To repository" method requires an Infrahub branch created with **Sync with Git** enabled. A non-synced branch would commit to a Git branch that isn't tracked by any Infrahub branch — the UI disables the toggle when that's the case.

#### Direct (no Git commit)

Applies the selected schemas directly to the target Infrahub branch via the schema-load API — the same endpoint `infrahubctl schema load` uses. No repository, no Git commit, no version history.

Pick **Direct** as the install method when you want to try a schema out quickly, when no writable repository is configured, or when you don't plan to edit the schemas later. Switch to `To repository` later if you decide you want version control.

#### CLI alternative

If no writable `CoreRepository` is configured, the page also renders an **Install via infrahubctl** block: copy-pasteable commands using `infrahubctl marketplace download` + `infrahubctl schema load` so you can run the install against your instance from the terminal.

#### Configuration

The Marketplace URL is configurable via the backend environment variable `INFRAHUB_MARKETPLACE_URL` (default `https://marketplace.infrahub.app`). The same variable is read by `infrahubctl marketplace download`, so a single export points both the UI and the CLI at an internal mirror or staging Marketplace.
...
You can defined a schema in an [external repository](../topics/repository.mdx).
...
[A reference guide for validators and migrations is available](../reference/schema/validator-migration.mdx)
