...
Install flow when you have a writable Git repository:

1. Open **Schema Marketplace** from the home tile or the sidebar.
2. Pick one or more schemas or collections to select them.
3. In the install drawer, pick a **CoreRepository** target and a branch, then confirm.
4. The Infrahub backend downloads the selected items from the Marketplace and commits them to `schemas/` on your chosen branch. Your existing repository-sync pipeline then loads the schema into the graph.

If you have no writable `CoreRepository` (no repositories configured, or only read-only ones), the page instead shows a prerequisite panel plus an **Install via infrahubctl** block: copy-pasteable commands that apply the selected schemas directly to your instance using `infrahubctl marketplace download` + `infrahubctl schema load`.
