...
When writing GraphQL queries for [transforms](./transformation.mdx), [generators](./generator.mdx), [artifacts](./artifact.mdx), and [computed attributes](./computed-attribute.mdx), it's critical to use a **single-target query pattern** to ensure optimal artifact generation and system performance.

### What is a single-target query?

A single-target query is a GraphQL query that targets a **unique node** using either an ID or a unique attribute (HFID - Human-Friendly Identifier). This pattern enables Infrahub to identify exactly which objects are affected by a change, allowing it to selectively regenerate only the necessary artifacts instead of regenerating all of them.

### Why is this important?

Without single-target queries, Infrahub cannot determine which specific artifacts need regeneration when data changes. This can lead to excessive artifact generation that significantly impacts performance.

**Real-world impact:** In one production scenario, a proposed change pipeline generated 600 artifacts when only 5 actually required regeneration. Properly using single-target queries resolved this issue.
...
2. **Use a required variable or static value**: The filter must use a required variable (e.g., `$name: String!`) or a hardcoded value
3. **Use exact match filters**: Use singular filters (e.g., `name__value: $name`), **not** list filters (e.g., `name__values: $name`)
...
**Using HFID with required variable:**

```graphql
query DeviceByHFID($device_hfid: String!) {
  InfraDevice(hfid: [$device_hfid]) {
    edges {
      node {
        id
        hfid
...
### Verifying your query is single-target

You can verify whether your query qualifies as single-target using the `InfrahubGraphQLQueryAnalyzer` in the Infrahub shell:
...
# Get the default branch and schema
...
# Check if it's a valid single-target query
print(f"Valid single-target: {analyzer.query_report.only_has_unique_targets}")
```
...
- **Computed attributes** - See [Computed attributes](./computed-attribute.mdx)
