...
A single-target query is a GraphQL query that targets a **unique node** using either a Human-Friendly ID (HFID) or a unique attribute. This pattern enables Infrahub to identify exactly which objects are affected by a change, allowing it to selectively trigger the necessary action instead of everything.
...
2. **Use a required variable**: The filter must use a required variable, for example, `$name: String!`. A hard-coded value is also valid but limits the query to a single fixed object
