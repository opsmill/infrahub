...
2. Extracts the value from the target object using the double-underscore path
...
- **In the UI**: when creating or editing an object, select the group in the "Member of groups" field
...
### CoreStandardGroup vs. CoreGeneratorGroup
...
| `CoreGeneratorGroup`  | Tracks which objects were **created by** a Generator. Managed automatically by the SDK's tracking feature. | No — Infrahub manages this automatically |
...
This is the simpler mode — no conversion overhead, and you work with plain Python dicts. You can also use [Pydantic models](https://docs.pydantic.dev/) to parse the dict into typed objects for better IDE support and validation.

### SDK object mode

When `convert_query_response` is `true`, Infrahub converts the GraphQL response into `InfrahubNode` SDK objects. These are available via `self.nodes` (a list) and `self.store` (indexed by node ID). You do **not** use the `data` parameter in this mode:
...
| The query returns a simple, flat structure | You want to use `self.store` to look up nodes by ID |
...
When `true`, the Generator runs during proposed change CI. Results appear in the **Checks** and **Data** tabs of the proposed change. This is how you preview what the Generator will create before merging.

When `false`, the Generator is skipped entirely in proposed changes. Use this for Generators that are triggered by events (checksum triggers) to avoid double execution — once by CI and once by the trigger.

### `execute_after_merge` (default: `true`)

When `true`, the Generator runs after a merge to apply changes to the target branch.

When `false`, the Generator is skipped after merges. This is rare — mainly useful for Generators that exist solely for preview or validation purposes in proposed changes.

### Common combinations

| `execute_in_proposed_change` | `execute_after_merge` | Use case |
|---|---|---|
| `true` | `true` | **Default.** Full lifecycle — preview in proposed changes, apply on merge. |
| `false` | `true` | Skip preview, only run on merge. Use for Generators with external side effects. |
| `true` | `false` | Preview only — validate in proposed changes but do not apply on merge. Rare. |
| `false` | `false` | Never runs automatically. Only manual execution via `infrahubctl` or event triggers. Use this for event-driven Generators in a [modular setup](modular-generators). |

### Implications for modular Generators

In [modular Generator](modular-generators) setups where multiple Generators are connected through triggers, these flags determine which layers run at each lifecycle stage. If you set `execute_in_proposed_change: false` on an upstream Generator, all downstream Generators will also effectively be skipped in proposed changes — since their upstream dependencies will not exist.
