...
1. The user is presented with available authentication options (local login and configured SSO providers)
    :::note
    It's possible to have multiple SSO providers configured. Even with SSO provider enabled, local login remains available for fallback scenarios (for example when identity provider is unavailable).
    :::
