...
:::info Color-coding
Standalone events are grey in the global list, while events containing child events are blue.
:::
...
* **Event type**: Filter by categories such as `node.updated`, `group.member_added`.
...
* **Event type**: The type of the event, for example, `infrahub.node.updated`. You can find more information in the reference page for [Infrahub Events](../reference/infrahub-events)
