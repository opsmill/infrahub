...
Node kinds can be configured to only trigger wehbooks for specific nodes. For example only send webhooks for the node kind of `InfraDevice`.

:::note

Node kind can only be used where `infrahub_node_kind_event` is `True` in [Infrahub events](../reference/infrahub-events.mdx). Below are some example events that are enabled:
