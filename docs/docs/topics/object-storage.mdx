...
Objects or files stored with the object-storage, are identified by its identifier (UUID). An identifier is assigned to the object at time of creation within the object-storage. Interactions with the object then happen leveraging the identifier.
