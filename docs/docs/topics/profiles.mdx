...
Profiles in Infrahub are configuration templates that define common attribute and relationships values for objects. They represent a fundamental approach to managing configuration at scale by separating the definition of common settings from individual object instances.
...
- **Schema evolution**: Profile schemas update automatically when schemas nodes change
