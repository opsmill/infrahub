...
Starting with Infrahub 1.7, profiles can define relationship values in addition to attributes. This enables you to set default relationships that nodes inherit when assigned to a profile.
...
1. **Optional relationships only**: Profiles can only define values for optional relationships. Required relationships must still be set directly on the node.

2. **Cardinality support**: Both `one` and `many` cardinality relationships are supported:
   - For `one` relationships, the profile defines a single related node
   - For `many` relationships, the profile defines a set of related nodes that the inheriting node will be connected to

3. **Priority-based resolution**: When multiple profiles define the same relationship, the profile with the highest priority (lowest priority number) takes precedence.

4. **Override capability**: Just like attributes, relationship values can be overridden on individual nodes when exceptions are needed.

5. **Metadata tracking**: Relationship values inherited from profiles include metadata indicating their source, enabling full data lineage tracking.

### Example use cases

- **Default VLAN assignment**: Define a profile that sets the untagged VLAN for all end-user interfaces
- **Default location assignment**: Set a default location or site for nodes of a specific type
...
2. **Relationship restrictions**:
   - Can only define optional relationships (not required ones)
   - Hierarchical relationships (`parent`/`children`) are not supported in profiles
...
- Maintain data integrity (unique constraints, required relationships)
