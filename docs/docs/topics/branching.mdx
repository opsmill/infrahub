...
- **Branch merging**: Once changes are complete, they can be merged back into the parent branch through a controlled process using Proposed Changes. This ensures that all modifications are validated and conflicts are resolved. The merge process creates a permanent record in the commit history that documents what changed and why. After a successful merge, the source branch is automatically frozen: GraphQL mutations (create, upsert, update, delete) are blocked, the UI disables editing controls, and opening a new Proposed Change against the merged branch is prevented.
- **Branch deletion**: After merging, branches can be deleted without losing any history. All changes are preserved in the commit history of the default branch. Deletion can be triggered in three ways:
