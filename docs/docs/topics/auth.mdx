...
- `object:*:*:any:deny` to deny anything on all objects, across all branches (this one is not required, but it is more explicit)
- `object:*:*:view:allow_all` to allow seeing all objects, across all branches
