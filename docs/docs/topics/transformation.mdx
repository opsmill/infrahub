...
## Targeting transformations with groups

Transformations can operate on entire [groups](./groups.mdx), applying consistent changes across all members. This enables bulk operations without duplicating transformation logic. When used with [artifact definitions](./artifact.mdx) or [generators](./generator.mdx), transformations can target specific groups to process multiple objects with a single configuration.

Groups provide a flexible targeting mechanism that decouples transformation logic from specific object lists, making transformations more reusable and maintainable. See [organizing objects with groups](../guides/groups.mdx) for details on creating target groups.
