...
release_type: patch
description: "Fix bug in computed attribute calculation that prevented having a computed attribute on a branch if there were no computed attributes on the default branch (8270) Add migration to allow rebasing branches created before (8221) was fixed so that the uniqueness constraint on SchemaNode.name and SchemaGeneric.name is removed"
---
<table>
  <tbody>
    <tr>
