...
We can now list, create and delete tokens via the frontend.

The view is available under Your `Profile > Tokens`.

When a token is created, it will be available only once, so it must be copied right away.
