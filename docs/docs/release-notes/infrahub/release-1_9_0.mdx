...
release_type: minor
breaking: true
description: "Two headline additions: an interactive schema visualizer that turns your schema into a navigable graph, and syslog log forwarding for Enterprise — streaming activity, login/logout, and permission-denied events to SIEM systems such as Splunk, Datadog, and ELK over TCP or UDP with optional TLS. Jinja2 computed attributes now recalculate inline on local changes instead of spawning a background task per node, and new Jinja2 filters let artifacts inline and parse content from other artifacts and file objects."
---
<table>
  <tbody>
    <tr>
...
Webhooks can now send arbitrary custom HTTP headers - the most common use case being authentication to target systems that require `Authorization: Bearer <token>` or similar.
