...
release_type: patch
description: "Add a security setting for the default SSO user group, giving the name of a group to which SSO users are assigned if the identity provider does not give a list of groups to use (4924) Added a configuration setting for Git repositories that appends '.git' to repositories defined with an HTTP URL for the configured domains (5077)"
---
<table>
  <tbody>
    <tr>
