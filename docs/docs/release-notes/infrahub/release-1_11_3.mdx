...
release_type: patch
breaking: true
description: "Adds a configurable CA bundle for outbound TLS, an infrahub db reset command, and a load-shed marker header that the web UI uses to wait and retry. Schema load and check errors now return one 422 entry per offending field, and many fixes improve diff, merge, and listing performance on large branches."
---
<table>
  <tbody>
    <tr>
