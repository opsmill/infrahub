...
release_type: patch
breaking: true
description: "Releases branch-agnostic attribute and relationship values once no branch can still read their object, so uniqueness constraints and allocated resource pool values no longer stay reserved after a delete. Diff calculation, diff saving and display label enrichment on large branches are substantially faster, and a new CA bundle setting lets every component trust a private certificate authority."
---
<table>
  <tbody>
    <tr>
