...
breaking: true
description: "Enables OIDC id_token verification by default and blocks local password changes for directory-backed accounts. Schema names containing __ are now rejected."
---
<table>
  <tbody>
    <tr>
...
- Fix schema processing crashing with `'NodeSchema' object has no attribute 'get'` when a node defines a single-element relationship-only `uniqueness_constraints` entry (for example `[["parent"]]`). ([#4483](https://github.com/opsmill/infrahub/issues/4483))
