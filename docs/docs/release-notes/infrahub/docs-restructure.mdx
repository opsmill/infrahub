...
Get started, Learn, Deployment & Management, Development Resources, and Reference round out the sidebar as supporting sections.
...
The "Object Storage" section has been renamed to **Artifact & File Storage**. The previous name was ambiguous — "object storage" is a common term for cloud blob storage services like S3 or GCS, which is not what this feature is. The new name reflects what the section actually covers: storage for artifacts produced by Infrahub's generators and transformations, and for files attached to schema nodes via the File Object kind. The rename makes the connection to those two features explicit and removes the confusion with external storage services.
...
- **Extended schema kinds**
  ...
  - **Number pool attribute** · `schema/number-pool` · from `topics/schema-attr-kind-number-pool` — moved verbatim
  - **File object node** · `schema/file-object` · from `topics/file-object` — moved verbatim
