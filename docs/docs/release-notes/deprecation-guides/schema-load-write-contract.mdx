...
`.message`, plus the same information split into parts: the path as a tuple of keys and
indexes in `.loc`, the value received there in `.input` and the reason on its own in
`.reason`. `result.raise_for_status()` raises a `ValueError` joining all the messages when
the payload is invalid. The server reports the same violations in its 422 response, one
entry per offending field with `loc`, `input` and `msg` matching these parts. Each entry in `result.warnings` additionally carries the schema kind
