...
Earlier versions rejected these too, so this is not a change in whether the payload loads —
only in how the problem is reported. The path is now field-level, and the same message is
available offline before you submit.
...
- A **missing required** field, for example a node without a `name`, or an attribute without
  a `kind`.
- A missing `version` at the root of the payload.
...
### How the server reports a rejection

`POST /api/schema/load` and `POST /api/schema/check` reject an invalid payload with a `422`
request-validation response. Each violation is one entry in `detail`, located on the field
that caused it:
...
```json
{
  "detail": [
    {
      "type": "value_error",
      "loc": ["body", "schemas", 0, "nodes", 0, "attributes", 0, "optionl"],
      "msg": "Unknown field, it is not part of the schema",
      "input": true,
      "ctx": { "reason": "Unknown field, it is not part of the schema" }
    }
  ]
}
```

- `loc` runs from the position of the schema in the request body down to the offending field.
  Integers index into the enclosing list, so `["body", "schemas", 0, "nodes", 0, "attributes",
  0, "optionl"]` is the `optionl` key of the first attribute of the first node of the first
  submitted schema.
- `input` is the value received at that location.
- `msg` is the reason on its own, without the location or the value. It is repeated in
  `ctx.reason`.

Reading the field from `loc` and the value from `input` is what lets a client name the problem
without parsing `msg`. An invalid attribute `kind` is the one case where `loc` stops at the
attribute rather than at a field, for the reason given above, and `input` is then the whole
attribute mapping.

A payload with several violations produces one entry per offending field.
...
`.message`, and the same three pieces the server responds with: `.loc`, `.input` and
`.reason`. Those three carry the same information as the server's `detail` entry, minus the
request-envelope prefix: the offline `.loc` starts at the schema root, so it reads `["nodes", 0,
"attributes", 0, "optionl"]` where the server's reads `["body", "schemas", 0, "nodes", 0,
"attributes", 0, "optionl"]`.
`result.raise_for_status()` raises a `ValueError` joining all the messages when the payload
is invalid. Each entry in `result.warnings` additionally carries the schema kind
