...
### Generic field for relationship

In the form, if a node has a generic relationship, the parent select is now hidden if it's not available.

It's displayed and avalaible only of the kind selected can point to an object which we can filter depending on its parent.
