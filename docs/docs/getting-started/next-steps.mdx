...
Additionally, you might want to install the Infrahub CLI tool on your laptop. This comes as a python package and provides a command-line interface to interact with your Infrahub instance. The CLI is useful for loading data, managing schemas, and performing other administrative tasks.
...
<ReferenceLink title="Consume the schema library" url="$(base_url)schema-library"/>
...
The git repository will be used to store any file needed to capture the intended state of your infrastructure, such as the schema, templates, python scripts...
...
You will need to push this repository to a remote location that Infrahub can access. It doesn't have to be a public repository as Infrahub supports authentication with private repositories. Depending on the provider you use (e.g., GitHub, GitLab, Bitbucket), the process may vary slightly, but the general steps are the same:
...
- [**Create your own schema**](../guides/create-schema) - Extand the existing schema to fit your organization's needs.
...
- [**Deploy artifacts to the field using Ansible**]($(base_url)ansible) - Push outputs to where they're needed and apply your intended state to your infrastructure. You can also use other tools like [Nornir]($(base_url)nornir) to deploy your artifacts.
...
<ReferenceLink title="Book a meeting with OpsMill" url="https://cal.com/pete-crocker/ac3-infrahub-demo" openInNewTab />
