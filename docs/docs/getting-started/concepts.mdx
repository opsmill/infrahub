...
The schema can be loaded in to Infrahub using one of these two methods:

- Using [`infrahubctl schema load`]($(base_url)infrahubctl/infrahubctl-load) command - This is recommended way for development or quick testing purposes.
...
Those concepts are tightly integrated and work together to cover any use cases in the infrastructure management space.

### Design-driven automation / Service Catalog
...
Taking the example of a new service that needs to be deployed:
...
- This sets robust data structure that will allow lifecycle management of the service, including updates, decommissioning, and more.
- Capturing the business logic in the generator allows consistent and repeatable deployments, reducing the risk of human error.
...
Taking the example of a large network software upgrade:
    - A branch is created to stage the changes
    - Any change to the schema could be made in the branch, such as adding a new attribute
    ...
    - The changes are collaboratly reviewed in a proposed changed
