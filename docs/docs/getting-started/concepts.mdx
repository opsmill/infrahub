...
- A new attribute for device objects is needed:
    - A branch is created to stage the changes
    - The schema is updated and loaded in the branch
    - [optional] Transformations/generators are updated to use the new attribute
    - Data for the new attribute is populated in the branch
    - The changes are reviewed and eventually merged in a proposed change

Any changes can be developed in parrallel and reviewed/merged when ready. This allows for a very flexible and agile way of working, where changes can be made quickly without holding back your production flow!