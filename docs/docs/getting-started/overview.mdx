...
Infrahub from [OpsMill](https://opsmill.com) takes a new approach to Infrastructure Management by providing a new generation datastore to organize and control all the data that defines how infrastructure should run. Infrahub offers a central hub to manage the data, templates, and playbooks that power your infrastructure. It combines the version control and branch management capabilities of Git with the flexible data model and UI of a graph database.
