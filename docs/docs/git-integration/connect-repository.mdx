...
The Git status indicator in the header tells you whether anything is wrong without opening a repository. It turns red with a pulsing dot when any repository on the branch you are viewing has an `Import Error` sync status, and selecting it opens the repository list filtered to those repositories. It reads sync status rather than task results, so it still reports a failure when the task that ran the import finished successfully. When no Git repositories are configured it stays inactive.
