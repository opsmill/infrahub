...
If you want to know more about the differences between these two types, refer to the topic on [Repositories](./overview.mdx).
...
- Access to a Git repository containing valid Infrahub resources (see [Initializing an Infrahub repository](../development-resources/developer-guide#initializing-an-infrahub-repository))
  - A [`.infrahub.yml`](./overview.mdx#infrahub-yaml) configuration file in your Git repository
...
:::warning The default branch must exist on the remote

When you connect a `Repository`, Infrahub lists the remote's branches and rejects the request if the `default_branch` you configured is not among them. No repository is left behind, and the mutation returns an error naming the branch Infrahub looked for and the branch the remote uses as its own default:

```text
Branch 'main' does not exist on the remote repository My Git Repository; the remote's default branch is 'master'.
```

When the remote does not report a default branch Infrahub can see - it is empty, its `HEAD` is detached, or its default branch is hidden from the branch listing - the message ends with `the remote is empty or has no default branch.` instead.

Set `default_branch` to a branch that exists on the remote and connect again. It does not have to be the remote's default branch: any existing branch is accepted.

Read-only repositories are not checked this way. Their `ref` can be a branch, a tag, or a commit hash, and a branch listing cannot confirm the last two.
...
### Change the default branch of a connected repository

The `default_branch` of a `Repository` can be edited after the repository is connected, but Infrahub treats that edit as a plain attribute update. Be aware of the following limitations:

- The new value is not validated against the remote. If the branch does not exist there, the edit is still accepted, and a later operation that needs that branch fails instead. Operations that do not need it, including a synchronization that an existing clone can serve, keep working, so the mistake can stay hidden for a while.
- Nothing re-clones the repository or reconciles the branches that were already imported under the previous mapping.
- The new value is served from cache for a short interval, so operations started immediately after the edit may still use the previous one.

The **Check connectivity** action does not validate the branch either: it verifies that the remote is reachable with the configured credentials, nothing more.

To change the mapping safely, confirm that the branch exists on the remote first, then apply the edit and let a synchronization cycle complete before relying on the repository.
...
For more details on how each repository type tracks changes, refer to the [Repository Topic](./overview.mdx#read-only-vs-core).
...
1. Check the repository status indicators:
   ...
   For detailed information about what each status means, refer to the [Repository Status documentation](./overview.mdx#repository-statuses).
...
For detailed instructions on how to apply configuration changes to your Infrahub instance, see [How to configure Infrahub](../deploy-manage/install-configure/configure-infrahub).
...
- [Repository concept explanation](./overview.mdx)
- [Infrahub configuration file documentation](./infrahub-yml.mdx)
