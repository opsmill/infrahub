...
When the remote is empty or has no default branch, the message ends with `the remote is empty or has no default branch.` instead.
...
- The new value is not validated against the remote. If the branch does not exist there, the next Git operation fails instead of the edit being rejected.
