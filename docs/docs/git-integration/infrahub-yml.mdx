...
- **Fragment definitions**: Reusable [GraphQL fragments](../development-resources/graphql-fragments) for composing queries
...
Python example, where a Transformation imports helpers from a sibling top-level package and from a module next to it:
  ...
  - name: DeviceNameAttribute
    ...
    watch:
      files:
        - utils/
        - shared/helpers.py
        - transforms/device_name_helpers.py
```

Generator example, where a Generator imports helpers from a sibling top-level package. The `watch` key behaves identically on a `generator_definitions` entry:
...
Declaring any `watch.files` entry also marks the definition's dependency closure as complete, which suppresses the conservative regenerate-on-any-change fallback that an incomplete Jinja2 auto-detection would otherwise trigger. On a Python Transformation or Generator, where auto-detection has no references to fail on, the declaration earns its precision the way described in the next section instead. The one exception is a closure that ends up empty, because every entry you declared matched no tracked file and auto-detection found nothing either: an empty closure is never trusted, and the definition keeps the regenerate-on-any-change fallback. See [the proposed changes overview](../proposed-changes/overview#understanding-artifact-regeneration) for what an incomplete closure means and how to resolve it.

The manifest itself is not in any closure by default, so for a definition with a complete closure, editing the manifest regenerates only the definitions whose own configuration the edit changed. Listing the manifest under `watch.files` is allowed and puts it in that definition's closure like any other file, which means every manifest edit - including one that only touches an unrelated entry, or only a comment - regenerates that definition. Spell the entry the way your repository spells the file, `.infrahub.yml` or `.infrahub.yaml`: a `watch.files` entry that matches no tracked file is only logged as a warning, so the wrong spelling silently leaves the manifest out of the closure.

#### Declaring `watch` with no extra files

On a Python Transformation or a Generator, declare `watch` with an empty list when the source depends on nothing but its own file:
...
See [organizing objects with groups](../groups/overview) for details on creating target groups.
...
- [How to connect external Git repositories](./connect-repository.mdx) - Step-by-step setup guide
- [Understanding Git repositories in Infrahub](./overview.mdx) - Repository integration concepts
- [Understanding groups](../groups/overview) - Organizational concepts for targeting operations
- [How to organize objects with groups](../groups/overview) - Creating and managing groups
- [Schema development](../schema/overview) - Designing infrastructure data models
- [Transformations](../transformations/overview) - Data processing and template concepts
- [Generators](../generators/overview) - Infrastructure provisioning automation
- [Artifacts](../artifacts/overview) - Output generation and delivery
