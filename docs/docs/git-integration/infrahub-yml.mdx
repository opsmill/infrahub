...
### Declaring extra transform dependencies with `watch`

Infrahub automatically detects the files a transform reads: a Python transform's package directory, and the templates a Jinja2 transform statically includes, imports, or extends. When a file in that detected set changes in a proposed change, only the artifacts of the affected transform regenerate.

Some dependencies cannot be detected automatically, such as a template pulled in through a dynamic `{% include some_variable %}` or a helper module imported at runtime from another package. Declare these with the optional `watch` key on a `jinja2_transforms` or `python_transforms` entry. Watched files are added to the transform's auto-detected dependencies; they never replace them.
...
Python example, where a transform imports helpers from a sibling top-level package:
