...
Because imports are never analyzed, Infrahub cannot distinguish a Python source with no further dependencies from one whose dependencies it has no way to see. Until `watch` is present it assumes the latter, and treats every commit to the repository as a possible change to the definition. The empty list is your statement that the auto-detected closure is complete, and it is what makes regeneration precise. The key has to carry the `files` list: a bare `watch:` with nothing under it reads as no declaration at all.
