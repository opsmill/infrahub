...
## Checking a repository's status on every branch

Listing branches tells you which ones Infrahub tracks. To see what a single repository looks like from each of those branches at once, including the commit each branch resolves and whether its last import succeeded, use the `InfrahubRepositoryBranchStatus` query. It returns one row per branch and reads everything from the graph, so it does not contact a Git server or a task worker.

The `id` argument accepts the repository's ID or its name.
...
Which branches appear depends on the repository type. A read-write repository returns only branches with `sync_with_git` set to true, which is the same set the rules on this page control. A read-only repository returns every branch, because it tracks a single reference and is not affected by selective synchronization. In both cases, merged and deleting branches are left out, and the branch you send the query to does not change the result.

### How values are resolved for each branch

A branch that has never imported the repository still gets a row, and that row is not an error. An isolated branch, which is what a branch created today is, inherits the values the default branch held at the point the branch was created. A branch created before isolation became the default reads the default branch as it stands now instead.

Values written when the repository was first added are visible on every branch, including branches that already existed, because Infrahub records them outside any single branch. A read-only repository's `commit` and `ref` are the exception: they are recorded on the branch the repository was created on, so they reach only that branch and the branches forked from it afterwards.

`sync_status__value`, `internal_status__value`, and `own_values_only` narrow the rows on the server, and `count` reflects them. Set `own_values_only` to true to keep only the branches holding their own `commit` rather than an inherited one: the branches that have imported the repository themselves, plus, for a read-only repository, the branch it was created on.

:::warning
On a row that inherits its values, `updated_at` is the time the default branch was written, not the time that branch last imported the repository. Do not present it as a last-import time. To find out whether a branch has imported the repository at all, use the `own_values_only` argument.
:::

:::info
This query always reports the present. A request that includes an `at` query parameter is rejected, because the list of branches it builds its rows from has no historical form. For a point-in-time view, query the repository object directly.
:::

### Permission requirement

The account running the query needs the `view` permission on the repository's kind, `Core/Repository` or `Core/ReadOnlyRepository`, with a decision that covers both the default branch and other branches. A single `ALLOW_ALL` grant satisfies this, and so does `ALLOW_DEFAULT` together with `ALLOW_OTHER`. Granting only one of the two is refused with an error rather than a shortened list of branches.
