...
A branch that has never imported the repository still gets a row. Its values are the ones the default branch held at the point that branch was created, which is the correct answer for that branch and is not an error.

`sync_status__value`, `internal_status__value`, and `own_values_only` narrow the rows on the server, and `count` reflects them. Set `own_values_only` to true to keep only the branches that have imported the repository themselves.
