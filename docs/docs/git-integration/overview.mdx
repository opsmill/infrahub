...
- **Check connectivity**: Validates network access and authentication without modifying repository content. The result is recorded in the repository's operational status: a successful check sets it to **Online**, while a failure sets it to the matching error state (**Error (credential)**, **Error (connection)**, or **Error**)
