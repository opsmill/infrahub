...
- **Check connectivity**: Validates network access and authentication without modifying repository content. For a read-write **Repository** the check also confirms that Infrahub can push to the remote, so credentials that can read but not push are caught here rather than at the first branch creation or merge; a failure to push is reported as a **Credential Error**. The result is recorded in the repository's operational status: a successful check sets it to **Online**, while a failure sets it to the matching error state (**Credential Error**, **Connectivity Error**, or **Error**). **Online** confirms repository-level write access only; it does not evaluate branch protection, push rules, or pre-receive hooks, which apply only when a real push is attempted
