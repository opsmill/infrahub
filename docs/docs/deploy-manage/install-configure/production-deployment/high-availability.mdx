...
<details>
  ...
  Prefect's messaging Redis (`prefect-server.backgroundServices.messaging.redis`) does not support Sentinel, so it connects to the Redis service directly and does not follow master failover. Making Prefect's messaging Redis highly available is tracked as a separate follow-up.
