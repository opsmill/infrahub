...
Do not combine a CA setting with the matching `*_TLS_INSECURE` setting: Infrahub refuses to start when both are set for git, the trace exporter or LDAP, and ignores the bundle for the other components.
...
Confirm that the task worker configured git with the bundle:

```shell
docker compose exec task-worker git config --global --get http.sslCAInfo
```
