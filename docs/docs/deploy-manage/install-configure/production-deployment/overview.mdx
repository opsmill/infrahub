...
# Only when your infrastructure uses a private CA: mount the bundle and uncomment, see "Trust a private CA" below
...
If your infrastructure uses a private certificate authority, mount its bundle into the containers, then set `INFRAHUB_TLS_CA_BUNDLE` to the mounted path. A component can override the bundle with its own `*_TLS_CA_FILE` or `*_TLS_CA_BUNDLE` setting. See [Trust a private CA](./private-ca.mdx) for the mount and verification steps.
...
A production deployment with Helm needs:

- Data persistence for the Neo4j database and the task manager PostgreSQL database
- Multiple replicas of the Infrahub API server and task workers, spread across zones with the `affinity` setting
- S3 storage for the Infrahub API server, which is required when it runs more than one replica

The dependencies bundled with the chart (Neo4j, PostgreSQL, RabbitMQ, Redis) are intended for non-production environments. In the example below, the bundled Neo4j and PostgreSQL run with persistent volumes. To remove them as single points of failure, run the dependencies outside the chart, as in the [high availability examples](./high-availability.mdx).
...
infrahubServer:
  ...
  infrahubServer:
    env:
      ...
      INFRAHUB_LOG_LEVEL: INFO
      INFRAHUB_PRODUCTION: "true"
      INFRAHUB_INITIAL_ADMIN_TOKEN: <generate-uuid>
      INFRAHUB_SECURITY_SECRET_KEY: <strong-random-string-at-least-32-chars>
...
Replace the placeholder values with your own. See the [configuration reference](/reference/configuration) for all available environment variables, and [Install Infrahub with Helm](../install/helm.mdx) for the chart versions and installation commands.
