...
# CA bundle trusted by every outbound connection (git, database, broker, cache, webhooks, SSO...)
...
If your infrastructure uses a private certificate authority, mount its bundle into the containers and point `INFRAHUB_TLS_CA_BUNDLE` at it. A component can override the bundle with its own `*_TLS_CA_FILE` or `*_TLS_CA_BUNDLE` setting. See [Trust a private CA](./private-ca.mdx) for the mount and verification steps.
