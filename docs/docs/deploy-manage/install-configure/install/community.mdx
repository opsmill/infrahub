...
By default, the Helm chart disables storage persistence on all components (Neo4j, the task manager PostgreSQL database, RabbitMQ, Redis) to enable quicker deployments using `emptyDir` storage and reduce requirements for test installations.
...
- Enable storage persistence for the Neo4j database and the task manager PostgreSQL database at minimum (but also on the other components if possible)
- Use Docker Compose instead, which provides more predictable behavior for non-production environments

The PostgreSQL database is used by the task manager (Prefect) and stores task execution history, logs, and workflow state.
...
To use a storage class other than the cluster default, add `storageClass: <name>` to the `persistence` sections of PostgreSQL, Redis, and RabbitMQ, and set Neo4j to `mode: dynamic`, as shown in the example in [Step 1](#step-1-fill-in-the-values-file). See [Task manager database (PostgreSQL) storage](../hardware-requirements.mdx#task-manager-database-postgresql-storage) to size the PostgreSQL volume.

### Production deployment requirements

The following are required for production deployments using Helm:

- Data persistence must be enabled for the Neo4j database and the task manager PostgreSQL database
...
We do not recommend using the included dependencies (Neo4j, PostgreSQL, RabbitMQ, Redis) for production.
