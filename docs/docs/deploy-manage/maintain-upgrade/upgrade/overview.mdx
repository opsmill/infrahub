...
Applied 6 migrations successfully in 1.42s
...
`infrahub db migrate` runs the same code path as **Step 1/6 of `infrahub upgrade`**, with identical formatting. Use `infrahub upgrade` when you also need to update the internal and core schema, refresh internal objects (menu, permissions), reconfigure the task manager, and rebase open branches. Use `infrahub db migrate` only when you specifically want the migration step in isolation.
...
- **GraphMigration** — applies one or more Cypher queries to the graph. The query list is shown by `db showmigration N`.
...
`infrahub db showmigration N` reports details about a specific migration, including its type, the graph version it requires, whether it has been applied, and (for `GraphMigration`) the list of Cypher queries it runs:

```console
$ infrahub db showmigration 1

Migration 001: add_version_to_graph
  Type:            GraphMigration
  Minimum version: 0
  Status:          Applied
  Source:          /source/backend/infrahub/core/migrations/graph/m001_add_version_to_graph.py

  Queries:
    1. Migration001Query01 (WRITE)
```

For `ArbitraryMigration` and `MigrationRequiringRebase` migrations the `Queries:` block is omitted because those types do not expose a query list.
