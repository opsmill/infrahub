...
:::note
In normal operation you do not need any of the `infrahub db` commands — `infrahub upgrade` takes care of everything. The commands below are intended for advanced debugging: inspecting which migrations exist, previewing what would run, or applying the migration step in isolation when investigating a failure.
:::
