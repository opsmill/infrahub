...
For the complete list of event types and their payload fields, see the [Infrahub events reference](../../../reference/infrahub-events/overview.mdx).
...
- `tls_insecure` skips certificate validation and takes precedence over `tls_ca_bundle`; restrict it to test environments
...
- **Event framework**: Log forwarding consumes the same events that power the activity log, webhooks, and event rules. See [Events](../../../events/event-system) and the [Infrahub events reference](../../../reference/infrahub-events/overview.mdx).
