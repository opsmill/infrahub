...
To skip validation of the syslog server certificate in a test environment, set `tls_insecure = true` on the destination (`INFRAHUB_LOG_FORWARDING_DESTINATION_<NAME>_TLS_INSECURE="true"` as an environment variable). It takes precedence over `tls_ca_bundle`, so the bundle can stay configured. Never enable it in production.
