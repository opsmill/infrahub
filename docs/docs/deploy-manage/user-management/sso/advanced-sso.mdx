...
- `GroupAutoCreateRejectedClaimEvent` — a claim matched but produced an invalid effective name (empty, whitespace-only)
- `GroupAutoCreateCapBreachEvent` — the per-login cap was reached and surplus claims were dropped
