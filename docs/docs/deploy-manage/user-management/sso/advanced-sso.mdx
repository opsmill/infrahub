...
Group memberships derived from provider claims are evaluated on **every** login and are **additive**: each login adds the user to the groups their claims map to. Infrahub never removes a user from a group based on claims — if a group is dropped from the provider, or an administrator removes a user from a group, that change is **not** re-synced or reverted on the next login. Revoking group membership is a manual action in Infrahub.
