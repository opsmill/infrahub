...
- [Create a group](./create.mdx) — add a new Standard group.
- [Add members to a group](./add-members.mdx) — attach objects to an existing group.
- [Remove members from a group](./remove-members.mdx) — detach objects without deleting them.
- [Delete a group](./delete.mdx) — remove a group (its members persist).
- [Query group membership](./query-members.mdx) — inspect what's in a group or what groups an object belongs to.
- [Use groups in automation](./use-in-automation.mdx) — target a group from an Artifact, Transformation, or Check.

## Learn by doing

New to groups? Walk through [Getting started with groups](../academy/getting-started/groups.mdx) in Academy. It creates a group end-to-end using a running example.
