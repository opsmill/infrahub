...
**The objects disappeared, not just the membership.** They shouldn't — removing members detaches them; it does not delete them. If objects are missing entirely, look for a cascading delete elsewhere (e.g. a Generator that owned them).
