...
As the Generator runs, Infrahub automatically tracks the objects it produces in a `CoreGeneratorGroup` via the SDK's tracking feature. You don't define this group; it's managed for you.
...
3. An **Artifact Definition** targets either:
    - the input `CoreStandardGroup` (to render config per device), or
