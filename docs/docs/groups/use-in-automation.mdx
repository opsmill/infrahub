...
Group membership decides which objects an artifact definition covers, not when Infrahub generates them. The next artifact generation run for the definition includes the new members, and a run starts only when a Proposed Change pipeline runs, a branch is merged, the definition changes, or you request one. See [when artifacts regenerate](../artifacts/overview#when-artifacts-regenerate) for what each of those means.
...
Adding a member does not start a Generator run on its own. For the five things that do, and how to run a Generator as soon as an object joins the group, see [how Generators are executed](../generators/overview#how-generators-are-executed).
