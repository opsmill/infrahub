...
Group membership decides which objects an artifact definition covers, not when it runs. A membership change is picked up by the next generation pass over the definition, and the pass has to be started by a proposed change, a merge, a change to the definition, or a request. See [when artifacts regenerate](../artifacts/overview#when-artifacts-regenerate) for the full list.
...
Adding a member does not start a Generator run on its own. To run the Generator as soon as an object joins the group, tie a group trigger rule to a Generator action. See [event rules](../events/event-rules).
