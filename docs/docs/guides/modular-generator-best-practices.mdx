...
Upsert matching relies on the node's [`human_friendly_id`](../topics/schema#human_friendly_id) (HFID). When the server processes an upsert mutation, it checks whether a node with the same HFID already exists. If it finds a match, it updates the existing node. If no match is found, it creates a new node.
...
**Cause:** The upstream Generator wrote checksums to some targets but not all — likely because it errored partway through its `update_checksum()` loop, or because it only queries a subset of downstream targets.
...
### Add `GeneratorTarget` to all triggerable nodes
...
Generators rely on `allow_upsert=True`, which matches objects by their [`human_friendly_id`](../topics/schema#human_friendly_id). Every node type that a Generator creates must have a `human_friendly_id` defined in the schema:
