...
For proper artifact detection, your query must target a **unique node** using either a Human-Friendly ID (HFID) or a unique attribute. This ensures Infrahub only regenerates the necessary artifacts instead of regenerating all artifacts unnecessarily.
