...
For proper artifact detection, your query must target a **unique node** using either an ID or a unique attribute (HFID). This ensures Infrahub only regenerates the necessary artifacts instead of regenerating all of them excessively.

**Requirements for a valid single-target query:**
- Must filter on a unique identifier (ID or unique attribute like `name`)
- Must use a required variable (e.g., `$name: String!`) or a static value
- Must use exact match filters (e.g., `name__value: $name`), not list filters (e.g., `name__values: $name`)
...
**Invalid example (will cause excessive artifact generation):**
```graphql
query BuiltinTag {
  BuiltinTag {
    edges { node { id } }
...
To verify your query is valid, you can use the `InfrahubGraphQLQueryAnalyzer` in the Infrahub shell:
