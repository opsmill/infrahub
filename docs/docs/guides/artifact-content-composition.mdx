...
This guide shows you how to build a composite [artifact](../topics/artifact.mdx) whose [Transformation](../topics/transformation.mdx) pulls in content from other artifacts or [file objects](../topics/file-object.mdx). This enables modular configuration pipelines where each artifact generates one section of a configuration, and a composite artifact assembles the final result.
...
- An [external repository](../topics/repository.mdx) connected to Infrahub
- Familiarity with [creating a Jinja2 Transformation](./jinja2-transform.mdx) or [creating a Python Transformation](./python-transform.mdx)
- Familiarity with [generating artifacts](./artifact.mdx)

## 1. Understand the composition model

During artifact generation, your Jinja2 templates have access to Infrahub filters that can fetch content from the [object storage](../topics/object-storage.mdx). To build a composite artifact, your Transformation's GraphQL query retrieves the `storage_id` of another artifact or file object, and your template uses a filter to fetch and include that content inline.
...
The `artifact_content`, `file_object_content`, `file_object_content_by_id`, and `file_object_content_by_hfid` filters are only available during artifact generation on the task worker. They are not available in [computed attributes](../topics/computed-attributes.mdx) or when rendering locally with `infrahubctl render` without a connected server.
...
In addition to artifacts, you can also include content from [file objects](../topics/file-object.mdx). There are several filters available depending on how you identify the file:
...
You can also retrieve file object content through additional methods on the object store:

- `await self.client.object_store.get_file_by_storage_id(storage_id=...)`
- `await self.client.object_store.get_file_by_id(node_id=...)`
- `await self.client.object_store.get_file_by_hfid(kind=..., hfid=[...])`

## 5. Register the composite Transformation

Once you have created your query and template files, define the query, Transformation, and artifact definition in your [`.infrahub.yml`](../topics/infrahub-yml.mdx) file:
...
3. **Text content only**: Only text-based content can be composed. Binary files (images, PDFs) are not supported by these filters.
...
- [Creating a Jinja2 Transformation](./jinja2-transform.mdx)
- [Creating a Python Transformation](./python-transform.mdx)
- [How to generate artifacts](./artifact.mdx)
- [Using the object storage](./object-storage.mdx)
- [File objects](../topics/file-object.mdx)
