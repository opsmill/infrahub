---
title: Setting up log forwarding
---
...
# Setting up log forwarding <EnterpriseBadge />

This guide walks through configuring Infrahub to forward audit events and application logs to external SIEM or syslog systems. For conceptual background on how log forwarding works, see the [log forwarding topic](../topics/log-forwarding.mdx).
...
`INFRAHUB_LOG_FORWARDING_DESTINATION_NAMES` and `INFRAHUB_LOG_FORWARDING_DESTINATIONS` are mutually exclusive. Use one or the other, not both. If both are set, Infrahub will raise a validation error on startup.
...
By default, only audit events are forwarded. Application log forwarding adds exception events such as permission denied (`infrahub.permission.denied`) to the forwarded stream. These are sent as `APPLICATION_LOG` messages through the `forward_exception()` path and require explicit opt-in per destination:
...
min_log_severity = "WARNING"
```

The `min_log_severity` field filters which messages are forwarded. Options are: `DEBUG`, `INFO`, `WARNING` (default), `ERROR`, `CRITICAL`.

Application log messages use syslog facility `LOG_LOCAL0` (16), while audit events use `LOG_AUTH` (4). This distinction helps receivers separate and route the two categories independently.
...
- [Log forwarding concepts](../topics/log-forwarding.mdx)
...
- [Activity log](../topics/activity-log.mdx)
