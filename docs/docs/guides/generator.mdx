...
:::info
Any widget object must be added to or part of the **widgets** group to be tied to the generator.
:::
...
<Tabs groupId="convertResponse">
  ...
  <TabItem value="Converted query response">
    Here we must provide `__typename` and `id` within the query so the SDK can convert the query to the correct type and properly stored in the SDK.
