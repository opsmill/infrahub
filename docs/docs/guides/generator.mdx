...
- Create a generator that reads data from Infrahub using GraphQL
- Generate new objects based on that data
- Run the generator both locally and through Infrahub's CI pipeline

## What you'll build

You'll build a simple generator that:
...
Since the default Infrahub schema doesn't include the object types we need for this example, we'll first create a simple schema with Widget and Resource nodes.
...
After merging your changes, confirm that the generator is imported correctly by checking the Infrahub UI.
