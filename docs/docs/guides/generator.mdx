...
1. Create an entry for the generator within an .infrahub.yml file.
2. Identify the relevant data you want to extract from the database using a [GraphQL query](/topics/graphql), that can take an input parameter to filter the data
3. Write a Python script that uses the GraphQL query to read information from the system and generates new data based on the response
4. Test the generator with infrahubctl
...
## 4. Test the generator using infrahubctl
...
Load the following schema using the [infrahubctl branch](/infrahubctl/infrahubctl-branch) command.
...
Before we can run our generator in an Proposed Change (PC) pipeline we must add the files to a local Git repository and push them up to a remote Git repository.
...
With this step completed you can [add your repository to Infrahub](/guides/repository) and then the generators will be executed as part of the proposed change CI pipeline.

## 6. Generator in the CI pipeline
