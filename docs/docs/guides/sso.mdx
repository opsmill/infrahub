...
# Optional: Filter which groups are created using a regex pattern
...
:::info
When the filter pattern contains a capture group, the first captured value becomes the group name in Infrahub. Groups that don't match the pattern are ignored.
:::
