...
This guide walks you through configuring single sign-on (SSO) in Infrahub using Open ID Connect (OIDC) or OAuth2 authentication protocols.
...
:::info
OIDC provides standardized user information and is recommended for new implementations. See [Authentication methods](../topics/authentication) for details on the differences between OIDC and OAuth2.
:::
...
:::info
We cover common identity providers, but if yours isn't listed, refer to your provider's documentation for similar steps.
:::
...
4. Enter a name for your application (e.g., "Infrahub")
...
| Display Label | This is cosmetic: `Microsoft Entra ID` |
| Icon | This is cosmetic: `mdi:microsoft` |
</TabItem>
</Tabs>
...
- Configure each provider using the same steps as above, but with the appropriate slot name (e.g., `provider1`, `provider2`)
- Make sure to set the `INFRAHUB_SECURITY_OIDC_PROVIDERS` or `INFRAHUB_SECURITY_OAUTH2_PROVIDERS` environment variable to include all configured providers
...
- [Authentication methods](../topics/authentication)
- [Configuration reference](../reference/configuration)
- [Security best practices](../topics/security)
