...
#### Step 4: Enable automatic group creation (optional)
...
:::info
When a filter pattern contains a capture group, the first captured value becomes the group name in Infrahub. If you configure multiple patterns, Infrahub evaluates them in order and uses the first matching pattern for each group. Groups that do not match any pattern are ignored.
:::
