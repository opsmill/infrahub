...
:::info
By default, Infrahub requires you to create groups manually before SSO users can be assigned to them. However, you can enable automatic group creation — see [Step 4: Enable automatic group creation](#step-4-enable-automatic-group-creation-optional) below.
:::
...
:::success
Now that group mapping is configured, manage user permissions in Infrahub by [assigning permissions and roles](./accounts-permissions.mdx) to these groups.
:::

#### Enable automatic group creation (optional)

Instead of manually creating groups, you can configure Infrahub to automatically create missing groups when SSO users authenticate.

:::warning
Automatically created groups have no permissions by default. You'll need to assign permissions to these groups after they're created.
:::

:::info
Infrahub adds group memberships during SSO login, but it does not remove existing memberships when your identity provider stops sending a group claim for a user.
:::
...
# Optional: Filter which groups are created using one regex pattern
# This example creates groups starting with "team-"
export INFRAHUB_SECURITY_SSO_GENERATE_GROUPS_FILTER='^team-.*'

# Optional: Filter with multiple patterns (JSON array, evaluated in order)
...
# Optional: Filter with one regex pattern
# This example creates groups starting with "team-"
sso_generate_groups_filter = "^team-.*"

# Optional: Filter with multiple patterns (evaluated in order)
...
##### Using capture groups to transform group names

If your identity provider sends groups with prefixes or paths (for example `ldap/groups/network_automation`), you can use regular expression (regex) capture groups to extract the relevant part as the group name:
<Tabs groupId="config-method">
<TabItem value="environment-variables" default>

```bash
export INFRAHUB_SECURITY_SSO_GENERATE_GROUPS='true'
# Extract group name from LDAP-style paths
# "ldap/groups/network_automation" creates a group named "network_automation"
export INFRAHUB_SECURITY_SSO_GENERATE_GROUPS_FILTER='ldap/groups/(\w+)'
```
...
:::info
Filter patterns use Python `re.match` semantics, so matching starts at the beginning of the SSO group identifier. When a filter pattern contains a capture group, the first captured value becomes the group name in Infrahub. If you only want grouping inside the regex without renaming the group, use a non-capturing group such as `(?:dev|prod)`. If you configure multiple patterns, Infrahub evaluates them in order and uses the first matching pattern for each group. Groups that do not match any pattern are ignored.
:::

## Related resources

- [Authentication topic](../topics/authentication.mdx)
- [SSO reference](../reference/sso.mdx)
- [Permissions and roles topic](../topics/permissions-roles.mdx)
