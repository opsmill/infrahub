...
1. Configure Infrahub to always create a merge commit by setting the following environment variables:
...
2. Restart the Infrahub stack to apply the changes.

3. Verify the configuration is applied by checking the environment variables in the `task-worker` container.
