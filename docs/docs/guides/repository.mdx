...
Unlike CoreRepositories, Infrahub does not automatically update Read-only Repositories with changes from the external repository. To pull in changes from the external repository you must either set the `ref` of the Read-only Repository to the desired value.
