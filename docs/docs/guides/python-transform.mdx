...
The next step is to create a query that returns the data we just created.
...
While it would be possible to create a transform that targets all devices, the goal is to be able to perform a transformation on a single device. For this reason we need to modify the query from above to take an input parameter so that we can filter the result to what we want.
...
Then save the query below as a text file named `device_config_render/device_config.gql` to target a specific device.
      ...
      "edges": [
        {
          "node": {
            "__typename": "NetworkDevice",
            "id": "184907fb-8fd7-b580-300f-c51502251311",
            "name": {
              "value": "switch1"
            },
            "description": {
...
With this configuration, the endpoint of our transform will be [http://localhost:8000/api/transform/python/device_config_json](http://localhost:8000/api/transform/python/device_config_json).
...
device_config_json (device_config.py::DeviceConfigTransform)
```

:::info
Trying to run the transform with just the transform name will produce an error.

```shell title="❯ infrahubctl transform device_config_json"
...
```json title="❯ infrahubctl transform device_config_json name=switch2"
{
...
```json title="❯ curl http://localhost:8000/api/transform/python/device_config_json?name=switch2"
{
  "device_hostname":"switch2",
  "device_description":"*This is device switch2*"
}
...
```json title="❯ curl http://localhost:8000/api/transform/python/device_config_json?name=switch3"
{
