...
:::note
You should see interface fields value automatically populated when selecting the profile.
:::
...
3. Fill in the interface details as follows:
    ...
    - **MTU**: 9000 (this override MTU value)
4. Click **Save** to create the interface
...
3. Click the "info" logo next to **MTU** to see the metadata
...
mutation {
  ProfileInfraInterfaceUpdate(
    data: {
      ...
      mtu: {value: 9000} # Change MTU the profile
    }
  ) {
    ok
  }
...
Learn more about profile priorities in the [Understanding profiles in Infrahub](../topics/profiles) topic.
...
:::note
You should see interface fields value automatically populated when selecting the profiles.
:::
...
- [Understanding profiles in Infrahub](../topics/profiles) - Detailed explanation of profile concepts, inheritance rules, and design decisions
...
- [Node schema reference](../reference/schema/node) - Complete reference for node configuration including the `generate_profile` option
...
- [Python SDK documentation](https://docs.infrahub.app/python-sdk) - Full SDK reference for programmatic profile operations
