...
To quickly spin up the latest Infrahub locally:
...
```shell
curl https://infrahub.opsmill.io/stable | docker compose -f - up -d
```

Alternatively, you could indicated `develop` for or any version in the URL:

```shell
curl https://infrahub.opsmill.io/develop | docker compose -f - up -d
curl https://infrahub.opsmill.io/1.0.7 | docker compose -f - up -d
```
...
```shell
curl https://infrahub.opsmill.io/stable | sudo docker compose -f - up -d
```
...
curl https://infrahub.opsmill.io/1.0.7 | sudo docker compose -f - up -d
```
...
For the different versions, you can check our [release notes, on GitHub](https://github.com/opsmill/infrahub/releases).
