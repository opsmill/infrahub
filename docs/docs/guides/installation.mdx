...
:::warning Upgrading the observability stack

When upgrading to a new compose file, services that use Docker Compose `configs` will not pick up configuration changes on a regular `up -d`. You must use `--force-recreate` on the affected services to reload the embedded config content:

```bash
docker compose -p infrahub up -d --force-recreate alloy loki tempo grafana
```

:::

:::tip Enable distributed tracing

To enable distributed tracing, set the following environment variables so that Infrahub exports traces to the Tempo instance via gRPC. Add them to a `.env` file alongside the compose file:

```bash
INFRAHUB_TRACE_ENABLE=true
INFRAHUB_TRACE_EXPORTER_TYPE=otlp
INFRAHUB_TRACE_EXPORTER_PROTOCOL=grpc
INFRAHUB_TRACE_EXPORTER_ENDPOINT=tempo:4317
INFRAHUB_TRACE_INSECURE=true
```

Traces will appear in Grafana under the Tempo datasource.
