...
`INFRAHUB_SECURITY_SECRET_KEY` must be explicitly set — the server refuses to start without it. Use a strong value and share it across every replica so authentication tokens issued by one instance validate on the others. See the [Security reference](../reference/configuration.mdx#security) for details.
