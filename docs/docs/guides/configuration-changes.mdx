...
Configure the environment variable before starting or restarting your Infrahub containers.
...
The method you choose depends on your Kubernetes deployment approach. ConfigMaps are recommended for non-sensitive configuration values.

:::

### Option A: Use a ConfigMap (recommended)

Create or update a ConfigMap to store your configuration:

```yaml title="infrahub-config.yaml"
apiVersion: v1
kind: ConfigMap
metadata:
...
```bash
kubectl apply -f infrahub-config.yaml -n <your-namespace>
```
...
### Option B: Set directly in Deployment

Add the environment variable directly in your Deployment manifest:
...
spec:
  template:
    spec:
      ...
      - name: infrahub
        env:
        - name: INFRAHUB_HTTP_TIMEOUT
          value: "20"
```

### Option C: Use Helm values (if using Helm)
...
helm upgrade infrahub opsmill/infrahub \
  --set env.INFRAHUB_HTTP_TIMEOUT=20 \
  -n <your-namespace>
```
...
The restart method depends on whether you used ConfigMaps, direct Deployment edits, or Helm.

:::

### For ConfigMap or Deployment changes

If you modified a ConfigMap or Deployment manifest, apply the changes:

```bash
kubectl apply -f deployment.yaml -n <your-namespace>
```

Then trigger a rollout restart to pick up the new configuration:

```bash
kubectl rollout restart deployment/infrahub-server -n <your-namespace>
```

Monitor the rollout status:

```bash
kubectl rollout status deployment/infrahub-server -n <your-namespace>
```
