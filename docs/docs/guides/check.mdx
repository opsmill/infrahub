...
Checks in Infrahub allow you to validate data based on custom business logic. They are defined in [external repositories](../topics/repository.mdx) and can be developed and tested locally using [infrahubctl check]($(base_url)infrahubctl/infrahubctl-check).

:::info Global vs. Targeted Checks

If you want to know more about the differences between these two types, refer to the topic on [Global vs. Targeted Checks](../topics/check.mdx).
...
- Familiarity with Checks and how they work in Infrahub (see [Checks topic](../topics/check.mdx))
...
Now that you have your GraphQL query and Python check, edit your [.infrahub.yml](../topics/infrahub-yml.mdx) file to tie everything together.
