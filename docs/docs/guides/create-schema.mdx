...
Create a file named `schema_guide.infrahub.yml` on your computer. The place where you create the file on the file system is not that important, as long as you know the path to the file. For this guide we will be storing the schema file in the `/tmp` directory.

:::note

Schemas should be named with the extension `<FILENAME>.infrahub.yml` to take advantage of [automatic schema validation](../topics/schema.mdx#schema-validation)
...
```bash
infrahubctl schema load --branch network-device-schema /tmp/schema_guide.infrahub.yml
```
...
Overwrite the `schema_guide.infrahub.yml` with the following contents:
...
```bash
infrahubctl schema load --branch network-device-relations /tmp/schema_guide.infrahub.yml
```
...
Overwrite the `schema_guide.infrahub.yml` file with the following contents:
...
```bash
infrahubctl schema load --branch network-device-generics /tmp/schema_guide.infrahub.yml
```
...
Overwrite the contents of the `schema_guide.infrahub.yml` file with this content:
...
```bash
infrahubctl schema check --branch network-device-generics /tmp/schema_guide.infrahub.yml
```

```bash
 schema '/tmp/schema_guide.infrahub.yml' is Valid!
diff:
        added: {}
        changed:
                NetworkDevice:
