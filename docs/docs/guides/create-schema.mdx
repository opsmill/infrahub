...
First, we will create a NetworkDevice `atl1-edge1` via the NetworkDeviceCreate mutation.

:::note

The id's returned from the result of this mutation need to be used within the next mutation when creating the interface, they will be different then the id's we use here in the guide
...
Second, obtain the **id** that was returned when creating `atl1-edge1` and update the NetworkInterfaceCreate mutation below.

```graphql
mutation {
  NetworkInterfaceCreate(data: {name: {value: "Ethernet1"}, description: {value: "WAN interface"}, device: {id: "17cbdc5e-466c-9161-3373-c515c808ec4d"}}) {
        ok
        object {
          id
        }
...
In the Web UI we can now see that the `atl1-edge1` device has a relation to the `Ethernet1` interface.
