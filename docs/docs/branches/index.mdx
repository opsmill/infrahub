...
- **Rebase before creating a Proposed Change**: Update your branch with the latest changes from the default branch to identify and resolve conflicts early. See [Rebase a branch](./rebase).
...
1. **Branch creation**: Establishes an isolated workspace at a specific point in time, capturing the state of the parent branch as a starting point. See [Create a branch](./create).
...
4. **Conflict resolution**: Any conflicts with the main branch are identified and resolved, ensuring smooth integration of changes. See [Resolve conflicts](./resolve-conflicts).
...
- [Git Integration](../git-integration/index.mdx)
- [Proposed Changes](../proposed-changes/index.mdx)
- [Immutable History](../topics/version-control.mdx)
- [Schema](../topics/schema.mdx)
