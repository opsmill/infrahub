...
Once changes are complete, they merge back into the parent branch through a controlled process — almost always via a [Proposed Change](../proposed-changes/index.mdx). This ensures that all modifications are validated and conflicts are resolved before integration.
...
- [Proposed Changes](../proposed-changes/index.mdx) — the recommended path for merging
