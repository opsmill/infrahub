...
Because a branch-agnostic value is stored once for every branch, deleting the object that holds it on a single branch does not remove the value. It is released only once no branch can still read that object, which may happen through a delete, a merge, a rebase or a branch deletion — see [Deleting a branch-agnostic attribute or relationship](../schema/branch-awareness#deleting-a-branch-agnostic-attribute-or-relationship).
