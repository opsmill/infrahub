...
Because a branch-agnostic value is stored once and shared by every branch, deleting the object that holds it on a single branch does not remove the value. It is released only once no branch can still read that object, which may take a later delete, merge, rebase or branch deletion — see [Deleting a branch-agnostic attribute or relationship](../schema/branch-awareness#deleting-a-branch-agnostic-attribute-or-relationship).
...
- [Branch awareness](../schema/branch-awareness)
