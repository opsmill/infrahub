...
When creating or editing Excalidraw diagrams (`.excalidraw` files) in the repository, always ensure the "Embed Scene" option is enabled before saving. This allows the diagrams to be properly editable in VS Code using the Excalidraw extension.

:::warning
Always save Excalidraw files with the "Embed Scene" option checked to maintain editability in VS Code.
:::
