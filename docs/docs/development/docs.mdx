...
To ensure that Infrahub's screenshots remain up to date and to check that our guides work properly, we use the Python end-to-end (e2e) test suite. You'll find the e2e tests specifically designed for tutorials and guides located in `tests/e2e/tutorial`.

### Updating all screenshots manually

#### 1. Run the tutorial e2e tests with screenshots enabled
...
```shell
UPDATE_DOCS_SCREENSHOTS=1 uv run pytest -c tests/e2e/pytest.ini tests/e2e/tutorial
```
