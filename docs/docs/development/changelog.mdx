...
Infrahub follows the best practices from [Keep a Changelog](https://keepachangelog.com/), and our categories of change are:
    ...
    - **Housekeeping** for internal maintenance and tooling (build, CI, dev scripts) — an Infrahub addition beyond the Keep a Changelog set
...
1. This file should be named with the format `<github issue id>.<change type>.md`. For example:
    ...
    - *Note*: Use the GitHub issue number, or `+` if there is no GitHub issue — never an internal ticket ID. `towncrier`'s `issue_format` turns the fragment name directly into a GitHub issue URL, so an internal ID ships as a broken link in the changelog.
1. Available Newsfragment suffixes are:
    ...
    - `security`
    - `housekeeping`
    - *Note*: dependency-version bumps are recorded as `changed` (see the `upgrade-backend-dependencies` command), not `housekeeping`. Reserve `housekeeping` for tooling, build, and CI maintenance.
