...
Setting `INFRAHUB_IMAGE_VER=local` is important because it tells Docker to do a fresh build and to bind mount the Infrahub repository inside the containers (this ensures containers are in sync with your local code).
...
When following a guide, like the [installation guide](/guides/installation.mdx), the command `demo.start` is mentioned. It is slightly different from the `demo.deps` that is mentioned here. The `demo.start` will bring up a demo environment as a whole including services and Infrahub while the `dev.deps` will only start the services as seen in the code block above.
