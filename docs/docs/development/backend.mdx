...
When following a guide, like the [installation guide](../deploy-manage/install-configure/install/overview), the command `demo.start` is mentioned. It is slightly different from the `dev.deps` that is mentioned here. The `demo.start` will bring up a demo environment as a whole including services and Infrahub while the `dev.deps` will only start the services as seen in the code block above.
...
When a test needs to change a value in `config.SETTINGS` (for example to exercise behavior gated
behind `schema_strict_mode`), do not mutate the setting globally — that leaks the change into other
tests in the module. Instead use a fixture with a `yield` so the original value is always restored,
...
A test then requests the fixture by name (`def test_something(self, disable_schema_strict_mode): ...`)
and the setting is scoped to that test only.
