...
This is particularly useful when multiple [Transformations](../transformations/) or [Generators](../generators/) need to query the same fields from a model. Update the fragment once, and all queries that reference it pick up the change on their next execution.
...
- Familiarity with [GraphQL queries](./graphql/) in Infrahub
...
- [GraphQL in Infrahub](./graphql/) — query format, stored queries, and endpoint reference
