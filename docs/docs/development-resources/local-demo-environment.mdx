...
| ------------------- | ------------------------------------------------------ | ------------------------------------------------------ |
| **database**        | `neo4j:2026.05.0-enterprise`                           | Graph Database                                         |
...
<ReferenceLink title="Check the architecture diagram to have more information about each component" url="../overview/architecture" />
...
- [uv](https://docs.astral.sh/uv/), which installs [pyinvoke](https://www.pyinvoke.org/) with the other Python dependencies
...
Clone the Infrahub repository with its submodules, then install the Python dependencies:
...
Run the `invoke` commands on this page from the repository directory with `uv run`, so that they use the environment uv created, for example `uv run invoke demo.start`.
