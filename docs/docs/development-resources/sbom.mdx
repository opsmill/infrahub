...
```shell
export IMAGE="registry.opsmill.io/opsmill/infrahub:latest"
...
```shell
export IMAGE="registry.opsmill.io/opsmill/infrahub-enterprise:latest"
...
Use the exact image reference you deploy. The `latest` tag points to the most recent release. To verify a specific version, substitute its tag; the released versions are listed on the [releases page](https://github.com/opsmill/infrahub/releases). Both the signature and the attestations are attached to the image digest, so what you verify matches the version you run.

These patterns cover release images, which are built from `infrahub-v` and `infrahub-enterprise-v` tags. The `latest` tag is a release image and verifies with them. Images published from a branch are not: `develop`, `stable` and the `dev-` tags are built on a schedule from a branch ref, so despite its name `stable` is a nightly build rather than a release. To verify one of those, replace the `@refs/tags/` portion of the pattern with the branch ref it was built from. A failed verification reports the identity it found, which tells you the value to use.
...
```text
Verification for registry.opsmill.io/opsmill/infrahub:latest --
