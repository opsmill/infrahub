...
- [Cosign](https://docs.sigstore.dev/system_config/installation/) installed locally.
...
cosign verify-attestation \
  ...
  --certificate-oidc-issuer "https://token.actions.githubusercontent.com" \
  "$IMAGE" \
  | jq -r '.payload' | base64 -d | jq '.predicate' > infrahub-sbom.spdx.json
```
...
cosign verify-attestation \
  ...
  --certificate-oidc-issuer "https://token.actions.githubusercontent.com" \
  "$IMAGE" \
  | jq -r '.payload' | base64 -d | jq '.predicate' > infrahub-sbom.cdx.json
```
...
cosign download attestation "$IMAGE" \
  | jq -r 'select(.payload | @base64d | fromjson | .predicateType == "https://spdx.dev/Document") | .payload' \
  | base64 -d | jq '.predicate' > infrahub-sbom.spdx.json

# CycloneDX
cosign download attestation "$IMAGE" \
  | jq -r 'select(.payload | @base64d | fromjson | .predicateType == "https://cyclonedx.org/bom") | .payload' \
