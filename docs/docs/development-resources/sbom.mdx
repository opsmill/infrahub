...
```shell
export IMAGE="registry.opsmill.io/opsmill/infrahub:1.11.2"
...
```shell
export IMAGE="registry.opsmill.io/opsmill/infrahub-enterprise:1.11.2"
...
Use the exact image reference you deploy. Both the signature and the attestations are attached to the image digest, so what you verify matches the version you run.

These patterns cover release images, which are built from `infrahub-v` and `infrahub-enterprise-v` tags. Images published from a branch, such as `develop` or a `dev-` tag, are signed by the same workflow at a branch ref instead. To verify one of those, replace the `@refs/tags/` portion of the pattern with the branch ref it was built from. A failed verification reports the identity it found, which tells you the value to use.
...
```text
Verification for registry.opsmill.io/opsmill/infrahub:1.11.2 --
