...
It then prints the signed payloads as JSON. Entries with the types `https://spdx.dev/Document` and `https://cyclonedx.org/bom` are the SBOM attestations covered in the next section: cosign v3 returns them next to the image signature.
