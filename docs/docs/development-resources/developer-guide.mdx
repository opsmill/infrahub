...
- [**Transformations** (Python and Jinja2)](../transformations/overview): Used to generate [artifacts](../artifacts/overview) and define [Computed Attributes](../computed-attributes/overview.mdx).
- [**Generators**](../generators/overview): Automate complex tasks by generating outputs dynamically.
- [**Checks**](../checks/overview): Validate your infrastructure with custom rules.
...
Refer to the [Testing Framework](../testing-framework/overview) for details on validating Transformations, Checks, and other resources inside Infrahub's CI pipeline.

For integration tests that run against a full Infrahub instance in Docker, see [Integration testing with Testcontainers](../testing-framework/testcontainers).
