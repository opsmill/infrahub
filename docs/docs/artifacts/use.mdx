...
Generate configuration files and other artifacts by combining Infrahub data with templates. This guide shows you how to define an artifact for every member of a group, and how to read the generated content. For what starts a regeneration, see [when artifacts regenerate](./overview#when-artifacts-regenerate).

For conceptual information about artifacts and their architecture, see [Artifacts](./overview).

> Assumes a working Infrahub instance, an existing Transformation ([Jinja2](../learn/tutorials/transformations/build-a-jinja2-transformation) or [Python](../learn/tutorials/transformations/build-a-python-transformation)), permission to create and modify schemas, and a Git repository connected to Infrahub.
...
Infrahub does not regenerate an artifact when the data behind it changes. To pick up a change without opening a Proposed Change, request a run:

1. To regenerate every member of the group, open the artifact definition and click **Generate**.
2. To regenerate one artifact, open that artifact and click **Re-generate**.

For everything else that starts a run, see [when artifacts regenerate](./overview#when-artifacts-regenerate).
...
- ✓ the content changes after you click **Generate** on the artifact definition
...
- [Write a Python Transformation](../learn/tutorials/transformations/build-a-python-transformation) — for non-template logic
