...
5. **Not refreshed after a merge changes the source**: If artifact B inlines content from artifact A and a merge changes A, B keeps the stale inlined content. Post-merge regeneration selects the artifacts the merge affected, and nothing records that one artifact's content is embedded in another, so B is not selected. Regenerate B explicitly, or set `INFRAHUB_SELECTIVE_EXECUTION_AFTER_MERGE` to `false` to regenerate every artifact after each merge.
