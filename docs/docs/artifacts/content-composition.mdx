...
This guide shows you how to build a composite [artifact](./) whose [Transformation](../transformations/) pulls in content from other artifacts or [file objects](../schema/file-object). This enables modular configuration pipelines where each artifact generates one section of a configuration, and a composite artifact assembles the final result.
...
The `artifact_content`, `file_object_content`, `file_object_content_by_id`, and `file_object_content_by_hfid` filters are only available during artifact generation on the task worker. They are not available in [computed attributes](../computed-attributes/index.mdx) or when rendering locally with `infrahubctl render` without a connected server.
...
- [Use artifacts](./use)
- [Object storage](../artifact-file-storage/index.mdx)
