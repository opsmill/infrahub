...
```bash
uv tool run --from 'copier' copier copy -r bgi-invoke-schema-library https://github.com/opsmill/infrahub-template infrahub-automation
```

If unsure about the options, accept the default values for all prompts by pressing **Enter**.
...
The first run takes a few minutes while container images are downloaded.

2. Open your browser and go to [http://localhost:8000](http://localhost:8000).

3. Log in using the bottom left corner with the default credentials:
...
3. Copy the base schema into your project's `schemas/`:

:::info
As you have access to the full schema code, you can explore and modify the schemas to fit your organization's needs.
:::
...
4. Load the base schema into Infrahub:

:::info
`infrahubctl` is the CLI tool for developing with Infrahub. The `schema load` command reads schema definitions from a folder and loads them into Infrahub via the API.
:::
...
5. Add a location extension schema on top of the base:
...
There are multiple ways to create data in Infrahub — the WebUI, APIs, Python SDK... For development purposes, it's often easiest to create YAML files and load them with `infrahubctl`.
...
<Tabs groupId="branching-method">
  ...
  Create a `add-locations` branch using the branch selector in the top-left corner of the WebUI.
  </TabItem>
</Tabs>
...
<Tabs groupId="branching-method">
  ...
  ```bash
  uv run infrahubctl object load sites.yml --branch add-locations
  ```
