...
1. A [node trigger rule](../events/event-rules#configure-a-node-trigger-rule) matches the mutation that creates the input data and fires a group action that adds the node to the Generator definition's target group.
