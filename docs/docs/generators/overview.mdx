...
Inside a proposed change, Generator runs and artifact generation run concurrently, so an artifact that reads a Generator's output can render before that output exists (see [Ordering between Generators and artifacts](../proposed-changes/overview#ordering-between-generators-and-artifacts)). To avoid this, run the Generator on the branch before the proposed change is opened. Chain two event rules:
...
Create both rules on the default branch, and set `branch_scope` to `other_branches` so the chain fires on the branches where a proposed change will review the result. A rule created on another branch does not fire (see [where trigger rules are stored and evaluated](../events/event-rules#where-trigger-rules-are-stored-and-evaluated)).

Set `execute_in_proposed_change: false` on the Generator, so that it does not also run inside the proposed-change pipeline. This flag does not gate event-triggered runs.
