...
5. Automatically after a merge
   ...
   The Generators the merge affected run again on the target branch, unless the Generator sets `execute_after_merge` to `false`. See [Execution lifecycle](#execution-lifecycle).

A data change or a new group member does not start a run by itself. One of the paths above has to run the Generator: the proposed change pipeline, the post-merge run, a manual run, or a trigger rule that matches. On the default branch, with no proposed change involved, that leaves a manual run or a trigger rule scoped to the default branch.
...
The membership change itself does not start a run. To run the Generator against a member as soon as it joins the group, tie a group trigger rule to a Generator action. See [event rules](../events/event-rules) and [event actions](../events/event-actions).
