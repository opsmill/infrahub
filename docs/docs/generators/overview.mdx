...
4. Automatically via Events and Actions
   ...
   Configure a group trigger rule on the target group, or a node trigger rule on a node kind, and tie it to a Generator action. Adding a member to the group then runs the Generator against that member. Each rule has a `branch_scope` that decides which branches it fires on, and it defaults to the default branch. See [event rules](../events/event-rules) and [event actions](../events/event-actions).
5. Automatically after a merge

   After a merge, Infrahub runs the Generators the merge affected again on the target branch, unless `execute_after_merge` is `false` in the Generator's `.infrahub.yml` entry. Set [`INFRAHUB_SELECTIVE_EXECUTION_AFTER_MERGE`](../reference/configuration) to `false` to run every Generator after every merge instead. For what each flag controls, see [Execution lifecycle](#execution-lifecycle).

:::info What does not start a Generator run

Nothing outside those five paths starts a run. A data change, or an object joining the target group, does not run a Generator on its own. On the default branch, with no Proposed Change involved, that leaves a manual run, a run from `infrahubctl`, or a trigger rule scoped to the default branch.

Rebasing a branch is the exception worth knowing about. A rebase replays the branch's changes as object mutations, and a node trigger rule does not filter those out, so a rebase can fire a Generator action.
...
The membership change itself does not start a run. To run the Generator against a member as soon as it joins the group, tie a group trigger rule to a Generator action, as described in [how Generators are executed](#how-generators-are-executed).
...
## Running a Generator from events

Inside a proposed change, Generator runs and artifact generation start at the same time, so an artifact that reads a Generator's output can render before that output exists (see [Ordering between Generators and artifacts](../proposed-changes/overview#ordering-between-generators-and-artifacts)). Run the Generator on the branch before you open the proposed change. Chain two event rules:

1. A [node trigger rule](../events/event-rules#configure-a-node-trigger-rule) matches the mutation that creates the input data and fires a group action that adds the object to the Generator definition's target group.
2. A [group trigger rule](../events/event-rules#configure-a-group-trigger-rule) on that group matches the member-added event and fires a [Generator action](../events/event-actions#configure-a-generator-action) that runs the Generator against the new member.

The Generator runs only for the objects that are members of the Generator definition's target group. If none of the triggering objects are members, the run fails with `Target <id> is not part of the group <id>`.

Create both rules on the default branch, and set `branch_scope` to `other_branches` so the chain fires on the branches where you review the result in a proposed change. A rule created on another branch does not fire (see [where trigger rules are stored and evaluated](../events/event-rules#where-trigger-rules-are-stored-and-evaluated)).

Set `execute_in_proposed_change: false` on the Generator, so that it does not also run inside the proposed-change pipeline. The flag applies only to that pipeline: it does not prevent an event from running the Generator.
...
For a step-by-step walkthrough that builds a Generator from scratch, see [Build your first Generator](../learn/tutorials/generators/build-your-first-generator) in the Academy tutorials.
