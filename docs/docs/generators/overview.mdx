...
When an artifact generated in the same proposed change reads the Generator's output, run the Generator on the branch before the proposed change instead of in its pipeline, where the two run concurrently (see [Ordering between Generators and artifacts](../proposed-changes/overview#ordering-between-generators-and-artifacts)). Chain two event rules:
...
The Generator action's definition must target the same group the rule watches; Infrahub rejects the run otherwise. Create both rules on the default branch and set `branch_scope` to `other_branches`, so the chain fires on branches where a proposed change will review the result: rules are read from the default branch only, and a rule created on another branch never fires (see [where trigger rules are stored and evaluated](../events/event-rules#where-trigger-rules-are-stored-and-evaluated)).

Set `execute_in_proposed_change: false` and `execute_after_merge: false` on the Generator. The two flags do not apply to event-triggered runs, so leaving them `true` runs the Generator both from the event and in the pipeline.
