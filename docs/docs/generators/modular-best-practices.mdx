...
For foundational concepts, see [modular Generators](./modular). For the chaining mechanism, see [Build chained generators](../learn/tutorials/generators/build-chained-generators).
...
Upsert matching relies on the node's [`human_friendly_id`](../schema/nodes-and-attributes#human-friendly-identifier-hfid) (HFID). When the server processes an upsert mutation, it checks whether a node with the same HFID already exists. If it finds a match, it updates the existing node. If no match is found, it creates a new node.
...
|------------------------------------|----------------------------------------------------------------------------------------------------------------------------------|
| Expected objects were not created  | Check whether the Generator for that layer ran at all ([Generator instances](./overview#generator-instances) in the UI) |
...
The Infrahub UI shows [Generator instances](./overview#generator-instances) per definition. Each instance corresponds to one target and shows its status:
...
Any node kind that participates in a cascade as a downstream target should inherit from the `GeneratorTarget` generic (see [Build chained generators](../learn/tutorials/generators/build-chained-generators)). This gives it the `checksum` attribute needed for trigger-based chaining.
...
Generators rely on `allow_upsert=True`, which matches objects by their [`human_friendly_id`](../schema/nodes-and-attributes#human-friendly-identifier-hfid). Every node type that a Generator creates must have a `human_friendly_id` defined in the schema:
...
The `branch_scope: "other_branches"` trigger configuration (from [Build chained generators](../learn/tutorials/generators/build-chained-generators)) ensures triggers only fire in branches, giving you a controlled testing environment.
