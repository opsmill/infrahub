...
Infrahub uses an object storage layer to persist binary and text content outside of the graph database. This layer stores the raw bytes of [file objects](../schema/file-object) and rendered [artifacts](../artifacts/overview). Separating file content from graph data allows Infrahub to keep the graph database focused on relationships, metadata, and version control while delegating bulk storage to a system optimized for that purpose.

## What uses object storage

Object storage serves two use cases in Infrahub:

- **File objects**: User-uploaded files attached to nodes in the graph. These are managed through the file object system and access is controlled by Infrahub's [permission system](../deploy-manage/user-management/permissions-roles/overview). See [file objects](../schema/file-object) for details.
- **Artifacts**: System-generated outputs from Transformations. These are created automatically by artifact definitions and stored in the same storage layer. See [artifacts](../artifacts/overview) for details.
...
Every stored object has a checksum recorded in the graph database when it is written: an MD5 for artifacts, a SHA-1 for file objects. Before the API returns the content of an artifact or a file object, it computes the checksum of what it read from object storage and compares it with the recorded one. Content that does not match, because it was modified or corrupted outside of Infrahub, is refused with an HTTP 409 error instead of being served, and the UI shows that error in place of the content.

A refused artifact is queued for regeneration, which writes it again from its definition. Artifacts are also rewritten whenever a regeneration finds their stored object missing or modified, even if the rendered content itself did not change. File objects cannot be regenerated: a refused file object must be re-uploaded or restored from a backup.

The verification protects against changes made directly in object storage. It does not replace access control on the bucket or the storage directory, and it does not encrypt the content, which remains readable to anyone with access to the storage.
...
<ReferenceLink title="How to configure Infrahub" url="../deploy-manage/install-configure/configure-infrahub" />
...
<ReferenceLink title="Production deployment guide" url="../deploy-manage/install-configure/production-deployment/overview" />
...
<ReferenceLink title="How to backup object storage" url="../deploy-manage/maintain-upgrade/database-backup/backup-and-restore#step-3-backup-the-object-storage" />

### Backend migration

:::warning Backend migration
Switching storage backends (for example, from `local` to `s3`) does not migrate existing content. Files stored under the previous backend become inaccessible. Artifacts are written again by regenerating their definitions, but user-uploaded file objects must be re-uploaded.
:::

## Connection to other concepts

- **[File objects](../schema/file-object)**: The primary way users interact with stored files. File objects are graph nodes that combine metadata with a reference to content in object storage.
- **[Artifacts](../artifacts/overview)**: System-generated content stored in the same storage layer, produced by Transformation pipelines.
- **[Permissions and roles](../deploy-manage/user-management/permissions-roles/overview)**: Access to file content is enforced at the API level based on permissions on the corresponding graph node.

## Further reading

- [Configure storage](./configure.mdx): Practical steps for uploading and retrieving content
- [File objects](../schema/file-object): Attaching files to nodes with full version control
