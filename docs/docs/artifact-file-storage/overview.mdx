...
Every artifact and file object has a checksum recorded in the graph database when it is written: an MD5 for artifacts, a SHA-1 for file objects. Before the API returns the content of an artifact or a file object, it computes the checksum of what it read from object storage and compares it with the recorded one. Content that does not match is refused with an HTTP 409 error instead of being served, and the UI shows that error in place of the content.
