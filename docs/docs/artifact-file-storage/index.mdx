...
Infrahub uses an object storage layer to persist binary and text content outside of the graph database. This layer stores the raw bytes of [file objects](../schema/file-object) and rendered [artifacts](../topics/artifact.mdx). Separating file content from graph data allows Infrahub to keep the graph database focused on relationships, metadata, and version control while delegating bulk storage to a system optimized for that purpose.

## What uses object storage

Object storage serves two use cases in Infrahub:

- **File objects**: User-uploaded files attached to nodes in the graph. These are managed through the file object system and access is controlled by Infrahub's [permission system](../topics/permissions-roles.mdx). See [file objects](../schema/file-object) for details.
- **Artifacts**: System-generated outputs from Transformations. These are created automatically by artifact definitions and stored in the same storage layer. See [artifacts](../topics/artifact.mdx) for details.
...
- **[Artifacts](../topics/artifact.mdx)**: System-generated content stored in the same storage layer, produced by Transformation pipelines.
- **[Permissions and roles](../topics/permissions-roles.mdx)**: Access to file content is enforced at the API level based on permissions on the corresponding graph node.
