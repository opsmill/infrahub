...
- Infrahub version, deployment type, and platform (Python / OS / architecture)
- Anonymous counters about the graph, schema, and features in use
- Adoption counts: active accounts, account groups, and open branches
- Database, worker, and infrastructure statistics
- Per-component CPU cores (available now, and the configured limit once one is enforced) and memory for the database, API server, and workers
- Daily activity over the previous day: logins, validation checks, artifact generation, branch operations, and webhook deliveries
