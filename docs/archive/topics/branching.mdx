...
5. **Merge**: Approved changes integrate into the main branch through a controlled process that preserves the integrity of the production environment. After a successful merge, the branch transitions to a frozen **`MERGED`** state — all data and schema mutations are blocked, and creating a new Proposed Change against it is prevented. The branch can still be viewed and deleted, but no further changes can be made to it.
6. **Cleanup**: The branch can be deleted after successful merge, maintaining a clean workspace and repository structure.

:::warning
A merged branch enters a terminal **`MERGED`** state immediately after merge. No further mutations are allowed on it — rebase, re-merge, and creating new Proposed Changes are all blocked. Only deletion remains permitted.

Once deleted, a branch additionally enters a `DELETING` terminal state and is removed permanently.
:::
