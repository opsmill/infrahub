...
2. Make sure the **List** view is selected using the toggle at the top-right of the page (the alternative **Graph** view shows relationships between models but does not display individual field weights)
3. Find your node type in the list
4. Click on the node to see its attributes and relationships
5. Note the current `order_weight` values (if any are set)
