...
Object templates expose a dedicated `member_of_groups_for_instances` relationship (and a `subscriber_of_groups_for_instances` counterpart). Groups assigned through this field are propagated to every object created from the template, while the template object itself is **not** added to those groups. This is useful when every instance produced from a given template should belong to the same operational group, for example "all devices created from `staged-spine` go into the `staged_devices` group".

:::info

If you want the template object itself to belong to a group (for example, to track templates as a set), use the regular `member_of_groups` field on the template instead. It behaves like on any other object and is independent from the `_for_instances` field.
